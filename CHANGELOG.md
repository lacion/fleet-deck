...
emitting a cryptic module error.

### Fixed

- **An incomplete install (missing daemon bundle) now names the real problem
  instead of a baffling `Cannot find module '.../takeover.ts'`.** `serve` resolves
  the daemon from the committed bundle (`src/daemon/fleetd.bundle.mjs`); a partial
  or interrupted `npm i -g fleetdeck` — or an in-place upgrade over a daemon still
  running out of the global dir — can land `bin/` without that bundle. `serve` then
  fell through to importing the unshippable TypeScript source and Bun reported
  `Cannot find module '.../src/daemon/takeover.ts' from '.../bin/fleetdeck.mjs'`,
  which points at the wrong file and offers no fix. `serve` now checks the daemon
  entrypoint exists **before** importing it and, when it does not, prints
  `install is incomplete … npm install -g fleetdeck` and exits `66` (EX_NOINPUT)
  without importing anything or binding a port. Service units and `supervise.sh`
  generated by 0.23.2+ treat exit `66` as a stop-the-loop code (alongside `3` and
  `78`), so a bundle-less install fails fast and readable rather than crash-looping.
  Reinstalling is the fix. (Surfaced by Coder workspaces after the 0.23.0 Bun
  cutover, where every workspace start re-installs the CLI from npm.)

## [0.23.1] - 2026-08-16

A stale service unit could crash-loop after an in-place upgrade to 0.23.0.

### Fixed

- **`fleetdeck serve` now fails fast under the wrong runtime instead of
  crash-looping.** From 0.23.0 the daemon serves over `Bun.serve`, so it must run
  under Bun. Upgrading a running install in place (`npm i -g fleetdeck@latest`)
  refreshes the package but does **not** rewrite an already-installed service
  unit, so a pre-0.23.0 `ExecStart=node …/fleetdeck.mjs serve` kept pointing Node
  at a now-Bun-only daemon — `ReferenceError: Bun is not defined` on every start,
  in a `Restart=always` hot loop. `serve` now preflights its runtime and, when it
  is not Bun (or Bun is older than the required `1.3.14`), exits `78` (EX_CONFIG)
  with a one-line fix on stderr **before** importing the daemon or binding a port —
  so the journal shows a readable message instead of a hot-looping `ReferenceError`
  stack, and no half-started daemon. Service units and `supervise.sh` generated by
  0.23.1+ additionally treat exit `78` as a stop-the-loop code (alongside the
  existing `3`); a unit installed before 0.23.1 keeps restarting until you re-run
  `fleetdeck service install`, but each restart now logs the fix and never binds.

  If you hit this, re-point the unit with Bun on `PATH`: `fleetdeck service
  install`. If your `service.env` carries a hand-tuned `PATH=` line (e.g. to pin a
  specific `tmux`), re-add it afterward — `service install` rewrites `service.env`.
