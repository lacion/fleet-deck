...
=======
4. Mail: `mail_pending` is a per-session count map `{<session_id>: <count>}` (sessions with no queued mail appear with 0). Compute N as the total across all sessions — `sum` of every value in the map. If N > 0: `mail: N queued — delivered at the next turn boundary`. Otherwise omit the line.
>>>>>>> /tmp/mf-theirs
