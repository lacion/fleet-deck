...
| `FLEETDECK_TRUST_LOOPBACK` | off | `on` waives the 0.16.0 loopback power gates (`/ws/term`, `POST /mail`, `gateway_*` writes, unsupervised arm) so a plain-loopback board needs no key — for single-user machines and port-forwarded Coder workspaces where no other OS user can reach 127.0.0.1. Hooks stay authenticated. Refuses to combine with LAN mode or `FLEETDECK_REQUIRE_TOKEN`. |
