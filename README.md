...
- **Loopback needs no token, ever.** Your hooks and your local board keep working exactly as before — this is why LAN mode changes nothing about a normal single-machine setup. (On a shared multi-user box, other local users sit inside that loopback trust zone; `FLEETDECK_REQUIRE_TOKEN=on` closes even loopback behind the token.)
...
| `FLEETDECK_REQUIRE_TOKEN` | off | `on` requires the bearer token even over loopback on every control and data route (`/state`, `/mail`, `/command`, `/api/*`, `/ws`, `/ws/term`); only `/hook/*` and the public board shell stay exempt. Closes the loopback trust zone against other OS users on a shared machine (and the Host-rewriting reverse-proxy residual). The fleet's own hooks attach the token from `$FLEETDECK_HOME/token` automatically, so a normal single-user setup keeps working. |
