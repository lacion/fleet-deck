...
| `FLEETDECK_CAPTURE_PAYLOADS` | off | `on` writes every hook payload to `$FLEETDECK_HOME/hook-payloads.jsonl` (mode `0600`) to debug schema drift. Off by default; secrets are scrubbed, but it's still raw telemetry — leave it off unless you're chasing a hook bug. |
