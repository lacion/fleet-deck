...
if (process.versions.bun) {
  ...
  process.emitWarning = function fleetdSqliteWarningFilter(warning, type, ...args2) {
...
var applyOverlays = (map3, overlay) => {
  if (!overlay) return;
  applyOverlays(map3, overlay.parent);
  map3.set(overlay.key, overlay.value);
};
var flatten = (self) => {
  if (self._flat) return self._flat;
  if (!self.overlay) return self._flat = self.base;
  const map3 = new Map(self.base);
  applyOverlays(map3, self.overlay);
  return self._flat = map3;
};
var withFlat = (self, f) => {
...
var make = (key, service2) => makeUnsafe(/* @__PURE__ */ new Map([[key.key, service2]]));
...
var addUnsafe = (self, key, service2) => {
  ...
  if (impl.depth >= MaxDepth) {
    const map3 = new Map(impl.mapUnsafe);
    map3.set(key, service2);
    return makeImpl(cacheRoot, map3, void 0, 0);
  }
...
var merge = /* @__PURE__ */ dual(2, (self, that) => {
  ...
  return withFlat(self, (map3) => that.mapUnsafe.forEach((value, key) => map3.set(key, value)));
});
...
var MixedScheduler = class {
  ...
  constructor(executionMode = "async", setImmediateFn) {
    ...
    this.setImmediate = setImmediateFn ?? (executionMode === "sync" ? setMicrotask : setImmediate2);
  }
  /**
...
var fiberAwait = (self) => {
  ...
  return callback((resume) => {
    ...
    return sync(self.addObserver((exit2) => resume(succeed(exit2))));
  });
};
...
var fiberInterrupt = (self) => withFiber((fiber2) => fiberInterruptAs(self, fiber2.id));
var fiberInterruptAs = /* @__PURE__ */ dual((args2) => hasProperty(args2[0], FiberTypeId), (self, fiberId2, annotations) => withFiber((parent) => {
...
var promise = (evaluate2) => callbackOptions(function(resume, signal) {
  internalCall(() => evaluate2(signal)).then((a) => resume(succeed(a)), (e) => resume(die(e)));
}, evaluate2.length !== 0);
...
var flatten2 = (self) => flatMap(self, identity);
...
var exitPrimitive = /* @__PURE__ */ makePrimitive({
  ...
  [contA](value, _, exit2) {
    return succeed(exit2 ?? exitSucceed(value));
  },
  [contE](cause, _, exit2) {
    return succeed(exit2 ?? exitFailCause(cause));
  }
});
...
var scopeForkUnsafe = (scope2, finalizerStrategy) => {
...
var scopeAddFinalizerExit = (scope2, finalizer) => {
  return suspend(() => {
    if (scope2.state._tag === "Closed") {
...
var scopeAddFinalizerUnsafe = (scope2, key, finalizer) => {
...
var scopeRemoveFinalizerUnsafe = (scope2, key) => {
...
var forkUnsafe = (parent, effect2, immediate = false, daemon = false, uninterruptible2 = false) => {
...
var runForkWith = (context2) => (effect2, options) => {
...
var runPromiseWith = (context2) => {
  const runPromiseExit3 = runPromiseExitWith(context2);
  return (effect2, options) => runPromiseExit3(effect2, options).then((exit2) => {
    if (exit2._tag === "Failure") {
...
var runSyncWith = (context2) => {
  const runSyncExit3 = runSyncExitWith(context2);
  return (effect2) => {
    const exit2 = runSyncExit3(effect2);
...
var fromBuild = (build2) => fromBuildUnsafe((memoMap, scope2) => {
  const layerScope = forkUnsafe2(scope2);
  return onExit(build2(memoMap, layerScope), (exit2) => exit2._tag === "Failure" ? close(layerScope, exit2) : void_);
});
var fromBuildMemo = (build2) => {
  const self = fromBuild((memoMap, scope2) => memoMap.getOrElseMemoize(self, scope2, build2));
  return self;
};
var memoMapBuild = (memoMap, layer, scope2, build2) => {
  const layerScope = makeUnsafe3();
  const deferred2 = makeUnsafe2();
  const entry = {
    observers: 1,
    effect: _await(deferred2),
  ...
  return scopeAddFinalizerExit(scope2, entry.finalizer).pipe(flatMap(() => build2(memoMap, layerScope)), onExit((exit2) => {
    entry.effect = exit2;
    return done2(deferred2, exit2);
  }));
};
var MemoMapImpl = class {
  ...
  getOrElseMemoize(layer, scope2, build2) {
    return suspend(() => {
      const existing = this.get(layer, scope2);
      if (existing) {
        return existing;
      }
      return memoMapBuild(this, layer, scope2, build2);
    });
  }
};
...
var CurrentMemoMap = class _CurrentMemoMap extends (/* @__PURE__ */ Service()("effect/Layer/CurrentMemoMap")) {
  static forkOrCreate(self) {
    const current = getOrUndefined(self, _CurrentMemoMap);
    return current ? forkMemoMapUnsafe(current) : makeMemoMapUnsafe();
  }
};
var buildWithMemoMap = /* @__PURE__ */ dual(3, (self, memoMap, scope2) => provideService(map(self.build(memoMap, scope2), add(CurrentMemoMap, memoMap)), CurrentMemoMap, memoMap));
var effect = function() {
  if (arguments.length === 1) {
...
var effectImpl = (service2, effect2) => effectContext(map(effect2, (value) => make(service2, value)));
...
var TypeId4 = "~effect/ManagedRuntime";
var make2 = (layer, options) => {
  ...
  const contextEffect = withFiber2((fiber2) => {
    ...
    return flatten3(await_(buildFiber));
  });
  const self = {
    [TypeId4]: TypeId4,
    memoMap,
    ...
    context() {
      return self.cachedContext === void 0 ? runPromise2(self.contextEffect) : Promise.resolve(self.cachedContext);
    },
    dispose() {
      return runPromise2(self.disposeEffect);
    },
    [Symbol.asyncDispose]() {
      return self.dispose();
    },
    disposeEffect: suspend2(() => {
      ;
      self.contextEffect = die2("ManagedRuntime disposed");
      self.cachedContext = void 0;
      return close(self.scope, void_2);
    }),
    runFork(effect2, options2) {
      return self.cachedContext === void 0 ? runFork2(provide2(self, effect2), mergeRunOptions(options2)) : runForkWith2(self.cachedContext)(effect2, mergeRunOptions(options2));
    },
    runCallback(effect2, options2) {
      return self.cachedContext === void 0 ? runCallback2(provide2(self, effect2), mergeRunOptions(options2)) : runCallbackWith2(self.cachedContext)(effect2, mergeRunOptions(options2));
    },
    runSyncExit(effect2) {
      return self.cachedContext === void 0 ? runSyncExit2(provide2(self, effect2)) : runSyncExitWith2(self.cachedContext)(effect2);
    },
    runSync(effect2) {
      return self.cachedContext === void 0 ? runSync2(provide2(self, effect2)) : runSyncWith2(self.cachedContext)(effect2);
    },
    runPromiseExit(effect2, options2) {
      return self.cachedContext === void 0 ? runPromiseExit2(provide2(self, effect2), mergeRunOptions(options2)) : runPromiseExitWith2(self.cachedContext)(effect2, mergeRunOptions(options2));
    },
    runPromise(effect2, options2) {
      return self.cachedContext === void 0 ? runPromise2(provide2(self, effect2), mergeRunOptions(options2)) : runPromiseWith2(self.cachedContext)(effect2, mergeRunOptions(options2));
    }
  };
  return self;
};
function provide2(managed, effect2) {
  return flatMap2(managed.contextEffect, (context2) => provideContext2(effect2, context2));
}

// src/daemon/app/services/app-config.ts
var AppConfig = class extends Service()(
  "fleetdeck/daemon/app/AppConfig"
) {
};

// src/daemon/app/services/process-runner.ts
var ProcessRunner = class extends Service()(
  "fleetdeck/daemon/app/ProcessRunner"
) {
};
function execEffect(request) {
  return flatMap2(ProcessRunner, (runner) => runner.run(request));
}
function execBoundedEffect(request) {
  return flatMap2(ProcessRunner, (runner) => runner.runBounded(request));
}

// src/daemon/app/errors.ts
var StartupConfigurationError = class extends TaggedError2("StartupConfigurationError") {
};
var ProcessRunnerStartupError = class extends TaggedError2("ProcessRunnerStartupError") {
};
var ProcessRunnerUnavailableError = class extends TaggedError2(
  "ProcessRunnerUnavailableError"
) {
};
var ProcessSpawnError = class extends TaggedError2("ProcessSpawnError") {
};
var ProcessNonZeroExitError = class extends TaggedError2("ProcessNonZeroExitError") {
};
var ProcessTimeoutError = class extends TaggedError2("ProcessTimeoutError") {
};
var ProcessOutputLimitError = class extends TaggedError2("ProcessOutputLimitError") {
};
var ApplicationQuiescingError = class extends TaggedError2("ApplicationQuiescingError") {
};

// src/daemon/app/services/process-driver.ts
var PROCESS_DRIVER_DEFAULT_TIMEOUT_MS = 3e4;
...
function processFailureEffect(request, result2) {
  ...
  switch (result2.code) {
    ...
    default:
      return fail2(
        typeof result2.code === "number" || result2.code === null ? new ProcessNonZeroExitError({ ...details, exitCode: result2.code }) : new ProcessSpawnError(details)
      );
  }
}
function interpretProcessResult(request, result2) {
  return result2.ok ? succeed2(result2) : processFailureEffect(request, result2);
}
function makeProcessRunnerServiceFromDriver(driver) {
  return {
    run(request) {
      return effectFromExecution(
        () => driver.start(request),
        (result2) => interpretProcessResult(request, result2)
      );
    },
    runBounded(request) {
      return effectFromExecution(
        () => driver.startBounded(request),
        (result2) => succeed2(result2)
      );
    }
  };
}
function makeProcessRunnerLayerFromDriver(acquireDriver) {
  const acquireService = acquireRelease2(
    acquireDriver,
    (driver) => promise2(() => driver.close())
  ).pipe(map2(makeProcessRunnerServiceFromDriver));
  return effect(ProcessRunner, acquireService);
}
...
var OUTPUT_LIMIT_RESULT = {
  ok: false,
  code: "ERR_CHILD_PROCESS_STDIO_MAXBUFFER",
  err: `subprocess output exceeded ${String(PROCESS_DRIVER_MAX_OUTPUT_BYTES)} bytes`
};
var PROCESS_REAP_BUDGET_MS = PROCESS_DRIVER_KILL_GRACE_MS;
var PROCESS_GROUP_POLL_MS = 10;
function deferred() {
  let resolveValue = () => void 0;
  const promise3 = new Promise((resolve) => {
    resolveValue = resolve;
  });
  return { promise: promise3, resolve: resolveValue };
}
function unrefTimer(callback3, milliseconds) {
  const timer = setTimeout(callback3, milliseconds);
  timer.unref();
  return timer;
}
...
function settlesWithin(promise3, milliseconds) {
  return new Promise((resolve) => {
    let settled = false;
    let timer = null;
    const finish = (value) => {
...
function spawnFailure(error) {
  ...
  return code === void 0 || code === "ERR_INVALID_ARG_VALUE" ? { ok: false, err } : { ok: false, code, err };
}
var EXEC_PROCESS_POLICY = {
  maxOutputBytes: PROCESS_DRIVER_MAX_OUTPUT_BYTES,
  ...
  onExit: (exit2, output) => {
    if (exit2.error) return spawnFailure(exit2.error);
    const out = output.stdout.toString("utf8");
    const err = output.stderr.toString("utf8").trim();
    return exit2.code === 0 ? { ok: true, out } : { ok: false, code: exit2.code, err: err || `process exited ${String(exit2.code ?? "")}` };
  }
};
function completedCancellation() {
  return {
...
function completedDecision(result2) {
  return {
    decision: Promise.resolve(result2),
    cleanup: Promise.resolve(),
    cancel() {
    }
  };
...
function legacySpawnValidationMessage(request) {
  ...
  for (let index = 0; index < arguments_.length; index += 1) {
    ...
    if (value === void 0 || !value.includes("\0")) continue;
...
function boundedOutput(output, overrides) {
  return {
    ...overrides,
...
function boundedSpawnFailure(error) {
  const code = errorCode(error);
  const rawErrno = typeof error === "object" && error !== null && "errno" in error && typeof error.errno === "number" ? error.errno : null;
...
function boundedProcessPolicy(request) {
  return {
    ...
    onTimeout: (_timeoutMs, output) => boundedOutput(output, { code: null, truncated: true, timedOut: true }),
    onOutputLimit: (output) => boundedOutput(output, { code: null, truncated: true, timedOut: false }),
    onExit: (exit2, output) => exit2.error ? boundedSpawnFailure(exit2.error) : boundedOutput(output, {
      code: exit2.code,
      truncated: false,
      timedOut: false
    })
...
var BunProcessExecution = class {
  ...
  spawn(request) {
    const timeout2 = request.timeoutMs ?? PROCESS_DRIVER_DEFAULT_TIMEOUT_MS;
    const stdin = request.stdin === void 0 ? "ignore" : typeof request.stdin === "string" ? Buffer.from(request.stdin) : request.stdin;
    try {
      ...
      this.drains = Promise.all([stdoutDrain, stderrDrain]).then(() => void 0);
      this.deadline = unrefTimer(() => {
        this.selectTermination(() => this.policy.onTimeout(timeout2, this.output()));
      }, timeout2);
  ...
  selectTerminalDecision(result2) {
    if (this.terminalSelected) return false;
    this.terminalSelected = true;
    this.publishDecision(result2);
    return true;
  }
  selectTermination(decision) {
    if (this.terminalSelected) return false;
    this.terminalSelected = true;
    this.stopAdmission();
  ...
  async terminateAndReap(child) {
    const cleanupDeadline = Date.now() + this.policy.killGraceMs + PROCESS_REAP_BUDGET_MS;
    const streams = this.cancelStreams();
    try {
      if (this.killTree) {
        const groupGone = this.policy.killGraceMs === 0 ? false : await this.waitForProcessGroupGone(child.pid, this.policy.killGraceMs);
        ...
        const exitedAfterTerm = this.exitObservation !== null || this.policy.killGraceMs > 0 && await settlesWithin(this.exited.promise, this.policy.killGraceMs);
  ...
  async waitForProcessGroupGone(pid, milliseconds) {
    const deadline = Date.now() + milliseconds;
    while (this.processGroupAlive(pid)) {
      const wait = remaining(deadline);
      if (wait === 0) return false;
      await delay2(Math.min(PROCESS_GROUP_POLL_MS, wait));
    }
    return true;
  }
  cancelStreams() {
...
function startBunProcessAttempt(request, policy) {
  return new BunProcessExecution(request, policy);
}
var BunProcessDriver = class {
...
function makeBunProcessDriver() {
  return new BunProcessDriver();
}

// src/daemon/platform/bun/process-runner-live.ts
var ProcessRunnerLive = makeProcessRunnerLayerFromDriver(
  sync2(makeBunProcessDriver)
);

// src/daemon/app/bootstrap-process-runtime.ts
var CANCELLED_RESULT2 = {
  ok: false,
  code: "ECANCELED",
  err: "cancelled"
};
...
function exitResult(exit2) {
  if (isSuccess(exit2)) return exit2.value;
  const defect = exit2.cause.reasons.find(isDieReason2);
  if (defect) throw defect.defect;
  const failure2 = exit2.cause.reasons.find(isFailReason2);
  if (failure2) {
    return failure2.error._tag === "ProcessRunnerStartupError" ? expectedFailure(failure2.error) : failure2.error.result;
  }
  if (hasInterruptsOnly2(exit2.cause)) return CANCELLED_RESULT2;
  throw squash(exit2.cause);
}
function boundedExitResult(exit2) {
  if (isSuccess(exit2)) return exit2.value;
  const defect = exit2.cause.reasons.find(isDieReason2);
  if (defect) throw defect.defect;
  const failure2 = exit2.cause.reasons.find(isFailReason2);
  if (failure2) {
    return {
      code: null,
      stdout: Buffer.alloc(0),
      stderr: failure2.error.message || failure2.error._tag,
      truncated: false,
      timedOut: false
    };
  }
  if (hasInterruptsOnly2(exit2.cause)) {
    return {
      code: null,
      stdout: Buffer.alloc(0),
      stderr: "cancelled",
...
function createBootstrapProcessRuntimeBridge(layer = ProcessRunnerLive) {
  let phase = "open";
  let runtime = null;
  ...
  const submit = (effect2, fromExit, cancelled) => {
    if (phase !== "open") return Promise.resolve(cancelled);
    const controller = new AbortController();
    let exit2;
    try {
      exit2 = getRuntime().runPromiseExit(effect2, { signal: controller.signal });
    } catch (defect) {
      return Promise.reject(defect);
    }
    const tracked = { controller, exit: exit2 };
    active.add(tracked);
    const forget = () => {
      active.delete(tracked);
    };
    void exit2.then(forget, forget);
    return exit2.then(fromExit);
  };
  const run = (request) => submit(execEffect(request), exitResult, CANCELLED_RESULT2);
  const runBounded2 = (request) => submit(execBoundedEffect(request), boundedExitResult, {
    code: null,
    stdout: Buffer.alloc(0),
    stderr: "cancelled",
    truncated: true,
  ...
  return { run, runBounded: runBounded2, quiesce, close: close2 };
}
...
function git(args2, cwd) {
  try {
    const raw = execFileSync("git", args2, {
      cwd,
      timeout: 1500,
      encoding: "utf8",
...
function blockedPaths(root, limit = 8) {
  ...
  const out = [];
  const ownerOf = (st) => {
    const cached2 = owners.get(st.uid);
...
function tailLines(transcriptPath, { maxBytes = 262144 } = {}) {
  ...
  const it = Object.assign(gen2, { truncated: start > 0 });
  return it;
}
...
function boundedPayload(value, maxBytes) {
  let remaining2 = Math.max(0, maxBytes);
  const seen = /* @__PURE__ */ new WeakSet();
  const marker = "[truncated]";
  function textWithinBudget(value2) {
    if (remaining2 <= 0) return marker;
    let out = String(value2).slice(0, remaining2);
    while (out && Buffer.byteLength(out) > remaining2)
      out = out.slice(0, Math.floor(out.length * 0.75));
    remaining2 -= Buffer.byteLength(out);
  ...
  function visit(current, depth = 0) {
    ...
    if (Array.isArray(current)) {
      const arr = current;
      const out2 = [];
      for (let i = 0; i < arr.length && remaining2 > 0; i++) out2.push(visit(arr[i], depth + 1));
    ...
    for (const key in obj) {
      if (!Object.hasOwn(obj, key) || remaining2 <= 0) continue;
      remaining2 -= Math.min(remaining2, Buffer.byteLength(key) + 4);
      if (isSecretKey(key)) {
        out[key] = REDACTED;
        continue;
      }
...
// src/daemon/exec.ts
var execFileDelegate = null;
function bindExecFileDelegate(delegate) {
...
function execFileP(cmd, args2, options = {}) {
  ...
  if ("maxBytes" in options) {
    ...
    return Promise.resolve().then(() => delegate.runBounded(request2));
  }
  const { timeout: timeout2 = 3e4, env, signal, killTree = false, cwd, stdin } = options;
  const request = {
    argv: [cmd, ...args2],
    timeoutMs: timeout2,
    ...env === void 0 ? {} : { env },
...
async function baseBranch(worktree) {
  const head2 = await execFileP(
    "git",
    ["-C", worktree, "symbolic-ref", "--short", "refs/remotes/origin/HEAD"],
    { timeout: 5e3 }
  );
  if (head2.ok && head2.out.trim()) return { ref: head2.out.trim(), local: false };
...
function parseTmuxVersion(output) {
  const match2 = /^tmux\s+(\d+)\.(\d+)([a-z][a-z0-9-]*)?\s*$/i.exec(String(output ?? ""));
  if (!match2) return null;
  const [, majorStr, minorStr, suffix] = match2;
...
var FIELD_SEP = "~";
async function tmuxResult(args2, { noStart = false } = {}) {
  try {
    const socket = process.env["FLEETDECK_TMUX_SOCKET"]?.trim();
    const argv = [...socket ? ["-L", socket] : [], ...noStart ? ["-N"] : [], ...args2];
...
async function tmux(args2) {
  const result2 = await tmuxResult(args2);
  return result2.ok ? result2.out : null;
}
...
async function readPersistedGeneration(home, port) {
  ...
  try {
    ...
    const rec = record;
    const gen2 = rec.generation;
    const pid = rec.serverPid;
    if (keys.length !== 2 || keys[0] !== "generation" || keys[1] !== "serverPid" || typeof gen2 !== "string" || !GENERATION_UUID_RE.test(gen2) || typeof pid !== "number" || !Number.isSafeInteger(pid) || pid <= 1) {
      throw new Error("persisted tmux generation is malformed");
    }
    return { generation: gen2.toLowerCase(), serverPid: pid, legacy: false };
  } finally {
    await handle.close();
  }
}
...
async function readServerGeneration(port) {
  const result2 = await tmuxResult(
  ...
  if (!result2.ok) {
    return {
      reachable: false,
      absent: SERVER_ABSENT_RE.test(result2.error),
      generation: null,
      serverPid: null
    };
  }
  const value = result2.out.endsWith("\n") ? result2.out.slice(0, -1) : result2.out;
...
async function generationVerifiedResult(port, args2) {
  let state;
  try {
  ...
  if (!state.enabled) return tmuxResult(args2);
  ...
  const newline = result2.out.indexOf("\n");
  if (newline === -1 || result2.out.slice(0, newline) !== `${GENERATION_HEADER}${state.expected.generation}${FIELD_SEP}${String(state.expected.serverPid)}`) {
    return { ok: false, generationError: "tmux server generation unavailable or changed" };
  }
  return { ok: true, out: result2.out.slice(newline + 1), generation: state.expected.generation };
}
...
function exactTargetPort(target) {
  const match2 = /^=fleetdeck-(\d+):=fd(\d+)-[A-Za-z0-9-]+$/.exec(target);
  return match2?.[1] !== void 0 && match2[1] === match2[2] ? match2[1] : null;
}
var sessionConfirmed = (result2) => result2.ok && !result2.authoritativeEmpty;
...
async function newWindow({
  ...
  const inspect = (args2) => generation.enabled ? generationVerifiedResult(port, args2) : tmuxResult(args2);
...
async function paneCurrentCommand(target) {
  ...
  const result2 = port === null ? await tmuxResult(args2) : await generationVerifiedResult(port, args2);
  if (!result2.ok) return null;
  const out = result2.out;
...
async function killWindowVerified(name, opts) {
  const scope2 = /^fd(\d+)-[^\x00-\x1f\x7f]+$/.exec(name);
  if (!scope2) return { ok: false, error: "invalid scoped tmux window name" };
  if (/[=;:.]/.test(name)) return { ok: false, error: "invalid scoped tmux window name" };
  const scopePort = scope2[1];
  if (scopePort === void 0) return { ok: false, error: "invalid scoped tmux window name" };
  const expectedSession = sessionName(scopePort);
  const format2 = ["#{session_name}", "#{window_name}", "#{window_id}"].join(FIELD_SEP);
  const listArgs = [
    "list-panes",
    "-a",
    "-f",
...
async function sendEnter(target) {
  ...
  return result2.ok && result2.out.trim() !== GENERATION_MISMATCH;
}
...
async function capturePane(target) {
  const args2 = ["capture-pane", "-p", "-t", target];
  const port = exactTargetPort(target);
  if (port === null) return tmux(args2);
  const result2 = await generationVerifiedResult(port, args2);
  return result2.ok ? result2.out : null;
}
function sendBringupEnter(target) {
  return sendEnter(target);
}
function launchOverride(cmd, spec, onError2 = () => {
}) {
  try {
...
function createStatements(db2) {
  const cached2 = statementsByDb.get(db2);
...
async function gitCommonDir(dir) {
  const result2 = await execFileP("git", ["-C", dir, "rev-parse", "--git-common-dir"], {
    timeout: 5e3
  });
  if (!result2.ok) return null;
  const raw = result2.out.trim();
  if (!raw) return null;
    ...
    try {
      ...
      try {
        if (state.exists) {
          ...
          const args2 = ["-C", repo, "worktree", "remove"];
          if (body.force === true) args2.push("--force");
          args2.push(worktreePath);
          const removed = await execFileP("git", args2, { timeout: 3e4 });
          if (!removed.ok) {
...
function originSecrets(origin_url) {
  ...
  for (const match2 of origin.matchAll(ORIGIN_PARAM_VALUE_RE)) parts.push(match2[1] ?? "");
...
async function originOf(root) {
  const result2 = await execFileP("git", ["-C", root, "remote", "get-url", "origin"], {
    timeout: 5e3
  });
  return result2.ok ? result2.out.trim() || null : null;
}
...
function createRepos(ctx) {
  ...
  function accessFailure(origin, result2) {
    const { provider, transport } = originKind(origin);
    const raw = result2.err ?? "";
  ...
  async function probeRepoAccess(origin, signal) {
    ...
    const timeout2 = Number.isFinite(configured) && configured >= 1e3 && configured <= 6e4 ? configured : 15e3;
    const result2 = await execFileP("git", ["ls-remote", "--", origin, "HEAD"], {
      timeout: timeout2,
      signal,
      killTree: true,
      env: { GIT_TERMINAL_PROMPT: "0", GCM_INTERACTIVE: "Never" }
    });
    if (!result2.ok) return accessFailure(origin, result2);
  ...
  async function validateBranch(branch) {
    ...
    const result2 = await execFileP("git", ["check-ref-format", "--branch", branch], {
      timeout: 5e3
    });
    if (!result2.ok) throw namedError(400, result2.err || "branch is not a valid git branch name");
    return branch;
  }
  ...
  async function cloneRepo({
    ...
    try {
      ...
      const timeout2 = Number.isFinite(configuredTimeout) && configuredTimeout > 0 ? configuredTimeout : 6e5;
      const result2 = await execFileP("git", ["clone", "--", origin_url, temp], {
        timeout: timeout2,
        env: { GIT_TERMINAL_PROMPT: "0", GCM_INTERACTIVE: "Never" },
        signal,
        killTree: true
      ...
      if (!result2.ok) {
        if (result2.code === "ECANCELED") throw namedError(409, "spawn cancelled");
        if (result2.err) console.error(`fleetd clone failed \u2014 ${origin_url}
${result2.err}`);
        const { note, detail } = gitFailureText(result2.err, origin_url);
  ...
  async function materializeBranch({
    ...
    const git2 = async (args2, options = {}) => {
      const result2 = await execFileP("git", args2, {
        ...options,
        signal,
        killTree: true
      });
      if (!result2.ok && result2.code === "ECANCELED") throw namedError(409, "spawn cancelled");
      return result2;
    };
    ...
    if (mode === "in-place") {
      ...
      const args2 = local.ok ? ["-C", root, "switch", branch] : remote.ok ? ["-C", root, "switch", "--track", `origin/${branch}`] : ["-C", root, "switch", "-c", branch, requireBaseRef()];
      const switched = await git2(args2, { timeout: 3e4 });
    ...
    for (const candidate of candidates) {
      ...
      const args2 = local.ok ? ["-C", root, "worktree", "add", candidate, branch] : remote.ok ? ["-C", root, "worktree", "add", "--track", "-b", branch, candidate, `origin/${branch}`] : ["-C", root, "worktree", "add", "-b", branch, candidate, requireBaseRef()];
      last = await git2(args2, { timeout: 3e4 });
...
function runBounded(cmd, args2, { cwd, timeoutMs, maxBytes, input = null }) {
  return execFileP(cmd, args2, {
    timeout: timeoutMs,
    maxBytes,
    ...cwd === void 0 ? {} : { cwd },
...
function parseGitGrep(raw, hitCap) {
  ...
  for (const record of raw.toString("utf8").split("\n")) {
    if (!record) continue;
    const match2 = /^(.*?):([0-9]+):(.*)$/.exec(record);
    if (!match2) continue;
    const [, file = "", lineStr = "", text = ""] = match2;
...
async function gitSearch(root, q, mode, deadline) {
  const remaining2 = () => Math.max(1, deadline - Date.now());
  if (mode === "name") {
...
function createFiles(ctx) {
  ...
  async function searchAt(resolve, q, { mode } = {}) {
    ...
    try {
      ...
      const result2 = resolved.git ? await gitSearch(resolved.root, q, mode, deadline) : await walkSearch(resolved.root, q, mode, deadline);
      return {
        status: 200,
        body: {
          ok: true,
...
function createMail(ctx) {
  ...
  function own(promise3) {
    inFlight.add(promise3);
    const forget = () => {
      inFlight.delete(promise3);
    };
    void promise3.then(forget, forget);
    return promise3;
  }
  ...
  function resolveTargets(to) {
    const all2 = q.visibleSessions.all();
    const active = all2.filter((s) => s.ended_at == null);
    ...
    const routable = all2.filter((s) => s.source !== "shell");
  ...
  function notifyWatchers(sid) {
    for (const fn2 of [...watchWaiters.get(sid) ?? []]) {
      try {
        fn2();
      } catch {
      }
  ...
  function addWatchWaiter(sid, fn2) {
  ...
  async function tryOwnedPaneDeliveryImpl(sid) {
    ...
    const { batch: box, remaining: remaining2 } = claimAllMail(sid);
    if (!box.length) return false;
    if (remaining2) rearmPaneMailTimer(sid);
  ...
  function claimMail(sid, gen2 = null) {
    if (gen2 !== null && !isWatchGen(sid, gen2)) return null;
    const now = Date.now();
  ...
  const mailLifecycle = { quiesce, close: close2 };
  return {
    mail,
    drainMail,
    ackMail,
...
function createCommands(ctx) {
  ...
  function clearTicket(sid) {
    ...
    let result2 = { ok: true, renamed: false, callsign: c.callsign, ticket: null };
  ...
  function command(text) {
    ...
    if (parsed.cmd === "broadcast") {
      ...
      if (/^clear$/i.test(parsed.ticket)) {
        result2 = clearTicket(resolved.sid);
      } else {
        ...
        result2 = applyTicket(resolved.sid, key, "manual");
      }
      logCommand({ result: result2 });
      onMutate();
      return { session_id: resolved.sid, ...result2 };
      ...
      const result2 = applyCustomName(resolved.sid, clearing ? null : parsed.suffix);
      logCommand({ result: result2 });
      onMutate();
      return { session_id: resolved.sid, ...result2 };
    } else {
...
function createSpawns(ctx) {
  ...
  function runMaintenance(operation) {
    ...
    let promise3;
    try {
      promise3 = Promise.resolve(operation());
    } catch (err) {
      promise3 = Promise.reject(err);
    }
    inFlightMaintenance.add(promise3);
    const forget = () => {
      inFlightMaintenance.delete(promise3);
    };
    void promise3.then(forget, forget);
    return promise3;
  }
  ...
  function scheduleRegistrationRemoteHarvest(spawn_id) {
    ...
    const promise3 = delayedRemoteHarvest(spawn_id);
    registrationRemoteHarvests.set(spawn_id, promise3);
    return promise3;
  }
  ...
  async function spawn2(body) {
    ...
    const wrapSpawnFailure = (fn2) => async (...a) => {
      try {
        return await fn2(...a);
      } catch (err) {
        releasePlanClaim();
    ...
    if (body.worktree === true) {
      ...
      if (!result2.ok) {
        const addErr = scrubUrlCredentials(result2.err);
        await spawnCompensate({
          spawn_id,
          session_id,
          callsign,
  ...
  async function adoptSession(session_id, body = {}, { deferred: deferred2 = false } = {}) {
    ...
    const adoptArmRefusal = deferred2 ? null : unsupervisedGate(skip, body);
    ...
    if (deferred2 && c.adopt_armed_until == null) {
      return { status: 200, body: { ok: true, canceled: true } };
    }
    if (deferred2 && c.adopt_armed_until != null && c.adopt_armed_until <= Date.now()) {
      updateSession(session_id, { adopt_armed_until: null, adopt_armed_skip: null });
      tick(
  ...
  function enableRemote(spawn_id) {
    ...
    const promise3 = enableRemoteOnce(spawn_id).finally(() => {
      if (remoteEnables.get(spawn_id) === promise3) remoteEnables.delete(spawn_id);
    });
    remoteEnables.set(spawn_id, promise3);
    return promise3;
  }
  async function enableRemoteOnce(spawn_id) {
    ...
    let timeout2;
    const timed2 = new Promise((resolve) => {
      timeout2 = setTimeout(() => {
    ...
    return {
      status: 200,
      body: { ok: true, enabled: true, url: result2.url, pending: result2.pending }
    };
  }
...
${detail}` : note);
  ...
  const ownedSpawn = (body) => spawnMaintenance.run(() => spawn2(body)) ?? Promise.resolve(shuttingDown2());
...
function validateHookEvent(input) {
  if (!isRecord(input)) {
    return fail3("hook body must be a JSON object");
  }
  if (!isNonEmptyString(input["session_id"])) {
    return fail3("hook body requires a non-empty string session_id");
  }
  return ok(input);
}
...
function validateSpawnRequest(input) {
  if (!isRecord(input)) {
    return fail3("spawn body must be a JSON object");
  }
  return ok(input);
}
...
var ControlModeParser = class {
  ...
  #line(line, events) {
    ...
    const exit2 = /^%exit(?:\s+(.*))?$/.exec(line);
    if (exit2) {
      events.push({ type: "exit", reason: exit2[1] || "tmux control client exited" });
      return;
    }
...
function createTermBridge({
  ...
  function createClient() {
    ...
    const child = spawn(override2 || "tmux", override2 ? [] : argv, {
  ...
  async function openViewer({
    ...
    try {
      ...
      const match2 = /^(\d+)\s+(\d+)(?:\s+(\d+))?\s*$/.exec(cursor.lines.at(-1)?.trim() ?? "");
      if (!match2) throw new Error("terminal cursor lookup returned invalid data");
      const bracketedPaste = match2[3] === "1";
      ...
      send({
        ...
        screen: CLEAR_SCREEN + Buffer.from(captured.lines.join("\r\n"), "latin1").toString("utf8") + `\x1B[${Number(match2[2]) + 1};${Number(match2[1]) + 1}H` + // Replay the pane's bracketed-paste (DEC 2004) mode into the seed after
  ...
  async function settlesWithin2(promise3, milliseconds) {
    let timer;
    try {
  ...
  async function terminateClient(c) {
    ...
    const exitedDuringGrace = await settlesWithin2(c.exited, terminateGraceMs);
    ...
    if (!exitedDuringGrace) {
      const exitedAfterKill = await settlesWithin2(c.exited, terminateGraceMs);
  ...
  return { openViewer, close: close2 };
}
...
var HttpReqShim = class {
  ...
  async _pump() {
    ...
    try {
      for (; ; ) {
        const { done: done3, value } = await reader.read();
        if (done3) break;
        if (this._destroyed) {
...
function serveBoardAsset(res, pathname, notFound2) {
  let decoded;
  try {
  ...
  if (abs !== BOARD_DIST && !abs.startsWith(BOARD_DIST + path16.sep)) return notFound2();
  let data;
  try {
...
function createHttp(core2, {
  ...
  function settleFilesystemOperation(res, scope2, operation) {
    operation.then(({ status, body }) => {
      json(res, status, body);
    }).catch((err) => {
      console.error(`fleetd ${scope2} filesystem error:`, err);
...
function encodeRdata(type, data) {
  switch (type) {
    ...
    case TYPE.SRV: {
      const srv = data;
      const head2 = Buffer.alloc(6);
      head2.writeUInt16BE(srv.priority, 0);
      head2.writeUInt16BE(srv.weight, 2);
      head2.writeUInt16BE(srv.port, 4);
      return Buffer.concat([head2, encodeName(srv.target)]);
    }
    default:
...
function encodeRecord(record) {
  ...
  const head2 = Buffer.alloc(8);
  head2.writeUInt16BE(type, 0);
  head2.writeUInt16BE(CLASS_IN | (record.flush ? FLUSH_BIT : 0), 2);
  head2.writeUInt32BE(Math.max(0, Number(record.ttl) || 0), 4);
  const rdlength = Buffer.alloc(2);
  rdlength.writeUInt16BE(rdata.length, 0);
  return Buffer.concat([name, head2, rdlength, rdata]);
}
...
function ptrRecord(_ad, service2, ttl) {
  return {
    name: service2.type,
    type: "PTR",
    ttl: ttlFor("PTR", ttl),
...
function srvRecord(ad, service2, ttl, flush) {
  return {
    name: service2.name,
    type: "SRV",
    ttl: ttlFor("SRV", ttl),
...
function txtRecord(ad, service2, ttl, flush) {
  return { name: service2.name, type: "TXT", ttl: ttlFor("TXT", ttl), flush, data: ad.txt };
}
function metaRecords(ad, ttl) {
  return ad.services.map((service2) => ({
    name: META_QUERY,
    type: "PTR",
    ttl: ttlFor("PTR", ttl),
    flush: false,
...
function buildAnnouncement(options = {}, { ttl } = {}) {
  ...
  for (const service2 of ad.services) {
    records.push(
      ptrRecord(ad, service2, ttl),
      srvRecord(ad, service2, ttl, flush),
      txtRecord(ad, service2, ttl, flush)
    );
  }
  const seen = /* @__PURE__ */ new Set();
...
function buildResponse(questions, options = {}, { ttl, flush = true } = {}) {
  ...
  for (const q of questions) {
    ...
    for (const service2 of ad.services) {
      if (qname === service2.type.toLowerCase() && wants(q, "PTR")) {
        answers.push(ptrRecord(ad, service2, ttl));
        additionals.push(
          srvRecord(ad, service2, ttl, flush),
          txtRecord(ad, service2, ttl, flush),
      ...
      if (qname === service2.name.toLowerCase()) {
        if (wants(q, "SRV")) {
          answers.push(srvRecord(ad, service2, ttl, flush));
          additionals.push(...aRecords(ad, ttl, flush));
        }
        if (wants(q, "TXT")) answers.push(txtRecord(ad, service2, ttl, flush));
      }
    }
  }
...
function uniqueConflict(msg, options = {}, { phase } = {}) {
  ...
  const services = new Set(ad.services.map((service2) => service2.name.toLowerCase()));
...
function createMdns({
  ...
  function sendRaw(packet, targetPort, targetAddress, callback3) {
  ...
  function sendMulticastAll(buildFor, done3) {
  ...
  function onBound() {
    ...
    try {
      ...
      die3("cannot set unicast TTL 255", err);
      return;
    }
    try {
      sock.setMulticastTTL(255);
    } catch (err) {
      die3("cannot set multicast TTL 255", err);
      return;
    }
    try {
    ...
    if (joins === 0) {
      die3("no multicast membership");
      return;
    }
    claimed = true;
    for (const delay3 of ANNOUNCE_DELAYS_MS)
      schedule(() => {
        announce();
      }, delay3);
    note(
    ...
    try {
      sock.bind({ port: MDNS_PORT }, () => {
        try {
          ...
          die3("bind setup failed", err);
        }
      });
    } catch (err) {
      die3("bind failed", err);
    }
  }
  ...
  function stop() {
    ...
    stopping = new Promise((resolve) => {
      ...
      try {
        ...
        if (joinedIfaces.length) {
          sendMulticastAll((iface) => announcementFor(iface, 0), done3);
        } else {
          const sock = socket;
          const answers = buildAnnouncement(options, { ttl: 0 });
          if (sock)
...
var MockSocket = class {
  ...
  bind(_options, callback3) {
    if (callback3) setImmediate(callback3);
    return this;
  }
  send(packet, _port, address, callback3 = () => {
  }) {
    const wire = Buffer.from(packet).toString("base64");
    recordMdns({ type: "send", id: this.id, wire, address });
    const delay3 = Number(process.env["FLEETDECK_MDNS_SEND_DELAY_MS"] ?? 150);
    const timer = setTimeout(() => {
...
function installConsoleRecorder() {
  ...
  console.log = (...args2) => {
    try {
      appendFileSync(file, `${args2.map((a) => String(a)).join(" ")}
`);
    } catch {
    }
    original(...args2);
