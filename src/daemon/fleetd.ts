...
function startupFatal(reason: string): never {
  ...
  try {
    ...
    /* startup refusal must still exit */
  }
  ...
  try {
    fs.writeSync(2, `fleetd refused to start: ${reason}\n`);
  } catch {
    /* exit still wins */
  }
  process.exit(1);
...
function removeOwnedPidFile(): void {
  ...
  try {
    ...
    /* shutdown/election cleanup is best effort */
  }
  ownsPidFile = false;
}
...
async function supersedeIfNewer(record: PidRecord): Promise<boolean> {
  ...
  try {
    ...
    incumbent = (await res.json()) as { managed?: boolean; version?: string; pid?: number } | null;
  } catch {
    return false;
