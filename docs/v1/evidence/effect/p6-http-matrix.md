...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| G1 | `/health` | loopback-open; bearer on LAN | no | **async** as of the P6.4 pilot (dispatched through the Effect bridge, joined via `res.done`/`activeResponses`); **snapshot settler** — quiesce/interrupt falls back to the legacy sync handler (always-200). Join-on-interrupt does not apply. Rollback: `effectRoutes=null` | `200 {ok:true, fleet, pid, version, managed, spawn, auth:termAuth, startup}` | 401/403 walls | `1450`–`1478` (freeze); live dispatch `dispatchHealth` ~`1922`–`1932` | `security-headers.test.ts` (nosniff+json), `lan-auth.test.ts`, `loopback-gates.test.ts`, `http-workflow-health-state.test.ts` |
| G2 | `/state` | loopback-open; bearer on LAN | no (walled by Host allowlist) | **async** as of the P6.4 pilot (dispatched through the Effect bridge, joined via `res.done`/`activeResponses`); **snapshot settler** — quiesce/interrupt falls back to the legacy sync handler (always-200). Join-on-interrupt does not apply. Rollback: `effectRoutes=null` | `200 snapshotWithLan()` — **includes token-bearing `lan.urls`/`lan.mdns`** | 401/403 walls | `1479`–`1481`, `771` (freeze); live dispatch `dispatchState` ~`1935`–`1936` | `static-serving.test.ts` (regression), `lan-auth.test.ts`, `http-workflow-health-state.test.ts` |
...
| P6 | `POST /api/settings` | loopback-open **+ gateway_* bearer gate** | **async** as of the P6.4 settings slice (dispatched through the Effect bridge via `settleEffectMutatingRoute`); the core call is `Effect.sync` so interrupt-after-start is a no-op. **Sync mutating:** intra-quiesce answers frozen `503 {"ok":false,"reason":"shutting-down"}` and does NOT replay the write. Gateway `gateway_*` bearer gate still runs BEFORE dispatch. Rollback: `effectRoutes=null` | `<out.status> <out.body>` | `gateway_*` key without bearer (unless `trustLoopback && !viaProxy && loopback`) → `401 {ok:false, reason:'gateway settings require the bearer token'}`; intra-quiesce 503 | `1776`–`1811` (freeze); live `dispatchSettings` ~`2210`–`2238` | `gateway.test.ts`, `settings-transaction.test.ts` (domain), `http-workflow-settings-command-mail-cleanup.test.ts` |
| P7 | `/command` | loopback-open; bearer on LAN | **async** as of the P6.4 settings slice (dispatched through the Effect bridge via `settleEffectMutatingRoute`); `Effect.sync`, interrupt-after-start is a no-op. **Sync mutating:** intra-quiesce 503, never replay. Rollback: `effectRoutes=null` | `200 core.command(text)` | walls; intra-quiesce 503 | `1812`–`1815` (freeze); live `dispatchCommand` ~`2241`–`2242` | `fleet-command.test.ts` (domain), `http-workflow-settings-command-mail-cleanup.test.ts` |
| P8 | `/api/paste-image` | loopback-open; bearer on LAN | **async** as of the P6.4 paste slice (dispatched through the Effect bridge, joined via `res.done`/`activeResponses`); legacy sync handler retained as the rollback seam. **Sync mutating** (`Effect.sync`): intra-quiesce (ingress refused, transport still admitting) answers the frozen shutdown `503 {"ok":false,"reason":"shutting-down"}` and does NOT replay the write. Join-on-interrupt does not apply. Validation/limit/dir/write failures remain data responses (E=never). | `<out.status> <out.body>` — 201 `{ok:true, path, bytes}` (path ends `.png`/`.jpg`/`.gif`/`.webp` from the sniff); 400/413/500 `{ok:false, reason}` from paste.ts | body cap = **MAX_PASTE_BODY (14e6)** (§2); walls; intra-quiesce 503 | `2245`–`2255`, `2063` | `paste-image.test.ts`, `http-workflow-paste.test.ts` |
...
| P12 | `/api/spawn/:id/kill` | loopback-open; bearer on LAN | **async** through the Effect bridge (`controlAsyncWorkflow` + `settleControlAsyncRoute` + `startOnce`); **join-on-interrupt**. Rollback: `effectRoutes=null` | `<out.status> <out.body>` (derive: 404 unknown / 409 not offline w/o force / 410 gone) | `.catch` → `500 {ok:false, reason:'internal'}`; intra-quiesce-before-start `503 {"ok":false,"reason":"shutting-down"}` | `1925`–`1939` (freeze); live ~`2347`–`2374` | `spawn.test.ts` (domain), `http-workflow-control.test.ts`, `ingress-supervisor.test.ts` (live join) |
| P13 | `/api/spawn/:id/revive` | loopback-open; bearer on LAN | **async** through the Effect bridge (`settleControlAsyncRoute` + `startOnce`); **join-on-interrupt**. Rollback: `effectRoutes=null` | `<out.status> <out.body>` | `.catch` → `500 {ok:false, reason:'internal'}`; intra-quiesce-before-start 503 | `1940`–`1956` (freeze); live ~`2376` | `revive.test.ts` (domain), `http-workflow-control.test.ts` |
| P14 | `/api/sessions/:sid/adopt` | loopback-open (unsupervised body derive-checked) | **async** through the Effect bridge (`settleControlAsyncRoute` + `startOnce`); **join-on-interrupt**. Rollback: `effectRoutes=null` | `<out.status> <out.body>` (derive: 404/400/409/410) | `.catch` → `500 {ok:false, reason:'internal'}`; intra-quiesce-before-start 503 | `1957`–`1997` (freeze); live ~`2403` | `adopt.test.ts` (domain), `http-workflow-control.test.ts` |
| P15 | `/api/sessions/:sid/name` | loopback-open; bearer on LAN | **async** as of the P6.4 control slice (dispatched through the Effect bridge via `settleEffectMutatingRoute` / `nameControl`); `Effect.sync`, interrupt-after-start is a no-op. **Sync mutating:** intra-quiesce 503, never replay. Rollback: `effectRoutes=null` | `out.ok ? 200 : 409` `<out>` | **`suffix` validated BEFORE unknown-id:** malformed/missing suffix → `400 {ok:false, reason:"suffix must be a string (or pass {clear:true})"}` (`2008`–`2013`); `validateNameSuffix` fail → `400 {ok:false, reason:bad}` (`2015`–`2021`); unknown-id (valid `{suffix:'x'}`) → `409` (`2023`–`2027`); intra-quiesce 503 | `1998`–`2029` (freeze); live ~`2456` | `rename.test.ts` (domain); unknown-id 409: `tests/p6-http-freeze.test.ts`; `http-workflow-control.test.ts` |
| P16 | `/api/sessions/:sid/dismiss` | loopback-open; bearer on LAN | **async** through the Effect bridge (`settleControlAsyncRoute` + `startOnce`); **join-on-interrupt**. Rollback: `effectRoutes=null` | `<out.status> <out.body>` (derive: 404/409) | `.catch` → `500 {ok:false, reason:'internal'}`; intra-quiesce-before-start 503 | `2030`–`2048` (freeze); live ~`2503` | `dismiss.test.ts` (domain), `http-workflow-control.test.ts` |
| P17 | `/api/sessions/:sid/dismiss/retry` | loopback-open; bearer on LAN | **async** through the Effect bridge (`settleControlAsyncRoute` + `startOnce`); **join-on-interrupt**. Rollback: `effectRoutes=null` | `<out.status> <out.body>` (BUG-145 idempotent) | `.catch` → `500 {ok:false, reason:'internal'}`; intra-quiesce-before-start 503 | `2052`–`2067` (freeze); live ~`2535` | `dismiss.test.ts` (domain), `http-workflow-control.test.ts` |
| P18 | `/api/spawn/:id/rc` | loopback-open; bearer on LAN | **async** through the Effect bridge (`settleControlAsyncRoute` + `startOnce`); **join-on-interrupt**. Rollback: `effectRoutes=null` | `<out.status> <out.body>` | `.catch` → `500 {ok:false, reason:'internal'}`; intra-quiesce-before-start 503 | `2068`–`2082` (freeze); live ~`2561` | `spawn.test.ts` / `needs-you.test.ts` (domain), `http-workflow-control.test.ts` |
| P19 | `/api/questions/:n/answer` | loopback-open; bearer on LAN | **async** as of the P6.4 control slice (`settleEffectMutatingRoute` / `controlSync`); `Effect.sync`, interrupt-after-start is a no-op. **Sync mutating:** intra-quiesce 503, never replay. Rollback: `effectRoutes=null` | `<out.status> <out.body>` | (derive-owned); intra-quiesce 503 | `2083`–`2095` (freeze); live ~`2586` | `choice-relay.test.ts`, `needs-you.test.ts` (domain), `http-workflow-control.test.ts` |
| P20 | `/api/questions/:n/dismiss` | loopback-open; bearer on LAN | **async** as of the P6.4 control slice (`settleEffectMutatingRoute` / `questionsDismiss`); `Effect.sync`, interrupt-after-start is a no-op. **Sync mutating:** intra-quiesce 503, never replay. Rollback: `effectRoutes=null` | `out.ok ? 200 : 404` `<out>` | intra-quiesce 503 | `2096`–`2103` (freeze); live ~`2615` | `question-rearm.test.ts`, `questions-audit.test.ts` (domain), `http-workflow-control.test.ts` |
| P21 | `/api/plans/:n/mark` | loopback-open; bearer on LAN | **async** as of the P6.4 control slice (`settleEffectMutatingRoute` / `controlSync`); `Effect.sync`, interrupt-after-start is a no-op. **Sync mutating:** intra-quiesce 503, never replay. Rollback: `effectRoutes=null` | `<out.status> <out.body>` (derive: 404/409) | (derive-owned); intra-quiesce 503 | `2104`–`2115` (freeze); live ~`2635` | `plans.test.ts`, `accept-plan-mark.test.ts` (domain), `http-workflow-control.test.ts` |
...
| `bodyCap` | `/api/paste-image` → `MAX_PASTE_BODY` (14e6); else `MAX_BODY` (1e6) | — | `2063` (P6.4 live; freeze was `1607`) |
...
**P6.4 decision (paste slice):** GET `/`, `/index.html`, `/assets/*` and the in-router `/favicon.ico` 204 stay the legacy `serveBoardAsset` path until P13. Path resolution + header policy are already pure in `http-policy.ts` (P6.2: `resolveBoardAssetPath`, `boardAssetHeaders`); the remaining work is `fs.readFileSync` + `res.end(data)`. Wrapping that I/O in an Effect workflow would add a fiber hop and a binary-response mapper for no behavioral gain, and would invent an intra-quiesce window the current sync path does not have (`fetchHandler`'s `quiescing` latch is the only gate today). P6.4's exit gate is route WORKFLOWS for application handlers; static assets are borderline and the freeze bytes (MIME/CSP/cache-control/nosniff/referrer-policy/404 `{err:'nope'}`) are best preserved by leaving the sync serve in place.
