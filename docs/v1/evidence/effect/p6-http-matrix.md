...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| G1 | `/health` | loopback-open; bearer on LAN | no | **async** as of the P6.4 pilot (dispatched through the Effect bridge, joined via `res.done`/`activeResponses` like the other async routes); legacy sync handler retained as the quiesce fallback + rollback seam | `200 {ok:true, fleet, pid, version, managed, spawn, auth:termAuth, startup}` | 401/403 walls | `1450`–`1478` | `security-headers.test.ts` (nosniff+json), `lan-auth.test.ts`, `loopback-gates.test.ts`, `http-workflow-health-state.test.ts` |
| G2 | `/state` | loopback-open; bearer on LAN | no (walled by Host allowlist) | **async** as of the P6.4 pilot (dispatched through the Effect bridge, joined via `res.done`/`activeResponses` like the other async routes); legacy sync handler retained as the quiesce fallback + rollback seam | `200 snapshotWithLan()` — **includes token-bearing `lan.urls`/`lan.mdns`** | 401/403 walls | `1479`–`1481`, `771` | `static-serving.test.ts` (regression), `lan-auth.test.ts`, `http-workflow-health-state.test.ts` |
...
| P8 | `/api/paste-image` | loopback-open; bearer on LAN | **async** as of the P6.4 paste slice (dispatched through the Effect bridge, joined via `res.done`/`activeResponses`); legacy sync handler retained as the rollback seam. MUTATING: intra-quiesce (ingress refused, transport still admitting) answers the frozen shutdown `503 {"ok":false,"reason":"shutting-down"}` and does NOT replay the write. Validation/limit/dir/write failures remain data responses (E=never). | `<out.status> <out.body>` — 201 `{ok:true, path, bytes}` (path ends `.png`/`.jpg`/`.gif`/`.webp` from the sniff); 400/413/500 `{ok:false, reason}` from paste.ts | body cap = **MAX_PASTE_BODY (14e6)** (§2); walls; intra-quiesce 503 | `1816`–`1826`, `1607` | `paste-image.test.ts`, `http-workflow-paste.test.ts` |
