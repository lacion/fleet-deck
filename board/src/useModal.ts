...
// The focus-restore/initial-focus paths guard `.focus` before calling it — not
// every DOM Element exposes it (activeElement is typed `Element`, and a
// querySelector hit is only structurally an Element). This local shape marks
// `focus` optional so those existence guards stay honest under strict typing,
// rather than being flagged as "always present" on a narrower element type.
interface Focusable {
...
interface ModalOptions {
  ...
  fallbackRef?: RefObject<(HTMLElement & Focusable) | null> | null;
}
...
export function useModal(
  ...
  { trap = true, initialFocus = true, fallbackRef = null }: ModalOptions = {},
): void {
  ...
  const [opener] = useState<(Element & Focusable) | null>(() =>
  ...
  useEffect(() => {
    ...
    if (initialFocus && node) {
      const first = node.querySelector<Element & Focusable>(FOCUSABLE);
      (first ?? node).focus?.();
    }
    ...
    return () => {
      ...
      if (opener?.focus && document.contains(opener)) opener.focus();
        ...
        if (fb?.focus && document.contains(fb)) fb.focus();
      }
    };
