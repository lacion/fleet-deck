...
import { clampWinRect, termChordHints, TERMWIN_MIN } from '../util.ts';
...
// The committed window geometry (App owns the rect + localStorage).
interface Rect {
  x: number;
  y: number;
  w: number;
...
// null | {kind:'exit'|'err'|'close', text} — TermPane's onNote payload.
interface TermNote {
  kind: 'exit' | 'err' | 'close';
  text: string;
}
...
export default function TermWindow({
  ...
  const gesture = (mode: 'drag' | 'resize') => (e: React.PointerEvent<HTMLDivElement>) => {
    if (maximized) return;
    ...
    if (
      (window as { matchMedia?: (query: string) => MediaQueryList }).matchMedia?.(COARSE_MQ).matches
    )
      return;
    if (e.button !== 0) return;
    if (mode === 'drag' && (e.target as Element).closest('button')) return; // header buttons are not a handle
    e.preventDefault();
    const start = { px: e.clientX, py: e.clientY, ...liveRect.current };
    const viewport = () => ({ w: window.innerWidth, h: window.innerHeight });
    const el = e.currentTarget;
    (el as { setPointerCapture?: (pointerId: number) => void }).setPointerCapture?.(e.pointerId);
  ...
  useEffect(() => {
    ...
    const onWin = () => {
      clearTimeout(t);
      t = setTimeout(() => {
        const next = clampWinRect(liveRect.current, {
          w: window.innerWidth,
          h: window.innerHeight,
        });
        if (
          next.x !== liveRect.current.x ||
          next.y !== liveRect.current.y ||
          next.w !== liveRect.current.w ||
          next.h !== liveRect.current.h
        ) {
          onRect(next);
        }
      }, 150);
    ...
    >
      <div className="fd-termhead fd-termgrab" onPointerDown={gesture('drag')}>
        ...
        <span className="fd-termhint">
          ⌨ Esc included · <b>⇧⏎</b> newline · <b>{CHORDS.select}</b> selects · <b>{CHORDS.copy}</b>{' '}
          copies
        </span>
        ...
        <button type="button" className="fd-x" aria-label="Close terminal" onClick={onClose}>
          ✕
        </button>
      </div>
      ...
      {/* m3 — the corner hides while the end-of-stream strip is up: it would
          ...
          nothing. */}
      {!maximized && !note && (
        <div className="fd-termresize" aria-hidden="true" onPointerDown={gesture('resize')}>
          ◢
        </div>
      )}
    </div>
