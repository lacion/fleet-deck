...
import { termChordHints } from '../util.ts';
...
const COLS = (n: number) => (n <= 1 ? 1 : n <= 4 ? 2 : n <= 9 ? 3 : 4);

// Same two unguessable chords the float's header names — see TermWindow.
const CHORDS = termChordHints();

// One live tile's identity (see hooks/useTermWindows termIdentity).
interface Tile {
  spawnId: string;
  callsign: string;
  window: string | null;
...
// A pane's ended-note, surfaced under its tile (TermPane's onNote payload).
interface TileNote {
  kind: 'exit' | 'err' | 'close';
  text: string;
}
...
export default function TermGrid({
  ...
  const [focused, setFocused] = useState<string | null>(tiles[0]?.spawnId ?? null);
  const [notes, setNotes] = useState<Record<string, TileNote | null>>({}); // spawnId -> {kind,text} | null
  ...
  const onCaptureKey = (e: React.KeyboardEvent<HTMLDivElement>) => {
  ...
  const removeTile = (t: Tile) => {
    ...
    setNotes((m) =>
      t.spawnId in m ? Object.fromEntries(Object.entries(m).filter(([k]) => k !== t.spawnId)) : m,
    );
    onRemoveTile(t);
  };
  ...
  return (
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label={`Live terminals — ${tiles.length} agents`}
        ref={dialogRef}
      >
        <div className="fd-gridhead">
          ...
          <span className="fd-termhint">
            ⌨ keystrokes go to the <b>focused</b> agent only — click a tile or press <b>Ctrl+←/→</b>{' '}
            to move the focus · <b>⇧⏎</b> newline · <b>{CHORDS.select}</b> selects ·{' '}
            <b>{CHORDS.copy}</b> copies
          </span>
          <span className="fd-spacer" />
          <button type="button" className="fd-x" aria-label="Close terminals" onClick={onClose}>
            ✕
          </button>
        </div>

        <div
          className="fd-gridbody"
              ...
              >
                ...
                <div className="fd-tilebody">
                  <TermPane
                    ...
                    live={live}
                    fontSize={11}
                    onNote={(n: TileNote | null) => {
                      setNotes((m) => (m[t.spawnId] === n ? m : { ...m, [t.spawnId]: n }));
                    }}
                  />
                  {note && (
