...
// legacy_upgrade rides even the tokenless /ws frame, and its interior is read
// defensively (`?.`) below — so this local view keeps those fields optional.
...
interface HeaderProps {
  ...
  stale: boolean;
  pendingQs: QuestionEntry[];
  liveN: number;
  conflictCount: number;
  version: string | undefined;
  now: number;
  legacyUpgrade: LegacyUpgradeLike | null | undefined;
  onCompose: () => void;
  termableSessions: SessionEntry[];
  watchable: SessionEntry[];
  termBtnRef: React.Ref<HTMLButtonElement>;
  onOpenGrid: (list?: SessionEntry[] | null) => void;
    ...
    <div className="fd-header">
      ...
      {pendingQs.length > 0 && (
        <button
          ...
          onClick={() => {
            getQuestion(pendingQs[0]?.id ?? '')?.scrollIntoView?.();
          }}
        >
      ...
      <div className="fd-fleetline">
        {liveN} session{liveN === 1 ? '' : 's'} · {conflictCount} conflict
        {conflictCount === 1 ? '' : 's'}
      </div>
      ...
      {version && (
        <div className="fd-fleetver" title="fleetd daemon version">
          fleetd v{version}
        </div>
      )}
      ...
      {spawnAvailable && (
        <button type="button" className="fd-hbtn" onClick={onSpawn}>
          ...
          {spawnActive > 0 && (
            ...
            <span className="fd-spawncount" title={`${spawnActive} board-spawned agents live`}>
              {spawnActive} live
            </span>
          )}
        </button>
      ...
      {wtSupported && (
        ...
        >
          ⑂ Worktrees
          {wtCount > 0 && <span className={`fd-wtbadge${wtHazard ? ' haz' : ''}`}>{wtCount}</span>}
        </button>
      )}
