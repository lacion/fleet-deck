...
function markErr(res: ApiResult, did: string | null) {
  const reason = res.reason; // raw daemon reason (null when it sent none)
  if (res.status === 409)
    return `${did ? did + ', but ' : ''}409 — ${(reason ?? '') || 'bad transition'}`;
  if (res.status === 404)
    return `${did ? did + ', but ' : ''}404 — ${(reason ?? '') || 'unknown plan'}`;
  return `${did ? did + ', but ' : ''}${reasonOf(res, `mark failed (${res.status})`)}`;
}

// Fields the assign response is read for, beyond the transport envelope.
...
interface PlanCardProps {
  ...
  onExecute: (p: PlanEntry) => void;
}

function PlanCard({ p, now, liveSessions, spawnAvailable, onExecute }: PlanCardProps) {
  ...
  return (
    <div className="fd-plan" data-planid={p.plan_id}>
      ...
      >
        ...
        {proposer && (
          <span className="fd-planlive">
            proposed by {(p.callsign ?? '') || proposer.callsign || proposer.session_id} — still
            live
          </span>
        )}
    ...
    <div className="fd-plans">
      ...
      {open && (
        <div className="fd-planlist">
          {plans.length === 0 && (
            <div className="none">
              no plans yet — spawn with permission-mode: plan, then Approve or Capture its exit-plan
              question in NEEDS YOU and it lands here
            </div>
          )}
          {plans.map((p) => (
            <PlanCard
