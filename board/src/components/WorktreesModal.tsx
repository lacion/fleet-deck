...
import { human, copyText } from '../util.ts';
import { useModal } from '../useModal.ts';
import type { Worktree } from '../hooks/useWorktrees.ts';
...
// The remove POST's options (useWorktrees.removeWt / api.removeWorktree).
interface RemoveOpts {
  force?: boolean;
  deleteBranch?: boolean;
}

// The remove POST's outcome, as the modal consumes it (useWorktrees.removeWt).
// Only { ok, reason } are read here; the success `json` is ignored.
type RemoveResult = { ok: true } | { ok: false; reason: string };

// The stored per-row failure the modal renders. useWorktrees.removeWt currently
// returns only { ok:false, reason }; blocked_paths/blocked_owner/fix_command are
// modelled OPTIONAL because WorktreeRow reads them defensively — the root-owned-
// paths remedy renders only if a future daemon/hook surfaces those fields.
interface RemoveErr {
  reason?: string;
...
const RANK: Record<string, number> = { 'has-work': 0, unknown: 1, safe: 2, gone: 3 };

const verdictOf = (w: Worktree | null | undefined): string =>
  w?.verdict != null && LABEL[w.verdict] ? w.verdict : 'unknown';
...
function ageOf(at: string | number | null | undefined, now: number): string | null {
...
function destroyLine(w: Worktree): string {
  if (verdictOf(w) === 'unknown') {
    return (
      'Git could not read this worktree — it may hold uncommitted changes and commits ' +
  ...
  if (unpushed > 0) {
    bits.push(
      `${plural(unpushed, 'commit')} that ${unpushed === 1 ? 'exists' : 'exist'} nowhere else`,
    );
  }
...
interface WorktreeRowProps {
  ...
  now: number;
  busy: boolean;
  err: RemoveErr | undefined;
  onRemove: (w: Worktree, opts: RemoveOpts) => Promise<void>;
}

function WorktreeRow({ w, now, busy, err, onRemove }: WorktreeRowProps) {
  ...
  const branchBox = (hazard: boolean) => (
    ...
    <div className={`fd-wtrow ${verdict}${busy ? ' busy' : ''}`}>
      <div className="r1">
        ...
        {alive ? (
          ...
          <span className="fd-wtspawn">{w.spawn_status}</span>
        ) : null}
      </div>

      <div className="path" title={w.path}>
        {w.path}
      </div>
      ...
      <div className="fd-wtev">
        ...
        {verdict === 'safe' && dirty === 0 && unpushed === 0 && (
          <span className="ev ok">
            ...
            {w.merged
              ...
              : w.upstream
                ? ` · everything is pushed to ${w.upstream}`
                : ''}{' '}
        ...
        {verdict === 'unknown' && (
          <span className="ev act">
            git could not read this worktree — treat it as if it holds work
          </span>
        )}
      ...
      {expand && files.length > 0 && (
        <div className="fd-wtfiles">
          {files.map((f) => (
            <span key={f} className="fd-filechip hot">
              {f}
            </span>
          ))}
        </div>
      ...
      {alive && (
        <div className="fd-wtblocked">
          ⚠ {(w.callsign ?? '') || 'That session'} is still running in here, so this cannot be
          removed — kill the session first (☠ on its card), then come back.
        </div>
      )}
      ...
      {confirm ? (
        <div className="fd-hazardconfirm">
          ...
          <div className="sub">
            The directory <span className="mono">{w.path}</span> is deleted from disk. Nothing in it
            can be recovered.
          </div>
          ...
          <div className="fd-wtfoot">
            ...
            <button
              ...
              disabled={busy || alive}
              onClick={() => {
                void onRemove(w, { force: true, deleteBranch: delBranch });
              }}
            >
        ...
        <div className="fd-wtacts">
          ...
          {verdict === 'safe' && (
            <>
              ...
              <button
                ...
                disabled={busy || alive}
                onClick={() => {
                  void onRemove(w, { force: false, deleteBranch: delBranch });
                }}
              >
          ...
          {verdict === 'gone' && (
            <>
              <span className="hint">
                purging clears the row and prunes git’s metadata — the branch is left alone
              </span>
              <button
                type="button"
                className="fd-ghostbtn"
                disabled={busy || alive}
                onClick={() => {
                  void onRemove(w, { force: false, deleteBranch: false });
                }}
              >
      ...
      {err && (
        <div className="fd-wterr" role="alert">
          ...
          {(err.blocked_paths?.length ?? 0) > 0 && (
            <>
              <ul className="fd-wtblocked">
                {err.blocked_paths?.map((p) => (
                  <li key={p}>
                    <code>{p}</code> <span className="own">owned by {err.blocked_owner}</span>
                  </li>
                ))}
              </ul>
...
interface WorktreesModalProps {
  ...
  onRemove: (path: string, opts: RemoveOpts) => Promise<RemoveResult>;
  onClose: () => void;
}

export default function WorktreesModal({
  worktrees,
  loading,
  error,
  now,
  ...
  const [busyPaths, setBusyPaths] = useState(() => new Set<string>());
  ...
  return (
    <div className="fd-composewrap" onClick={requestClose}>
      ...
      >
        ...
        <div className="sub">
          Every spawn with the worktree option gets its own checkout and its own branch. When the
          agent dies the worktree stays on disk — this is where you look at what is inside it and
          decide.
        </div>
        ...
        <div className="fd-wtlist">
          ...
          {worktrees !== null && n === 0 && (
            <div className="fd-wtempty">
              ...
              <div className="t2">
                Spawns with the worktree option create them here — each agent gets a checkout of its
                own and a branch of its own, and this is where you clean them up once it is done.
              </div>
            </div>
          )}
          {rows.map((w) => (
