...
export default function TokenGate({ attempts, onSubmit }: TokenGateProps) {
    ...
    <div className="fd fd-gate">
      <form className="fd-gatecard" onSubmit={submit}>
        ...
        <div className="hint">
          ...
          <span className="cmd">
            http://&lt;this-machine&gt;:4711/?t=<span className="c">&lt;key&gt;</span>
          </span>
          …or paste just the key here. Never saw that link? An auto-started daemon prints it into
          its log instead of a terminal — on the machine the daemon runs on:
          <span className="cmd">fleetdeck token</span>
          <span className="cmd">cat ~/.fleetdeck/token</span>
        </div>
        <div className="row">
          <input
            ref={inputRef}
          ...
          />
          <button type="submit" className="fd-send" disabled={!value.trim()}>
            Unlock
          </button>
        </div>
