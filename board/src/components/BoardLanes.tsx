...
// Contract types mark these fields required, but the board reads them
// defensively; local structural shapes keep each `?.`/`??`/`|| []` honest
// while the real snapshot rows flow in structurally.
interface RepoLike {
  repo_id: string | null;
...
interface BoardLanesProps {
  ...
  mailPending: Record<string, number>;
  mailMeta: Record<string, MailMeta>;
  compact: boolean;
  stale: boolean;
  legacyUpgrade: LegacyUpgradeLike | null;
  repoFilter: string;
  onRepoFilter: (id: string) => void;
  ripples: Map<string, number>;
  priorities: Set<string>;
  onOpenSession: (sessionId: string) => void;
  onOpenTerm: (s: SessionEntry) => void;
  reviving: Set<string> | undefined;
  revivingAll: boolean;
  onRevive: (s: SessionEntry) => void;
  onReviveAll?: (revivables: SessionEntry[]) => void;
  enablingRemote: Set<string> | undefined;
  onEnableRemote: (s: SessionEntry) => void;
  onKill: (s: SessionEntry) => void;
  onToggleWatch: (s: SessionEntry) => void;
  watch: Set<string> | undefined;
  onArmMove: (s: SessionEntry) => void;
  onDisarm: (s: SessionEntry) => void;
  adopting: Set<string> | undefined;
  onRename: (s: SessionEntry) => void;
  onDismiss: (s: SessionEntry) => void;
  dismissing: Set<string> | undefined;
  onSpawnShell: ((s: SessionEntry) => void) | null;
  shelling: Set<string> | undefined;
}
...
function BoardLanes({
  ...
  const { lanes, chips, colHeads, revivables, conflictFiles, conflictPeers } = useMemo(() => {
    const repoKey = (s: SessionEntry) => s.repo_id ?? '(none)';
    const byId = sessionsById(sessions);
    const csOf = (sid: string) => callsignOf(byId, sid);
    ...
    const cFiles = new Map<string, string[]>(); // sid -> [file]
    const cPeerSets = new Map<string, Set<string>>(); // sid -> Set<callsign>
    ...
    const cPeers = new Map<string, string[]>();
    ...
    const laneDefs = repos
      .map((r) => ({
        key: r.repo_id ?? '(none)',
        name: (r.repo_name ?? '') || (r.repo_id ? basename(r.repo_id) : 'no repo'),
      }))
    ...
    for (const s of visible) {
      if (!laneDefs.some((l) => l.key === repoKey(s))) {
        laneDefs.push({
          key: repoKey(s),
          name: (s.repo_name ?? '') || basename((s.repo_id ?? '') || (s.cwd ?? '')) || 'no repo',
        });
      }
    }
    ...
    const laneList = laneDefs.map((l) => {
      ...
      const laneConf = conflicts.filter((c) =>
        (c.sessions ?? []).some((sid) => ss.some((s) => s.session_id === sid)),
      );
      return {
        ...l,
        sessions: ss,
        ...
        conflictNote: laneConf.length
          ? [...new Set(laneConf.map((c) => basename(c.file || c.rel_path)))].join(', ') +
            ' contested'
          : null,
      };
    });
    ...
    const chipList = [
      ...
      ...repos
        ...
        .map((r) => ({
          id: r.repo_id ?? '(none)',
          label: (r.repo_name ?? '') || (r.repo_id ? basename(r.repo_id) : 'no repo'),
        })),
    ];
      ...
      <div className={`fd-lanes${stale ? ' stale' : ''}`}>
        <div className="fd-colheads">
          {colHeads.map((c) => (
            ...
            <div className="h" key={c.key} title={COL_HINTS[c.key as keyof typeof COL_HINTS]}>
                  ...
                  {lane.sessions
                    ...
                    .map((s) => (
                      <SessionCard
                        ...
                        spawningShell={!!shelling?.has(s.session_id)}
