...
import type { SessionEntry, MailMeta } from '../../../contracts/index.ts';
...
// `spawn.remote` predates remote-control (v1.6) and is absent on older spawn
// rows, so it is read through this optional view even though the wire contract
// marks it present — the defensive `?.` on it stays honest.
interface SpawnRemoteLike {
...
interface SessionCardProps {
  ...
  compact: boolean;
  mailCount: number;
  mailMeta: MailMeta | null | undefined;
  conflictFiles: string[];
  conflictPeers: string[];
  ripple: boolean;
  priority: boolean;
  onOpen: (s: SessionEntry) => void;
  onOpenTerm?: (s: SessionEntry) => void;
  onRevive?: (s: SessionEntry) => void;
  reviving: boolean;
  onEnableRemote?: (s: SessionEntry) => void;
  enablingRemote: boolean;
  onKill?: (s: SessionEntry) => void;
  onToggleWatch?: (s: SessionEntry) => void;
  watched?: boolean;
  onArmMove?: (s: SessionEntry) => void;
  onDisarm?: (s: SessionEntry) => void;
  adopting: boolean;
  onRename?: (s: SessionEntry) => void;
  onDismiss?: (s: SessionEntry) => void;
  dismissing: boolean;
  legacy: boolean;
  onSpawnShell?: ((s: SessionEntry) => void) | null;
  spawningShell: boolean;
}
...
function SessionCard({
  ...
  const files = ((s as { files?: string[] }).files ?? []).map(basename);
  ...
  const stalledTip = [
    ...
    s.spawn?.stall_detail
      ? `captured screen:\n${s.spawn.stall_detail}`
      : 'no pane excerpt was captured — open the live terminal',
  ]
    .filter(Boolean)
    .join('\n\n');
  ...
  const mailHint = mailMeta ? MAIL_HINT[mailMeta.route] : undefined;
  ...
  const remoteUrl = safeUrl((s.spawn as SpawnRemoteLike | undefined)?.remote?.url);
  ...
  const hasActs =
    canTerm || canKill || canWatch || canAdopt || canRename || canDismiss || canSpawnShell;
  ...
  return (
    ...
    <div role="group" aria-label={s.callsign || s.session_id} className={cls}>
      ...
      <span className="row1">
        ...
        {priority && (
          <span className="star" title="priority">
            ★
          </span>
        )}
        {s.spawn && (
        ...
        {legacy && (
          ...
          <span
            className="fd-legacytag"
            title="running pre-0.16.0 hooks — dark on this board until the session is restarted"
          >
            ⬆ restart me
          </span>
        )}
        {s.spawn?.gateway && (
          ...
          <span
            className="fd-gwchip"
            title="routed through the configured LLM gateway, not Anthropic"
          >
            🛰 gateway
          </span>
        )}
        {s.spawn?.setup_cmd && (
          <span className="fd-setupchip" title={s.spawn.setup_cmd}>
            ⚙ setup
          </span>
        )}
        {!shell &&
          (s.spawn as SpawnRemoteLike | undefined)?.remote?.enabled &&
        ...
        {!shell && stalled && (
          <span className="fd-stalledchip" title={stalledTip}>
            never registered
          </span>
        )}
        {!shell && s.stale && (
          <span className="fd-stalechip" title="no events for a while; may be stuck">
            stale
          </span>
        )}
        ...
        {shell ? (
          <span className="fd-shellchip" title="shell-only terminal — no Claude conversation">
            &gt;_ shell
          </span>
        ) : (
      ...
      <span style={{ display: 'flex', gap: 8, alignItems: 'baseline' }}>
        ...
        {compact && <Age className="age" from={(s.lastSeen ?? 0) || s.startedAt} />}
      </span>
      {failDetail && (
      ...
      {!compact && (
        <span className="row-files">
          ...
          {files.length > shown.length && (
            <span className="fd-filechip">+{files.length - shown.length}</span>
          )}
          <span className="fd-spacer" />
          {!shell && <Sparkline data={s.sparkline} />}
          <Age className="age" from={(s.lastSeen ?? 0) || s.startedAt} />
        </span>
      )}
    </div>
  );
