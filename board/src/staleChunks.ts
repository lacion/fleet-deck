...
// What installStaleChunkGuard needs from its host. Deliberately structural, and
// `sessionStorage` is OPTIONAL so the real `window` satisfies it while the
// `?.` guard below stays honest for a test double that omits storage.
interface StaleChunkWindow {
  addEventListener: (type: string, listener: (event: PreloadErrorEvent) => void) => void;
  removeEventListener: (type: string, listener: (event: PreloadErrorEvent) => void) => void;
...
// The `vite:preloadError` event only needs `preventDefault` here; both it and
// the parameter are optional so the `event?.preventDefault?.()` guard survives a
// test double that dispatches a bare event.
...
export function installStaleChunkGuard(
  win: StaleChunkWindow,
  { guardMs = RELOAD_GUARD_MS }: { guardMs?: number } = {},
): () => void {
  let reloading = false;

  function onPreloadError(event?: PreloadErrorEvent) {
