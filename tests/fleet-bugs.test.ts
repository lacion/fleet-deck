...
// --- typing seams -----------------------------------------------------------
// The DB handle, the core surface, and the tmux-adapter slot are derived from
// the real factories so the test tracks their signatures. spawn/revive/
// spawnKill are `(...args: unknown[]) => unknown` on the surface, so their
// results are cast to the {status, body} shapes the HTTP layer documents; the
// call is cast to a Promise before awaiting (the surface return is bare
// `unknown`, not a thenable type).
type Db = ReturnType<typeof openDb>;
type Core = ReturnType<typeof createCore>;
type CoreTmuxAdapter = NonNullable<NonNullable<Parameters<typeof createCore>[1]>['tmuxAdapter']>;
type SnapshotSession = ReturnType<Core['snapshot']>['sessions'][number];

interface SpawnResult {
  status: number;
  body: { session_id: string; spawn_id: string; tmux: { window: string } };
}
...
interface ReviveResult {
  status: number;
  body: { ok: boolean; adopted: boolean; reason: string; current_spawn_id: string };
}
...
// Row shapes for the .get()/.all() reads. Under noUncheckedIndexedAccess and
// the sqlite seam, prepare<R>().get() is `R | undefined`.
interface StatusRow {
  status: string;
}
interface CountRow {
...
interface FakeTmuxAdapter {
  ...
  hasTmux: () => boolean;
  sessionName: (p: number | string) => string;
  windowName: (p: number | string, callsign: string) => string;
  ensureSession: (p: number | string) => Promise<string>;
  newWindow: (
    spec: NewWindowLike,
  ) => Promise<{ session: string; window: string; window_id: string }>;
  listScopedWindows: (port?: number | string) => Promise<FakeWindow[]>;
  paneCurrentCommand: (target: string) => Promise<PaneCmd | null>;
  pasteText: () => Promise<boolean>;
  sendEnter: () => Promise<boolean>;
  sendBringupEnter: () => Promise<boolean>;
  killWindowVerified: (name: string) => Promise<KillVerifiedResult>;
  launchOverride: () => void;
}
interface FakeTmux {
  state: FakeTmuxState;
...
function fakeTmux(port = 4711): FakeTmux {
  const state: FakeTmuxState = { windows: [], argv: null, killed: [] };
  const adapter: FakeTmuxAdapter = {
    ...
    launchOverride: () => {
      /* no-op: nothing in these tests exercises the override launch path */
    },
  };
  return { state, adapter, port };
}

interface MemoryCoreOpts {
...
// The card must exist for every assertion the callers make; the narrowing
// throw stands in for the TypeError the untyped `.find(...).col` used to raise
// when a card was missing — same failure, earlier and named.
function cardOf(core: Core, sid: string): SnapshotSession {
  const card = core.snapshot().sessions.find((s) => s.session_id === sid);
  assert.ok(card, `sanity: card ${sid} exists in the snapshot`);
  return card;
}
...
test('BUG 1: SessionEnd(reason="clear") keeps the pane and the card LIVE', async (t) => {
  ...
  const spawn = await (core.spawn({ cwd }) as Promise<SpawnResult>);
  ...
  assert.equal(
    card.spawn?.status,
    'live',
    'a /clear must NOT condemn the pane to pane-dead — the terminal stays',
  );
  assert.equal(
    db.prepare<StatusRow>('SELECT status FROM spawns WHERE session_id = ?').get(sid)?.status,
    'live',
  );
});
...
test('BUG 1 control: a real SessionEnd (reason="other") still ends the session and pane', async (t) => {
  ...
  assert.equal(
    card.spawn?.status,
    'pane-dead',
    'a real end marks the pane dead (window kept for scrollback)',
  );
});
...
test('BUG 2: a silent-but-pane-alive spawn is NOT presumed dead/goned; a pane-dead one IS', async (t) => {
  ...
  const seedSession = (sid: string, callsign: string): void => {
    db.prepare(
      `INSERT INTO sessions
    (session_id, callsign, col, note, events, started_at, last_seen, source)
    VALUES (?, ?, 'idle', 'waiting at its prompt', 0, ?, ?, 'hooks')`,
    ).run(sid, callsign, silent, silent);
  };
  const seedSpawn = (spawnId: string, sid: string, callsign: string, window: string): void => {
    db.prepare(
      `INSERT INTO spawns
    (spawn_id, session_id, callsign, tmux_session, tmux_window, requested_at, status)
    VALUES (?, ?, ?, 'fleetdeck-4711', ?, ?, 'live')`,
    ).run(spawnId, sid, callsign, window, silent);
  };
  ...
  const alive = db
    .prepare<SessionSelectRow>("SELECT * FROM sessions WHERE session_id = 'alive'")
    .get();
  assert.ok(alive, 'sanity: the alive session row exists');
  ...
  assert.equal(
    db.prepare<StatusRow>("SELECT status FROM spawns WHERE spawn_id = 'sp-alive'").get()?.status,
    'live',
  ...
  const dead = db
    .prepare<SessionSelectRow>("SELECT * FROM sessions WHERE session_id = 'dead'")
    .get();
  assert.ok(dead, 'sanity: the dead session row exists');
  assert.equal(dead.col, 'offline', 'a tmux-confirmed dead pane IS condemned');
  assert.ok(dead.ended_at);
  assert.equal(
    db.prepare<StatusRow>("SELECT status FROM spawns WHERE spawn_id = 'sp-dead'").get()?.status,
    'pane-dead',
  );
});
...
test('BUG 2: a pane-less hook-only session keeps the silence-based presume-dead behavior', async (t) => {
  ...
  db.prepare(
    `INSERT INTO sessions
    (session_id, callsign, col, note, events, started_at, last_seen, source)
    VALUES ('hookonly', 'hook-1', 'idle', 'waiting', 0, ?, ?, 'hooks')`,
  ).run(now - 4 * HOUR, now - 4 * HOUR);

  await core.retentionSweep(now);

  const s = db
    .prepare<SessionSelectRow>("SELECT * FROM sessions WHERE session_id = 'hookonly'")
    .get();
  assert.ok(s, 'sanity: the hook-only session row exists');
...
test('BUG-045: a revive landing mid-sweep stops the stale probe from condemning the revived card', async (t) => {
  ...
  db.prepare(
    `INSERT INTO sessions
    (session_id, callsign, col, note, events, started_at, last_seen, source)
    VALUES ('rev', 'rev-1', 'idle', 'waiting at its prompt', 0, ?, ?, 'hooks')`,
  ).run(silent, silent);
  db.prepare(
    `INSERT INTO spawns
    (spawn_id, session_id, callsign, tmux_session, tmux_window, requested_at, status)
    VALUES ('sp-old', 'rev', 'rev-1', 'fleetdeck-4711', 'fd4711-rev-1', ?, 'live')`,
  ).run(silent);
  ...
  tmux.adapter.listScopedWindows = async (port) => {
    ...
    db.prepare(
      `INSERT INTO spawns
      (spawn_id, session_id, callsign, tmux_session, tmux_window, requested_at, status)
      VALUES ('sp-new', 'rev', 'rev-1', 'fleetdeck-4711', 'fd4711-rev-1', ?, 'live')`,
    ).run(now);
    return wins;
  };
  ...
  assert.equal(
    db.prepare<StatusRow>("SELECT status FROM spawns WHERE spawn_id = 'sp-new'").get()?.status,
    'live',
    'the revived spawn must NOT be flipped pane-dead by the stale probe',
  );
  const card = db
    .prepare<SessionSelectRow>("SELECT * FROM sessions WHERE session_id = 'rev'")
    .get();
  assert.ok(card, 'sanity: the revived session row exists');
...
test('BUG-045 control: with no mid-sweep revive, a tmux-confirmed dead pane is still condemned', async (t) => {
  ...
  db.prepare(
    `INSERT INTO sessions
    (session_id, callsign, col, note, events, started_at, last_seen, source)
    VALUES ('stilldead', 'sd-1', 'idle', 'waiting at its prompt', 0, ?, ?, 'hooks')`,
  ).run(silent, silent);
  db.prepare(
    `INSERT INTO spawns
    (spawn_id, session_id, callsign, tmux_session, tmux_window, requested_at, status)
    VALUES ('sp-sd', 'stilldead', 'sd-1', 'fleetdeck-4711', 'fd4711-sd-1', ?, 'live')`,
  ).run(silent);
  state.windows.push({
    session: 'fleetdeck-4711',
    window: 'fd4711-sd-1',
  ...
  assert.equal(
    db.prepare<StatusRow>("SELECT status FROM spawns WHERE spawn_id = 'sp-sd'").get()?.status,
    'pane-dead',
    'the revalidation must not weaken the plain dead-pane verdict',
  );
  const card = db
    .prepare<SessionSelectRow>("SELECT * FROM sessions WHERE session_id = 'stilldead'")
    .get();
  assert.ok(card, 'sanity: the still-dead session row exists');
...
async function condemnedSpawnWithLivePane(t: TestContext, status: string) {
  const tmux = fakeTmux();
  ...
  db.prepare(
    "UPDATE sessions SET col = 'offline', note = 'pane died', ended_at = ?, archived_at = ? WHERE session_id = ?",
...
for (const status of ['pane-dead', 'gone']) {
  test(`BUG 3: the liveness tick RESURRECTS a '${status}' spawn whose pane is a live claude`, async (t) => {
    ...
    assert.equal(
      card.spawn?.status,
      'live',
      'the terminal is shown again (spawn live in the snapshot)',
    );
    assert.equal(
...
test('BUG 3: a genuinely dead pane-dead spawn is NOT resurrected', async (t) => {
  ...
  const win0 = state.windows[0];
  assert.ok(win0, 'sanity: the spawn seeded a window');
  win0.pane_dead = true; // the pane really is dead this time
...
test('BUG 3: a bare-shell remnant is NOT resurrected', async (t) => {
  ...
  const win0 = state.windows[0];
  assert.ok(win0, 'sanity: the spawn seeded a window');
  win0.pane_cmd = 'zsh'; // claude exited, remain-on-exit login shell remains
...
test('BUG-152: a kill landing DURING the resurrection probe is never undone by the stale verdict', async (t) => {
  ...
  db.prepare(
    "UPDATE sessions SET col = 'offline', note = 'pane died', ended_at = ? WHERE session_id = ?",
  ).run(Date.now(), sid);
  ...
  tmux.adapter.paneCurrentCommand = async (target) => {
    ...
    if (probeCalls === 1) {
      ...
      const res = await (core.spawnKill(spawnId, true) as Promise<SpawnKillResult>);
...
test('BUG 3: revive() ADOPTS a gone spawn whose window is a live claude (no 409, no duplicate)', async (t) => {
  ...
  const out = await (core.revive(spawnId) as Promise<ReviveResult>);

  assert.equal(
    out.status,
    200,
    'a human clicking Revive on the very-alive pane must not be stuck on a 409',
  );
  ...
  assert.notEqual(
    card.col,
    'offline',
    'the terminal is shown again immediately, not one poll later',
  );
});
...
test('BUG 4: oversize mail is bounded and postMail reports truncated + original_length', async (t) => {
  ...
  const out = (await core.postMail({
    to: sid,
    from: 'ops',
    text: big,
  })) as unknown as PostMailResult;
  ...
  assert.equal(
    out.truncated,
    true,
    'the sender is TOLD the tail was cut — never a silent {ok:true}',
  );
  assert.equal(out.original_length, 9000);
  assert.ok(
    out.max_length >= 1000 && out.max_length < 9000,
    'the cap is a sane, bounded few-KB value',
  );
  const stored = db
    .prepare<TextRow>('SELECT text FROM mail WHERE to_session = ? ORDER BY id DESC LIMIT 1')
    .get(sid)?.text;
  assert.ok(stored != null, 'sanity: the mail row was stored');
  ...
  assert.ok(
    stored.length > 500,
    'the cap is far larger than the old 500-char limit that truncated real messages',
  );
});
...
test('BUG-021: an oversize broadcast is rejected atomically — no mail rows, no silent ok', async (t) => {
  ...
  const out = core.command(`broadcast ${'x'.repeat(5000)}`) as unknown as CommandResult;

  assert.equal(
    out.ok,
    false,
    'the operator is TOLD the message is too long — never a silent {ok:true}',
  );
  assert.match(out.reason, /too long/);
  assert.equal(out.original_length, 5000);
  assert.equal(out.max_length, 4000);
  assert.equal(
    db.prepare<CountRow>('SELECT COUNT(*) AS n FROM mail').get()?.n,
    0,
...
test('BUG-021: an oversize direct assign is rejected atomically', async (t) => {
  ...
  const callsign = db
    ...
    .get(sid)?.callsign;
  assert.ok(callsign, 'sanity: the session has a callsign');

  const out = core.command(`assign ${callsign} ${'y'.repeat(5000)}`) as unknown as CommandResult;

  assert.equal(out.ok, false);
  assert.match(out.reason, /too long/);
  assert.equal(
    db.prepare<CountRow>('SELECT COUNT(*) AS n FROM mail WHERE to_session = ?').get(sid)?.n,
    0,
...
test('BUG-021: assign auto is framed BEFORE the cap check — a body that fits unframed but overflows once framed is rejected', async (t) => {
  ...
  const out = core.command(`assign auto ${'z'.repeat(3990)}`) as unknown as CommandResult;

  assert.equal(
    out.ok,
    false,
    'the frame itself counts against the cap — it rides inside the paste',
  );
  assert.equal(out.original_length, 23 + 3990);
  assert.equal(db.prepare<CountRow>('SELECT COUNT(*) AS n FROM mail').get()?.n, 0);
});
...
test('BUG-021: a normal-size assign still delivers whole (no rejection fields)', async (t) => {
  ...
  const out = core.command(`assign ${callsign} ${msg}`) as unknown as CommandResult;
...
async function reviveScenario(t: TestContext, { status = 'gone', paneDead = false } = {}) {
  ...
  const spawn = await (ctx.core.spawn({ cwd }) as Promise<SpawnResult>);
  const sid = spawn.body.session_id;
  ...
  writeFileSync(transcript, '{}\n');
  if (paneDead) {
    const win0 = ctx.state.windows[0];
    assert.ok(win0, 'sanity: the spawn seeded a window');
    win0.pane_dead = true;
  }
...
test('Fix 1 [HIGH]: two concurrent revives for one session launch exactly ONE pane', async (t) => {
  ...
  const { core, db, state, sid, oldId } = await reviveScenario(t, {
    status: 'gone',
    paneDead: true,
  });
  ...
  const [r1, r2] = await Promise.all([
    core.revive(oldId) as Promise<ReviveResult>,
    core.revive(oldId) as Promise<ReviveResult>,
  ]);
  ...
  assert.ok(refused, 'sanity: one revive was refused');
  assert.match(
    refused.body.reason,
  ...
  const liveEligible = db
    .prepare<CountRow>(
      "SELECT COUNT(*) AS n FROM spawns WHERE session_id = ? AND status IN ('provisioning','spawning','stalled','live')",
    )
    .get(sid)?.n;
  assert.equal(
    liveEligible,
    1,
    'exactly one new live-eligible row — never two panes for one session',
  );
  ...
  const freshId = db
    .prepare<SpawnIdRow>("SELECT spawn_id FROM spawns WHERE session_id = ? AND status = 'spawning'")
    .get(sid)?.spawn_id;
  assert.ok(freshId, 'sanity: the successful revive left a spawning row');
  db.prepare("UPDATE spawns SET status = 'gone' WHERE spawn_id = ?").run(freshId);
  db.prepare("UPDATE sessions SET col = 'offline', ended_at = ? WHERE session_id = ?").run(
    Date.now(),
    sid,
  );
  const again = await (core.revive(freshId) as Promise<ReviveResult>);
  assert.notEqual(
    again.status,
    409,
    'the single-flight claim was released; a later solo revive is not blocked',
  );
});
...
test('Fix 2 [MED]: reviving a NON-newest id whose window is a live claude refuses (no double-live, no un-killable row)', async (t) => {
  const { core, db, sid, oldId, window } = await reviveScenario(t, {
    status: 'pane-dead',
    paneDead: false,
  });
  ...
  const orig = db.prepare<SpawnSelectRow>('SELECT * FROM spawns WHERE spawn_id = ?').get(oldId);
  assert.ok(orig, 'sanity: the original spawn row exists');
  ...
  db.prepare(
    ...
    orig.cwd,
    orig.worktree_path,
    orig.requested_at + 1000,
  );

  const out = await (core.revive(oldId) as Promise<ReviveResult>);

  assert.equal(out.status, 409, 'the older row does not own the window — adoption is refused');
  assert.equal(
    out.body.current_spawn_id,
    newerId,
    'the caller is told which row actually owns the window',
  );
  assert.equal(
  ...
  assert.equal(
    db.prepare<StatusRow>('SELECT status FROM spawns WHERE spawn_id = ?').get(newerId)?.status,
    'pane-dead',
    'the newer row is untouched',
  );
  assert.equal(
    db.prepare<CountRow>("SELECT COUNT(*) AS n FROM spawns WHERE status = 'live'").get()?.n,
    0,
...
test('Fix 3 [MED]: a single transient dead read does NOT condemn-then-flap a steadily-live claude', async (t) => {
  ...
  const win0 = state.windows[0];
  assert.ok(win0, 'sanity: the spawn seeded a window');
  win0.pane_dead = true; // ONE transient dead read (a momentary tmux glitch)
  await core.spawnLivenessTick();
  assert.equal(
    statusOf(),
    'live',
    'a single dead read must not condemn a live spawn (hysteresis arms, does not fire)',
  );
  assert.notEqual(
    cardOf(core, sid).col,
    'offline',
    'the card is never tombstoned by a transient read',
  );

  win0.pane_dead = false; // recovered
  await core.spawnLivenessTick();
  assert.equal(statusOf(), 'live', 'the streak resets on the live read — no live↔pane-dead flap');
  assert.equal(
    core.snapshot().ticker.filter((x) => x.msg != null && /pane died|restored/.test(x.msg)).length,
    0,
...
test('Fix 3 [MED]: a genuinely dead pane is still condemned — on the second consecutive dead read', async (t) => {
  ...
  assert.equal(
    statusOf(),
    'pane-dead',
    'a persistently dead pane IS condemned on the second consecutive read',
  );
...
test('Fix 3 [MED]: condemn and resurrect share ONE probe — a split pane (lowest=shell, active=claude) never condemns', async (t) => {
  ...
  tmux.adapter.paneCurrentCommand = () => Promise.resolve({ dead: false, cmd: 'claude' });
  ...
  const win0 = state.windows[0];
  assert.ok(win0, 'sanity: the spawn seeded a window');
  win0.pane_cmd = 'zsh'; // scoped lowest pane reads as a bare shell
...
test('Fix 3 [MED]: paneCurrentCommand UNKNOWN never advances death hysteresis', async (t) => {
  const tmux = fakeTmux();
  tmux.adapter.paneCurrentCommand = () => Promise.resolve(null);
  const { core, db, state } = memoryCore(t, { tmux });
  ...
  const win0 = state.windows[0];
  assert.ok(win0, 'sanity: the spawn seeded a window');
  win0.pane_cmd = 'zsh'; // force the active-pane probe
  const statusOf = (): string | undefined =>
    db.prepare<StatusRow>('SELECT status FROM spawns WHERE spawn_id = ?').get(spawn.body.spawn_id)
      ?.status;
  ...
  tmux.adapter.paneCurrentCommand = () => Promise.resolve({ dead: true, cmd: 'claude' });
  await core.spawnLivenessTick();
...
test('Fix 4 [LOW]: resurrection clears a stale needs-you reason and refreshes last_seen', async (t) => {
  ...
  db.prepare("UPDATE spawns SET status = 'pane-dead' WHERE spawn_id = ?").run(spawnId);
  db.prepare(
    "UPDATE sessions SET col = 'offline', ended_at = ?, notification_type = 'spawn_stalled', last_seen = ? WHERE session_id = ?",
  ).run(stale, stale, sid);

  await core.spawnLivenessTick();

  const s = db.prepare<SessionSelectRow>('SELECT * FROM sessions WHERE session_id = ?').get(sid);
  assert.ok(s, 'sanity: the session row exists');
  assert.equal(
    db.prepare<StatusRow>('SELECT status FROM spawns WHERE spawn_id = ?').get(spawnId)?.status,
    'live',
  );
  assert.equal(
    s.notification_type,
    null,
    'a resurrected card sheds the stale needs-you reason chip',
  );
  assert.ok(
    s.last_seen > stale,
    'last_seen is refreshed so the very next retention sweep cannot re-presume it dead',
  );
...
test('Fix 6 [LOW]: mail truncation never splits a surrogate pair (no lone high surrogate stored)', async (t) => {
  ...
  assert.equal(
    out.original_length,
    raw.length,
    'original_length stays code-unit based (unchanged semantics)',
  );
  assert.equal(out.max_length, 4000);
  const stored = db
    .prepare<TextRow>('SELECT text FROM mail WHERE to_session = ? ORDER BY id DESC LIMIT 1')
  ...
  assert.ok(
    (stored as string & { isWellFormed: () => boolean }).isWellFormed(),
    'the stored body is a well-formed string — no orphaned high surrogate',
  );
  assert.equal(
    stored.length,
    3999,
    'the split emoji is dropped whole rather than left half-encoded',
  );
...
test('Fix 7 [LOW]: a silent override-process spawn (no tmux window) is presumed dead, not lingered active', async (t) => {
  ...
  db.prepare(
    `INSERT INTO sessions
    (session_id, callsign, col, note, events, started_at, last_seen, source)
    VALUES ('ov', 'ov-1', 'idle', 'waiting at its prompt', 0, ?, ?, 'hooks')`,
  ).run(silent, silent);
  db.prepare(
    `INSERT INTO spawns
    (spawn_id, session_id, callsign, tmux_session, tmux_window, requested_at, status)
    VALUES ('sp-ov', 'ov', 'ov-1', 'fleetdeck-4711', 'fd4711-ov-1', ?, 'live')`,
  ).run(silent);
  // Deliberately push NO window into tmux — an override process has none.

  await core.retentionSweep(now);

  const s = db.prepare<SessionSelectRow>("SELECT * FROM sessions WHERE session_id = 'ov'").get();
  assert.ok(s, 'sanity: the override session row exists');
  assert.equal(
    s.col,
    'offline',
    'silence is the only signal a pane-less override process exposes → presume it dead',
  );
  assert.ok(s.ended_at);
  assert.match(s.note ?? '', /presumed ended \(silent .+h\)/);
  assert.equal(
    db.prepare<StatusRow>("SELECT status FROM spawns WHERE spawn_id = 'sp-ov'").get()?.status,
    'pane-dead',
...
test('BUG-025: a delayed SessionEnd from a previous run must NOT tombstone a live resumed session', async (t) => {
  ...
  assert.notEqual(
    card.col,
    'offline',
    'a stale async end must NOT tombstone the live resumed card',
  );
  ...
  assert.equal(
    card.spawn?.status,
    'live',
    'a stale async end must NOT condemn the pane to pane-dead',
  );
  assert.equal(
    db.prepare<StatusRow>('SELECT status FROM spawns WHERE session_id = ?').get(sid)?.status,
    'live',
  ...
  assert.equal(
    db.prepare<StatusRow>('SELECT status FROM questions WHERE id = ?').get(hold.id)?.status,
    'pending',
  ...
  assert.equal(
    cardOf(core, sid).col,
    'offline',
    "the active run's own SessionEnd still tombstones",
  );
  assert.ok(cardOf(core, sid).endedAt);
});

test('BUG-025: an untagged SessionEnd keeps the historical unconditional tombstone', (t) => {
  const { core } = memoryCore(t);
...
test('BB2-1: a mismatched-nonce SessionEnd still tombstones when the spawn is proven terminal (no live run to protect)', async (t) => {
  ...
  const dismissed = (await core.dismissSession(sid)) as unknown as DismissResult;
  assert.equal(
    dismissed.status,
    200,
...
test('BUG-025: a tagged SessionEnd against a row with no recorded run tombstones (reconciled with max-turns abort)', (t) => {
  const { core } = memoryCore(t);
