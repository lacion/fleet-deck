// tests/git-stderr-detail.test.ts — the "git's remedy reaches the board, git's
...
test('the note is unchanged: still only the distilled verdict', () => {
  ...
  assert.doesNotMatch(
    note,
    /settings\/ssh\/new/,
    'the note stays a one-liner; the detail carries the rest',
  );
});
...
test('URL userinfo never reaches the detail — the credential control', () => {
  const cases = [
    ...
    // the positional scrub is the only thing standing between it and the board.
    {
      ...
      leak: 'glpat-AbCdEf1234567890',
      keep: '[redacted]@gitlab.com',
    },
...
test('an exact secret with no known shape is scrubbed via the caller-supplied list', () => {
  ...
  const detail = gitStderrDetail(`remote: HTTP Basic: Access denied for user '${secret}'`, {
    secrets: [secret],
  });
  assert.ok(detail !== null);
...
test('the excerpt is bounded to 20 LINES, independently of the byte cap', () => {
  ...
  assert.equal(
    detail.split('\n').length,
    20,
    'exactly GIT_DETAIL_LINES lines, not merely "at most"',
  );
...
test('an over-long URL userinfo FAILS CLOSED instead of passing through', () => {
  ...
  for (const out of [
    scrubUrlCredentials(line),
    gitStderrDetail(line),
    distillGitStderr(scrubUrlCredentials(line)),
  ]) {
    assert.ok(out !== null);
...
test('a credential in the URL QUERY STRING or FRAGMENT is redacted too', () => {
  ...
  const cases: [string, string][] = [
    [
      "fatal: unable to access 'https://gitea.example/o/r.git?access_token=deadbeefcafebabe0123': 401",
      'deadbeefcafebabe0123',
    ],
    [
      "fatal: unable to access 'https://gitlab.example/o/r.git?private_token=abcd1234abcd1234': 401",
      'abcd1234abcd1234',
    ],
    [
      "fatal: repository 'https://host/r.git#auth_key=zzzzyyyyxxxxwwww' not found",
      'zzzzyyyyxxxxwwww',
    ],
  ];
  ...
  assert.equal(
    scrubUrlCredentials('see https://host/r.git?ref=main now'),
    'see https://host/r.git?ref=main now',
  );
});

test('a SCHEMELESS user:token@host pair is redacted, scp-style is not', () => {
  const out = scrubUrlCredentials(
    "fatal: could not read from 'user:tok3nvalue1234@host.example/x.git'",
  );
  ...
  assert.equal(
    scrubUrlCredentials('git@github.com: Permission denied (publickey).'),
    'git@github.com: Permission denied (publickey).',
  );
});
...
test('WHITESPACE inside the password half does not defeat the scrub', () => {
  ...
  const out = scrubUrlCredentials(
    "fatal: unable to access 'https://user:my pass phrase@host.example/x.git/'",
  );
...
test('a PRIVATE KEY BLOCK longer than the line cap is masked, not tail-sliced', () => {
  ...
  const body = Array.from(
    { length: 32 },
    (_, i) => `AAAAB3NzaC1yc2EAAAADAQABAAABgQSECRETKEYLINE${i}`,
  );
  const detail = gitStderrDetail(
    [
...
test('a 4-byte astral tail is cut on a character boundary, not inside a surrogate pair', () => {
  ...
  assert.equal(
    ...
    detail,
    'the excerpt must be well-formed UTF-16',
  );
  assert.equal(
    /[\uD800-\uDFFF]/.test(detail.replace(/[\uD800-\uDBFF][\uDC00-\uDFFF]/g, '')),
    false,
...
test('the NOTE and the DETAIL are hardened by the same pass — no weaker sink', () => {
  ...
  const cases: { stderr: string; leak: string; secrets: string[] }[] = [
    { stderr: `fatal: helper rejected token ghp_${'A'.repeat(30)}`, leak: 'ghp_A', secrets: [] },
    {
      stderr: "fatal: Authentication failed for user 'hunter2secretpw'",
      leak: 'hunter2secretpw',
      secrets: ['hunter2secretpw'],
    },
...
test('the composed credential scrub stays effectively linear on adversarial input', () => {
  // Five layers now, each with bounded runs; the ReDoS audit in payload-capture.ts
...
test('a legacy spawns table gains fail_detail additively, backfilled NULL', (t: TestContext) => {
  ...
  assert.equal(
    db.prepare<FailDetailRow>("SELECT fail_detail FROM spawns WHERE spawn_id = 'legacy'").get()
      ?.fail_detail,
    null,
  ...
  const twice = db
    .prepare<PragmaColumnInfo>('PRAGMA table_info(spawns)')
    .all()
    .filter((row) => row.name === 'fail_detail');
...
test('spawnFailureReason: one bounded line, hardened by the same pass', async () => {
  ...
  assert.equal(
    spawnFailureReason(new Error('SQLITE_BUSY: database is locked')),
    'SQLITE_BUSY: database is locked',
  );
  assert.equal(
    spawnFailureReason(new Error('')),
    'internal error',
  ...
  const oneLine = spawnFailureReason(
    new Error("Cloning into 'x'...\nfatal: Could not read from remote repository."),
  );
  ...
  const urlErr = spawnFailureReason(
    new Error("fatal: unable to access 'https://luis:glpat-AbCdEf1234567890@gitlab.com/x/y.git/'"),
  );
  assert.equal(
    urlErr.includes('glpat-AbCdEf1234567890'),
    false,
    'a PAT in a thrown message must not survive into the reason',
  );
  assert.ok(urlErr.includes('[redacted]@gitlab.com'));
  const bare = spawnFailureReason(
    new Error('fatal: helper rejected token ghp_abcdefghij1234567890'),
  );
  assert.equal(
    bare.includes('ghp_abcdefghij1234567890'),
    false,
