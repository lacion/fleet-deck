// tests/accept-plan-mark.test.mjs
//
...
function gate8Block() {
  ...
  const start = src.indexOf('# --------------------------------------------------------------- gate 8');
  const end = src.indexOf('# --------------------------------------------------------------- gate 9');
...
test('gate 8 does not mark the plan executed when execution was never proven (BUG-015)', (t) => {
  const noExecution = runGate8(t, { PLAN_ID: '42', EXECUTED: '', NO_PERMISSION_CARD: 'yes' });
  assert.equal(noExecution.marks, '', 'no mark request is issued when the implementation check failed');
  assert.match(noExecution.output, /FAIL: plan marked executed/, 'the gate reports FAIL instead of a false PASS');

  const permissionCard = runGate8(t, { PLAN_ID: '42', EXECUTED: 'yes', NO_PERMISSION_CARD: '' });
  assert.equal(permissionCard.marks, '', 'no mark request is issued when a permission card was observed');
  assert.match(permissionCard.output, /FAIL: plan marked executed/, 'the gate reports FAIL instead of a false PASS');
});

test('gate 8 marks the plan executed only after gate 7 proved execution', (t) => {
  const proven = runGate8(t, { PLAN_ID: '42', EXECUTED: 'yes', NO_PERMISSION_CARD: 'yes' });
  assert.equal(proven.marks, 'http://127.0.0.1:1/api/plans/42/mark', 'the mark request targets the captured plan');
