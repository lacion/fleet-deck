...
// through these local interfaces (the daemon internals are never imported).
// ---------------------------------------------------------------------------
...
interface SpawnOkBody {
  ok: boolean;
  spawn_id: string;
}

interface ExitPlanToolInput {
  plan: string;
  command?: string;
}

interface ExitPlanOverrides {
 ...
 * neither). */
function questionPlanId(q: QuestionEntry): {
  plan_id: number | string | null | undefined;
  where: string | null;
} {
  if (q.plan_id !== undefined && q.plan_id !== null)
    return { plan_id: q.plan_id, where: 'top-level' };
  const payloadPlanId = q.payload?.plan_id;
  if (payloadPlanId !== undefined && payloadPlanId !== null)
    return { plan_id: payloadPlanId, where: 'payload' };
  return { plan_id: undefined, where: null };
}

interface HeldPlan {
  held: Promise<JsonResponse>;
  state: StateSnapshot;
  q: QuestionEntry;
...
async function holdExitPlan(
  daemon: DaemonHandle,
  sid: string,
  cwd: string,
  holdMs: number,
  overrides: ExitPlanOverrides = {},
): Promise<HeldPlan> {
  const merged: Record<string, unknown> = { ...overrides };
  if (overrides.tool_input) {
    merged['tool_input'] = {
      ...(loadFixture(EXIT_PLAN_FIXTURE, { session_id: sid, cwd })['tool_input'] as Record<
        string,
        unknown
      >),
  ...
  // `snapshot` is assigned in the same predicate tick that produced `q`, so it
  // is always non-null once waitUntil resolves; the assert narrows the type
  // for callers and can never actually fire.
  assert.ok(
    snapshot,
    'internal: holdExitPlan captured the /state snapshot that produced the question',
  );
...
test('capture-before-answer: the plan row appears in /state (status proposed, plan_md byte-identical to the fixture) WHILE the ExitPlanMode question is still pending, and the question carries plan_id', async (t) => {
  ...
  assert.ok(
    plan,
    'a plans row should exist for this session while the ExitPlanMode question is still pending (capture happens BEFORE holding)',
  );
  assert.equal(plan.status, 'proposed', 'a freshly captured plan should be status:proposed');
  assert.equal(
    plan.plan_md,
    (payload['tool_input'] as ExitPlanToolInput).plan,
    "plan_md must be byte-identical to the fixture's tool_input.plan (markdown fidelity)",
  );
  assert.ok(
    plan.plan_id !== undefined && plan.plan_id !== null,
    'plan row needs an id to be marked/answered against later',
  );
  ...
  assert.ok(
    qPlanId !== undefined,
    `the held question's /state entry must carry plan_id somewhere (top-level or payload); got question: ${JSON.stringify(q)}`,
  );
  t.diagnostic(`question.plan_id found at: ${String(where)}`);
  assert.equal(
    String(qPlanId),
    String(plan.plan_id),
    "the question's plan_id must match the captured plan's plan_id",
  );
...
test('capture rollback: a failing plan insert leaves NEITHER the question nor the plan visible, and the hook fails open', async (t) => {
  ...
  assert.deepEqual(
    res.json,
    {},
    'fail-open: the terminal gets the bare immediate {} answer, not a held response',
  );
  assert.ok(
    elapsed < holdMs,
...
test('answer path: {behavior:"capture"} denies the held hook bare AND mails the pinned capture notice to the planner\'s next UserPromptSubmit; plan becomes captured', async (t) => {
  ...
  assert.equal(
    ansRes.status,
    200,
    `{behavior:"capture"} should 200 (got ${ansRes.status}: ${JSON.stringify(ansRes.json)})`,
  );
  ...
  const ctx = (upRes.json as HookContext | null)?.hookSpecificOutput?.additionalContext ?? '';
  ...
  const PINNED_CAPTURE_MAIL =
    '[FLEETDECK] Your plan was captured to the fleet plan library — do not execute it. Wrap up your turn.';
  assert.ok(
    ctx.includes(PINNED_CAPTURE_MAIL),
    `the planner's next UserPromptSubmit must carry the pinned capture mail VERBATIM — ${JSON.stringify(PINNED_CAPTURE_MAIL)} (got: ${JSON.stringify(ctx)})`,
  );
});
...
test('answer path: an unanswered ExitPlanMode hold expires to {} and the plan stays proposed (no activity follows, so it must NEVER settle)', async (t) => {
  ...
  assert.deepEqual(
    heldRes.json,
    {},
    'expiry should resolve to {} same as any other permission hold',
  );
  ...
  assert.equal(
    plan.status,
    'proposed',
    'hold expiry must leave the plan status untouched at proposed — the terminal chooser owns the question now, not the plan',
  );
});
...
test('regression: PermissionRequest tool_name=AskUserQuestion still answers {} in <200ms untouched by the v1.3 ExitPlanMode capture wiring', async (t) => {
  ...
  assert.deepEqual(
    res.json,
    {},
    'PermissionRequest for AskUserQuestion must still answer {} immediately (v1.2 F3c guard)',
  );
  ...
  assert.equal(
    questionsFor(state, sid, 'permission').length,
    0,
    'the guard must not create a permission hold row',
  );
  assert.equal(
    plansFor(state, sid).length,
    0,
    'AskUserQuestion is not ExitPlanMode — no plan row should ever be captured for it',
  );
});
...
test('/state plans: caps at 20 non-archived rows, newest first; archiving frees a cap slot and excludes the archived row', async (t) => {
  ...
  const TOTAL = 25;
  const pending: Promise<JsonResponse>[] = [];
  const indexOf = (md: string | null): number => Number(/# Test Plan (\d+)/.exec(md ?? '')?.[1]);
  ...
  // waitUntil only resolves once `Array.isArray(s.plans) && length >= 20`, so
  // `state.plans` is provably a populated array here; the assert narrows the
  // optional type and can never fire.
  assert.ok(state.plans, 'internal: /state exposed a plans array');
  assert.equal(
    state.plans.length,
    20,
    `/state plans must cap at 20 rows (got ${state.plans.length})`,
  );
  for (let i = 1; i < state.plans.length; i++) {
    const prev = state.plans[i - 1];
    const cur = state.plans[i];
    assert.ok(prev && cur, 'internal: adjacent plan rows exist for the ordering check');
    assert.ok(
      prev.created_at >= cur.created_at,
      `plans must be newest-first by created_at (position ${i - 1}=${prev.created_at}, position ${i}=${cur.created_at})`,
    );
  }
  ...
  assert.ok(newest, 'internal: the newest plan row exists');
  assert.equal(
    indexOf(newest.plan_md),
    TOTAL - 1,
    'sanity: state.plans[0] should be the newest (index 24)',
  );
  const markRes = await postJson(`${daemon.baseUrl}/api/plans/${String(newest.plan_id)}/mark`, {
    status: 'archived',
  });
  assert.equal(
    markRes.status,
...
test('mark: proposed -> executed records the optional {via} on /state, and it survives a daemon restart', async (t) => {
  ...
  try {
    ...
    const markRes = await postJson(`${daemon.baseUrl}/api/plans/${String(planId)}/mark`, {
      status: 'executed',
      via: 'assign',
    });
    assert.equal(
    ...
    assert.equal(
      plan1.via,
      'assign',
      '/state plan entry must expose the `via` recorded by the mark (provenance contract, BUG-206)',
    );
    ...
    assert.equal(
      plan2?.status,
      'executed',
      'an unrelated hold timeout must not revert a plan already marked executed',
    );
  ...
  assert.equal(
    plan3.via,
    'assign',
    'recorded `via` must survive a daemon restart (durable plan history)',
  );
});
...
test('mark: captured -> executed', async (t) => {
  ...
  assert.equal(
    ...
    'captured',
    'sanity: plan should be captured before marking executed',
  );
...
test('mark: archived from each non-archived status (proposed, approved, captured, rejected, executed); rejected/archived -> executed both 409', async (t) => {
  ...
  const behaviorFor: Record<'approved' | 'rejected' | 'captured' | 'executed', string> = {
    approved: 'allow',
    rejected: 'deny',
    captured: 'capture',
    executed: 'allow',
  };
  const pendingHolds: Promise<JsonResponse>[] = [];

  async function makePlanInStatus(
    status: 'proposed' | 'approved' | 'rejected' | 'captured' | 'executed',
  ): Promise<number | null> {
    const sid = randomUUID();
    const { held, q } = await holdExitPlan(daemon, sid, cwd, holdMs);
    ...
    if (status === 'executed') {
      ...
      assert.equal(
        markRes.status,
        200,
        `sanity: setting up the executed fixture should 200 (got ${markRes.status})`,
      );
    }
    return planId;
  }
  ...
  const badTransition = await postJson(
    `${daemon.baseUrl}/api/plans/${String(rejectedPlanId)}/mark`,
    { status: 'executed' },
  );
  assert.equal(
    badTransition.status,
    409,
    `rejected -> executed is not an allowed transition (got ${badTransition.status}: ${JSON.stringify(badTransition.json)})`,
  );

  const archiveRes = await postJson(`${daemon.baseUrl}/api/plans/${String(rejectedPlanId)}/mark`, {
    status: 'archived',
  });
  assert.equal(
    archiveRes.status,
  ...
  const archivedTransition = await postJson(
    `${daemon.baseUrl}/api/plans/${String(rejectedPlanId)}/mark`,
    { status: 'executed' },
  );
  assert.equal(
    archivedTransition.status,
    409,
    `archived -> executed must 409 (got ${archivedTransition.status}: ${JSON.stringify(archivedTransition.json)})`,
  );
...
test('mark: 404 for an unknown plan id', async (t) => {
  ...
  const res = await postJson(`${daemon.baseUrl}/api/plans/does-not-exist-${randomUUID()}/mark`, {
    status: 'executed',
  });
...
test('assign: mails the daemon-reserved [FLEETDECK ASSIGNMENT] frame to the target and marks the plan executed', async (t) => {
  ...
  const res = await postJson(`${daemon.baseUrl}/api/plans/${String(plan0.plan_id)}/assign`, {
    to: targetSid,
  ...
  assert.equal(
    res.status,
    200,
    `assign should 200 (got ${res.status}: ${JSON.stringify(res.json)})`,
  );
  assert.equal((res.json as AssignResult | null)?.ok, true);
  assert.equal(
    (res.json as AssignResult | null)?.session_id,
    targetSid,
    'assign should report the resolved target session id',
  );
  ...
  const box = (await getJson(`${daemon.baseUrl}/mail?session=${encodeURIComponent(targetSid)}`))
    .json as MailBox;
  assert.equal(box.mail.length, 1, 'target should have exactly one mail (the assignment)');
  const m = box.mail[0];
  assert.ok(m, 'internal: the single mail row exists');
  assert.equal(
    m.from,
    'orchestrator',
    'the assignment frame is daemon-sent (reserved sender, never board-forged)',
  );
  assert.ok(
    m.text.startsWith('[FLEETDECK ASSIGNMENT]'),
    `mail must open with the reserved assignment frame (got: ${m.text.slice(0, 60)}...)`,
  );
  assert.ok(
    m.text.includes('Custom instructions: run only the migration'),
    'custom instructions ride the frame',
  );
  assert.ok(
    m.text.includes((payload['tool_input'] as ExitPlanToolInput).plan),
    'the full plan markdown is the frame body',
  );
  ...
  if (plan2.via !== undefined) {
    assert.equal(
      plan2.via,
      `assign:${targetSid}`,
      'when `via` is exposed it should record the assign target',
    );
  }
  ...
  assert.equal(
    ans.status,
    200,
    'the planner hold should resolve once the plan is assigned elsewhere',
  );
});
...
test('assign: bad requests and bad transitions are refused without sending mail', async (t) => {
  ...
  const unknownPlan = await postJson(`${daemon.baseUrl}/api/plans/999999999/assign`, {
    to: targetSid,
  });
  assert.equal(
    unknownPlan.status,
    404,
    `assigning an unknown plan should 404 (got ${unknownPlan.status})`,
  );
  const unknownTarget = await postJson(
    `${daemon.baseUrl}/api/plans/${String(plan0.plan_id)}/assign`,
    { to: 'no-such-session-xyz' },
  );
  assert.equal(
    unknownTarget.status,
    404,
    `assigning to an unknown target should 404 (got ${unknownTarget.status})`,
  );
  const missingTo = await postJson(
    `${daemon.baseUrl}/api/plans/${String(plan0.plan_id)}/assign`,
    {},
  );
  assert.equal(
    missingTo.status,
    400,
    `assign without a target should 400 (got ${missingTo.status})`,
  );
  const badInstr = await postJson(`${daemon.baseUrl}/api/plans/${String(plan0.plan_id)}/assign`, {
    to: targetSid,
    instructions: 42,
  });
  ...
  assert.equal(ansRes.status, 200);
  await held;
  const rejected = await postJson(`${daemon.baseUrl}/api/plans/${String(plan0.plan_id)}/assign`, {
    to: targetSid,
  });
  assert.equal(
    rejected.status,
    409,
    `assigning a rejected plan should 409 (got ${rejected.status}: ${JSON.stringify(rejected.json)})`,
  );
...
test('in-terminal: turn-boundary activity retires the question and settles the plan handled-in-terminal in the same tick', async (t) => {
  ...
  await postHook(
    daemon.baseUrl,
    'UserPromptSubmit',
    loadFixture('user-prompt-submit', { session_id: sid, cwd }, { prompt: 'looks good, go' }),
    { token: daemon },
  );
  ...
  assert.equal(
    plan1?.status,
    'handled-in-terminal',
    `the turn boundary must settle the plan to handled-in-terminal within one snapshot refresh (got ${String(plan1?.status)})`,
  );
  const q1 = questionsFor(state1, sid, 'permission').find(
    (x) => x.payload?.tool_name === 'ExitPlanMode',
  );
  assert.equal(
    q1?.status,
    'expired',
    'the question row retires as expired, not answered — nobody answered it from the board',
  );
});
...
test('in-terminal: timer expiry THEN later PostToolUse activity settles handled-in-terminal (the deferred gate)', async (t) => {
  ...
  assert.equal(
    ...
    'proposed',
    'expiry alone is not a decision — the plan must wait for activity',
  );

  await postHook(
    daemon.baseUrl,
    'PostToolUse',
    loadFixture('post-tool-use-edit', { session_id: sid, cwd }),
    { token: daemon },
  );
...
test('in-terminal: a plan question raised in the SAME turn (still pending, never parked) must NOT be flipped by the gate', async (t) => {
  ...
  const upsRes = await postHook(
    daemon.baseUrl,
    'UserPromptSubmit',
    loadFixture(
      'user-prompt-submit',
      { session_id: sid, cwd },
      { prompt: 'looks good, one change' },
    ),
    { token: daemon },
  );
  ...
  const ansRes = await postJson(`${daemon.baseUrl}/api/questions/${second.q.id}/answer`, {
    behavior: 'deny',
  });
  assert.equal(ansRes.status, 200);
  await second.held;
...
test('in-terminal: handled-in-terminal cannot be marked executed (409), can be archived', async (t) => {
  ...
  await postHook(
    daemon.baseUrl,
    'UserPromptSubmit',
    loadFixture('user-prompt-submit', { session_id: sid, cwd }, { prompt: 'go' }),
    { token: daemon },
  );
  assert.equal(
    ((await getJson(`${daemon.baseUrl}/state`)).json as StateSnapshot).plans?.find(
  ...
  const execRes = await postJson(`${daemon.baseUrl}/api/plans/${String(planId)}/mark`, {
    status: 'executed',
  });
  assert.equal(
    execRes.status,
  ...
  const archRes = await postJson(`${daemon.baseUrl}/api/plans/${String(planId)}/mark`, {
    status: 'archived',
  });
  assert.equal(
    archRes.status,
...
test('mark executed with the question still pending dismisses it (fails the hold open with {})', async (t) => {
  ...
  const heldRes = await held;
  assert.ok(heldRes, 'internal: the hold was rebound to the real fetch before awaiting');
  assert.deepEqual(
    heldRes.json,
    {},
...
function planById(state: StateSnapshot, planId: number | null | undefined): PlanEntry | undefined {
...
test('BUG-040: spawn carrying plan_id claims the plan atomically BEFORE launch; a second claim 409s and launches nothing', async (t) => {
  ...
  const res = await postJson(`${daemon.baseUrl}/api/spawn`, {
    cwd,
    prompt: 'Execute this approved plan exactly.',
    plan_id: plan.plan_id,
  });
  assert.equal(
    res.status,
    200,
    `the claiming spawn should succeed (got ${res.status}: ${JSON.stringify(res.json)})`,
  );
  ...
  assert.equal(
    claimed?.status,
    'executed',
    'the plan must be executed the moment the spawn is accepted, not after a client mark',
  );
  if (claimed.via !== undefined) {
    assert.equal(
      claimed.via,
      `spawn:${(res.json as SpawnOkBody).spawn_id}`,
      'the recorded via must name the spawn that executed the plan',
    );
  }
  ...
  const res2 = await postJson(`${daemon.baseUrl}/api/spawn`, {
    cwd,
    prompt: 'Execute this approved plan exactly.',
    plan_id: plan.plan_id,
  });
  assert.equal(
    res2.status,
    409,
    `a second execution claim must 409 (got ${res2.status}: ${JSON.stringify(res2.json)})`,
  );
  assert.equal((res2.json as { ok: boolean }).ok, false);

  const specs = await waitForSpecRecords(rec, 1);
  assert.equal(
    specs.length,
    1,
    'exactly ONE executor was launched; the losing claim never reached the spawn backend',
  );
  ...
  const markRes = await postJson(`${daemon.baseUrl}/api/plans/${String(plan.plan_id)}/mark`, {
    status: 'executed',
    via: 'spawn:late',
  });
...
test('BUG-040: a refused spawn releases the claim — plan returns to proposed and is executable again', async (t) => {
  ...
  const good = await postJson(`${daemon.baseUrl}/api/spawn`, {
    cwd,
    prompt: 'Execute this approved plan exactly.',
    plan_id: plan.plan_id,
  });
...
test('BUG-040: spawn claim refusals — unknown plan 404, non-executable plan 409, shell kind cannot carry plan_id', async (t) => {
  ...
  const unknown = await postJson(`${daemon.baseUrl}/api/spawn`, {
    cwd,
    prompt: 'x',
    plan_id: 999999,
  });
  assert.equal(
    unknown.status,
    404,
    `an unknown plan_id must 404 (got ${unknown.status}: ${JSON.stringify(unknown.json)})`,
  );
  ...
  assert.equal(
    shell.status,
    400,
    `a shell spawn must refuse plan_id (got ${shell.status}: ${JSON.stringify(shell.json)})`,
  );
  ...
  assert.equal(ansRes.status, 200);
  await held;
  const denied = await postJson(`${daemon.baseUrl}/api/spawn`, {
    cwd,
    prompt: 'x',
    plan_id: planId,
  });
  assert.equal(
    denied.status,
    409,
    `a rejected plan must refuse an execution claim (got ${denied.status}: ${JSON.stringify(denied.json)})`,
  );
