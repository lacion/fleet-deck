...
// The stored shape a hook fixture leaves on a question row. Only the fields the
// correlation tests read are modeled; tool_use_id/tool_input stay `unknown`
// because the whole point is that the first is absent and the second is opaque.
interface Fixture {
  tool_use_id?: unknown;
  tool_name?: string;
  tool_input?: unknown;
...
// answer() returns a discriminated { status, body } union whose error branches
// alone carry `err`. Widening to this shape (every optional) lets the BUG-137
// rejection tests read `body.err` without narrowing each branch.
interface AnswerResult {
  status: number;
  body: { ok: boolean; err?: string; delivered?: boolean; note?: string };
}
...
function fixture(
  name: string,
  { session = 'sess-1', cwd = '/work' }: { session?: string; cwd?: string } = {},
): Fixture {
...
test('M-B1: a real permission fixture carries no tool_use_id, and PostToolUse fixtures carry none either', () => {
  ...
  assert.ok(
    perm.tool_input && typeof perm.tool_input === 'object',
    'it does carry tool_name + tool_input',
  );
  for (const f of ['post-tool-use-bash', 'post-tool-use-edit']) {
    assert.equal(
      fixture(f).tool_use_id,
      undefined,
      `${f} fixture has no tool_use_id — the old correlation key never existed`,
    );
  }
});
...
test('M-B1 (b): a completing PostToolUse expires only its own hold; a parallel sibling for a DIFFERENT tool call is preserved', () => {
  ...
  const changed = questions.expireOnActivity('s1', {
    toolName: permA.tool_name,
    toolInput: permA.tool_input,
  });
...
test('BUG 5: IDENTICAL-input parallel holds — a single PostToolUse expires exactly ONE, its twin is preserved', () => {
  ...
  assert.deepEqual(
    repliesA,
    [{}],
    'the OLDEST matching hold is failed open — one completion, one release',
  );
  assert.deepEqual(
    repliesB,
    [],
    'its identical-input twin stays held — one PostToolUse cannot retire both',
  );
  assert.deepEqual(
    questions.pendingOf('s1').map((r) => r.id),
    [rowB.id],
  ...
  const changed2 = questions.expireOnActivity('s1', {
    toolName: perm.tool_name,
    toolInput: perm.tool_input,
  });
...
test('BUG-138: completing an ANSWERED duplicate permission does NOT expire its still-pending identical twin', () => {
  ...
  assert.equal(
    changed,
    false,
    'the answered call absorbs its own completion — nothing pending was retired',
  );
  assert.deepEqual(
    repliesB,
    [],
    'the still-pending twin keeps its hold — the board card stays live',
  );
  assert.deepEqual(
    questions.pendingOf('s1').map((r) => r.id),
    [rowB.id],
...
test('BUG-138 (triple): one answered hold absorbs exactly ONE completion; further identical completions retire twins in order', () => {
  ...
  const repliesB: unknown[] = [];
  const repliesC: unknown[] = [];
  const rowA = questions.create('permission', 's1', perm);
  const rowB = questions.create('permission', 's1', perm);
  const rowC = questions.create('permission', 's1', perm);
  questions.attachHold(rowA, () => {
    /* A's reply is asserted via the pending set, not captured */
  });
  questions.attachHold(rowB, (b) => repliesB.push(b));
  ...
  const act = (): boolean =>
    questions.expireOnActivity('s1', { toolName: perm.tool_name, toolInput: perm.tool_input });
...
test('M-B1 (b2): a completing tool call that matches NO hold (e.g. a Read that needed no permission) leaves every hold pending', () => {
  ...
  const changed = questions.expireOnActivity('s1', {
    toolName: 'Read',
    toolInput: { file_path: '/work/util.js' },
  });
...
test('M-B1 (c): an activity with no tool identity (a turn boundary / UserPromptSubmit) expires session-wide — every hold and freeform row', () => {
  ...
  const repliesP: unknown[] = [];
  const repliesE: unknown[] = [];
  ...
  assert.deepEqual(
    repliesE,
    [{}],
    'the elicitation hold (no tool_name) is failed open on the turn boundary',
  );
  assert.deepEqual(
    questions.pendingOf('s1'),
    [],
    'the turn boundary clears everything, freeform included',
  );
  db.close();
});
...
test('M-B7: a timer persistence failure still fails the hold open without an uncaught exception', async () => {
  ...
  const row = questions.create(
    'permission',
    'timer-session',
    fixture('permission-request', { session: 'timer-session' }),
  );
  ...
  const keepAlive = setTimeout(() => {
    /* pin the loop open for the 25ms hold */
  }, 250);
  try {
    ...
    const firstLog = logged[0];
    assert(firstLog);
    assert.match(String(firstLog[0]), /expiry persistence error/);
  } finally {
...
test('BUG-137: an oversized freeform answer is rejected 413 and the question stays pending', () => {
  const db = openDb(':memory:');
  const mailed: { sid: string; from: string; text: string }[] = [];
  const questions = createQuestions(db, {
  ...
  const out = questions.answer(row.id, { text: 'yes — ' + 'x'.repeat(200) }) as AnswerResult;

  assert.equal(
    out.status,
    413,
    'an answer whose framed form exceeds the mailbox clamp is rejected',
  );
  assert.equal(out.body.ok, false);
  assert.match(out.body.err ?? '', /too long/);
  assert.equal(mailed.length, 0, 'nothing truncated ever reaches the mailbox');
  assert.equal(
    questions.pendingOf('s1').length,
    1,
    'the question is still pending — the human can retry',
  );
  db.close();
});
...
test('BUG-137: an oversized re-armed hold answer is rejected 413; the row and its re-arm chain survive', () => {
  ...
  const out = questions.answer(row.id, { behavior: 'allow' }) as AnswerResult;

  assert.equal(
    out.status,
    413,
    'even a short decision rejects when the fixed re-arm frame alone overflows',
  );
  assert.equal(out.body.ok, false);
...
test('BUG-137: an answer that fits the clamp still settles and mails exactly as before', () => {
  ...
  const sent = mailed[0];
  assert(sent);
  assert.match(sent.text, /^\[FLEETDECK ANSWER\] Q: Ship it\? — A: yes, ship it$/);
