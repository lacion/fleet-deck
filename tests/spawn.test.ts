...
import { mkdtempSync, mkdirSync, rmSync, existsSync, chmodSync } from 'node:fs';
...
// The POST /api/spawn success body. contracts/spawn.ts types only the REQUEST,
// so the success response shape is declared locally (per this file's caveat).
interface SpawnOk {
  ok: boolean;
  spawn_id: string;
  session_id: string;
...
// A refused spawn or an unrouted /command reply. `json` can be null when a body
// fails to parse, which is why callers reach it through `?.`.
...
// The spawn-capability object carried on /health and /state. contracts'
// SpawnCapability has no `max`, but these tests assert `spawn.max === undefined`,
// so the view carries an optional `max` to permit (and pin the absence of) it.
interface SpawnCapabilityView {
  available: boolean;
  active: number;
  reason?: string;
  max?: number;
...
interface CapabilityCarrier {
  spawn?: SpawnCapabilityView;
}

// One recorded FLEETDECK_SPAWN_CMD invocation (spawn-cmd-fixture.ts writes these;
// waitForSpecRecords hands them back as unknown[]).
...
function scratchDir(prefix = 'fleetdeck-spawn-'): string {
...
): Promise<NonNullable<Awaited<T>>> => waitUntilBase(fn, { timeoutMs: 8000, ...opts });
...
function extractArgv(spec: unknown): unknown[] | null {
  if (spec !== null && typeof spec === 'object') {
    const argv = (spec as Record<string, unknown>)['argv'];
...
function specCwd(spec: unknown): string | null {
...
function containsExactString(value: unknown, target: string, seen = new Set<unknown>()): boolean {
...
test('capability: FLEETDECK_SPAWN_CMD reports available:true reason:test-override on /health and /state', async (t) => {
  ...
  const healthSpawn = (health.json as CapabilityCarrier | null)?.spawn;
  assert.ok(healthSpawn, '/health should carry a top-level spawn capability object');
  assert.equal(
    healthSpawn.available,
    true,
    'FLEETDECK_SPAWN_CMD should make the daemon report spawn availability',
  );
  assert.equal(
    healthSpawn.reason,
    'test-override',
    'the override reason must be exactly "test-override"',
  );
  assert.equal(typeof healthSpawn.active, 'number', 'spawn.active should be a number');
  assert.equal(healthSpawn.active, 0, 'no spawns have happened yet');
  assert.equal(healthSpawn.max, undefined, 'there is no fleet cap — spawn.max must not come back');
  ...
  assert.ok(
    stateSpawn,
    '/state should carry the same top-level spawn capability object as /health',
  );
  assert.equal(stateSpawn.available, true);
  assert.equal(stateSpawn.reason, 'test-override');
});
...
test('capability: FLEETDECK_SPAWN=off reports available:false on /health and /state, and POST /api/spawn is refused 4xx', async (t) => {
  ...
  assert.ok(
    ...
    `POST /api/spawn with spawn disabled must 4xx (got ${res.status})`,
  );
  assert.equal(
    (res.json as SpawnRejection | null)?.ok,
    false,
    'a refused spawn must carry ok:false',
  );
});
...
test('spawn happy path: POST /api/spawn pre-creates the card immediately (source spawned, col queued, note mentions spawning) before any hook fires', async (t) => {
  ...
  assert.ok(
    body.spawn_id !== undefined && body.spawn_id !== null,
    'response should carry a spawn_id',
  );
  assert.match(body.session_id, UUID_RE, 'response should carry a UUID session_id');
  assert.ok(body.callsign, 'response should carry a callsign');
  assert.ok(body.tmux, 'response should carry a tmux descriptor');
  assert.equal(
    body.tmux.session,
    `fleetdeck-${daemon.port}`,
    'tmux session name should be scoped fleetdeck-<port>',
  );
  assert.equal(
    body.tmux.window,
    `fd${daemon.port}-${body.callsign}`,
  ...
  assert.equal(
    card.source,
    'spawned',
    'a spawned-but-not-yet-hooked card should carry source:spawned',
  );
  assert.equal(card.col, 'queued', 'the pre-created card should be col:queued');
  assert.match(
    card.note ?? '',
    /spawn/i,
    `the pre-created card's note should mention spawning (got: ${JSON.stringify(card.note)})`,
  );
});
...
test("spawn happy path: the fixture's SessionStart flips source to hooks and the spawns row to live, and the card gains a spawn{} descriptor", async (t) => {
  ...
  const body = res.json as SpawnOk;
  const { session_id: sid, spawn_id: spawnId } = body;
  ...
  assert.equal(
    card.col,
    'queued',
    'SessionStart should still derive col:queued as normal, now via the hooks path',
  );
  assert.ok(card.spawn, 'a hooked spawned card should carry a spawn{} descriptor');
  assert.equal(
    card.spawn.spawn_id,
    spawnId,
    'card.spawn.spawn_id should match the /api/spawn response',
  );
  assert.equal(
    card.spawn.tmux_window,
    body.tmux.window,
    'card.spawn.tmux_window should match the tmux window name',
  );
  assert.equal(
    card.spawn.status,
    'live',
    "the spawns row should flip to status 'live' on the session's first hook event",
  );
...
test('argv construction: prompt/model/permission-mode survive intact through the FLEETDECK_SPAWN_CMD override, including a shell-metachar-hostile prompt', async (t) => {
  ...
  const sid = (res.json as SpawnOk).session_id;

  const specs = await waitForSpecRecords(rec, 1);
  const record = specs[specs.length - 1] as SpecRecord;
  assert.equal(
    record.parseError,
    null,
    'the JSON.stringify(spec) argv element must parse back as valid JSON',
  );
  ...
  assert.ok(
    containsExactString(record.parsed, dangerousPrompt),
    `the exact dangerous prompt string must appear intact in the recorded spec (got: ${JSON.stringify(record.parsed)})`,
  );
  assert.ok(
    !existsSync(rmTarget),
    'sanity: the (harmless, nonexistent) rm target must still not exist',
  );

  const argv = extractArgv(record.parsed);
  assert.ok(
    argv,
    `expected an argv-shaped array in the spec per the contract ("claude --session-id <uuid> ..."); spec keys were: ${JSON.stringify(Object.keys(record.parsed))}`,
  );
  ...
  assert.deepEqual(
    argv.slice(0, expectedPrefix.length),
    expectedPrefix,
  ...
  assert.equal(
    argv[claudeIdx],
    'claude',
    'claude must immediately follow the deterministic env prefix',
  );
  ...
  assert.equal(
    argv[sidIdx + 1],
    sid,
    '--session-id must be followed by the pre-generated session UUID',
  );
  ...
  assert.equal(
    argv[argv.length - 2],
    '--',
    'a `--` end-of-options marker must immediately precede the prompt',
  );

  assert.ok(
    argv.includes(dangerousPrompt),
    'the dangerous prompt must appear as ONE whole argv element, not split into shell words',
  );
  ...
  assert.ok(
    !suspiciousSplit,
    'the prompt must not appear word-split across separate argv elements',
  );
});
...
test('option-injection: a prompt of "--dangerously-skip-permissions" rides after `--` as a positional and does NOT flip the persisted bypass flag', async (t) => {
  ...
  assert.ok(
    argv,
    `expected an argv-shaped array in the spec; spec keys were: ${JSON.stringify(Object.keys(record.parsed as object))}`,
  );
  ...
  assert.equal(
    argv[argv.length - 1],
    evilPrompt,
    'the flag-shaped prompt must be the final, positional argv element',
  );
  assert.equal(
    argv[argv.length - 2],
    '--',
...
test('fail-loud: missing cwd -> 4xx with a reason', async (t) => {
  ...
  const rejected = res.json as SpawnRejection | null;
  assert.equal(rejected?.ok, false);
  // `assert.equal` (node:assert/strict) is `asserts actual is T`, so the line
  // above narrows `rejected` to non-null; the reads below need no `?.`. The
  // reason check must use `assert.ok(typeof … === 'string')`, NOT `assert.equal(
  // typeof …, 'string')`: `assert.equal`'s `asserts` narrows the `typeof`
  // EXPRESSION, not `reason` itself, so it would not let the `.length` read pass.
  // Only the `=== 'string'` form is a typeof type-guard that narrows `reason` to
  // string (matching the original's unguarded `res.json.reason.length`). Pass/
  // fail is identical to the `.mjs`'s `assert.equal(typeof …, 'string')`.
  assert.ok(
    typeof rejected.reason === 'string',
    'a fail-loud rejection must carry a string reason',
  );
...
test('no fleet cap: the 8th concurrent spawn is as welcome as the 1st', async (t) => {
  ...
  const health = (await getJson(`${daemon.baseUrl}/health`)).json as { spawn: SpawnCapabilityView };
...
test('fail-loud: worktree:true on a non-git cwd -> 409', async (t) => {
  ...
  assert.equal(
    res.status,
    409,
    `worktree:true on a non-git cwd should 409 (got ${res.status}: ${JSON.stringify(res.json)})`,
  );
  assert.equal((res.json as SpawnRejection | null)?.ok, false);
});
...
test("worktree path: worktree:true creates a sibling --fd-<callsign> checkout on branch fd/<callsign>, and the card collapses to the main tree's repo_id", async (t) => {
  ...
  assert.equal(
    res.status,
    200,
    `worktree spawn on a real git repo should 200 (got ${res.status}: ${JSON.stringify(res.json)})`,
  );
  const { callsign, session_id: sid } = res.json as SpawnOk;

  const expectedWorktreePath = path.join(
    path.dirname(repo.root),
    `${path.basename(repo.root)}--fd-${callsign}`,
  );
  await waitUntil(() => existsSync(expectedWorktreePath) || null, {
    label: `sibling worktree directory ${expectedWorktreePath} appears`,
  });

  const listing = execFileSync('git', ['-C', repo.root, 'worktree', 'list', '--porcelain'], {
    encoding: 'utf8',
  });
  assert.match(
    listing,
    new RegExp(`worktree ${expectedWorktreePath.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`),
    `git worktree list should show the new sibling worktree (listing: ${listing})`,
  );
  assert.match(
    listing,
    new RegExp(`branch refs/heads/fd/${callsign.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`),
  ...
  const specs = await waitForSpecRecords(rec, 1);
  const cwdSeen = specCwd((specs[specs.length - 1] as SpecRecord).parsed);
  if (cwdSeen) {
    assert.equal(
      path.resolve(cwdSeen),
      path.resolve(expectedWorktreePath),
      'the spec handed to the spawn backend should carry the NEW worktree path as cwd',
    );
  } else {
    t.diagnostic(
      "could not locate a cwd-shaped field in the recorded spec to cross-check against the worktree path (see spec-shape caveat in this file's header)",
    );
  }
  ...
  assert.equal(
    card.repo_id,
    repo.gitCommonDir,
    "the worktree-spawned card's repo_id must collapse to the main tree's git-common-dir",
  );
});
...
test('fan-out: 4 agents spawned into ONE repo each get their own worktree and branch', async (t) => {
  ...
  const prompts = [
    'fix the flaky test',
    'update the README',
    'audit the spawn path',
    'fix the flaky test',
  ];
  const agents: SpawnOk[] = [];
  for (const prompt of prompts) {
    ...
    agents.push(res.json as SpawnOk);
  }
  ...
  assert.equal(
    new Set(agents.map((a) => a.tmux.window)).size,
    4,
    'tmux window names must be unique',
  );

  const listing = await waitUntil(
    () => {
      const out = execFileSync('git', ['-C', repo.root, 'worktree', 'list', '--porcelain'], {
        encoding: 'utf8',
      });
  ...
  for (const cs of callsigns) {
    ...
    assert.match(
      listing,
      new RegExp(`branch refs/heads/fd/${cs.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`),
    );
  }
...
test('kill: a live (non-offline) card is refused without force -> 409', async (t) => {
  ...
  const { spawn_id: spawnId, session_id: sid } = spawnRes.json as SpawnOk;
  ...
  assert.equal(
    killRes.status,
    409,
    `killing a live card without force should 409 (got ${killRes.status}: ${JSON.stringify(killRes.json)})`,
  );
});
...
test('kill: tmux lookup failure is 500 and keeps the offline spawn tracked', async (t) => {
  ...
  assert.match(
    (killRes.json as SpawnRejection | null)?.reason ?? '',
  ...
  assert.equal(
    card?.spawn?.status,
    beforeKillStatus,
    'failed kill does not mutate the tracked spawn status',
  );
});
...
test('stale flag: a working card with no events for FLEETDECK_STALE_MS gets stale:true; a fresh event clears it', async (t) => {
  ...
  await postHook(
    daemon.baseUrl,
    'UserPromptSubmit',
    { session_id: sid, hook_event_name: 'UserPromptSubmit', cwd, prompt: 'do a thing' },
    { token: daemon },
  );
  ...
  assert.equal(
    card.col,
    'working',
    'sanity: the session should be working before we let it go stale',
  );
  ...
  assert.equal(
    card.stale,
    true,
    `a working card with no events for > ${staleMs}ms should be stale:true`,
  );
  ...
  await postHook(
    ...
    'PostToolUse',
    {
      ...
      cwd,
      tool_name: 'Read',
      tool_input: { file_path: path.join(cwd, 'x.txt') },
    },
    { token: daemon },
  );
  state = (await getJson(`${daemon.baseUrl}/state`)).json as StateResponse;
...
test('unrouted assign-auto carries the task text verbatim on the response (v1.2 CTA support)', async (t) => {
  ...
  const body = res.json as SpawnRejection | null;
  assert.equal(body?.ok, false);
  // The strict-equal above narrows `body` to non-null, so the reads below drop `?.`.
  assert.equal(body.unrouted, true);
  assert.equal(
    body.text,
    text,
    `the unrouted response should carry the task text verbatim (got: ${JSON.stringify(res.json)})`,
  );
});
...
test('reconciliation: a spawn row whose window was never actually created in real tmux flips to gone + card offline after a daemon restart on the same port', async (t) => {
  ...
  t.after(() => {
    try {
      ...
      /* second.stop may already remove it */
    }
  });
  ...
  assert.equal(
    card.col,
    'offline',
    "restart reconciliation should mark a spawn row with no matching real window's card offline",
  );
  if (card.spawn?.status) {
    ...
    assert.equal(
      card.spawn.status,
      'gone',
      "contract rule: a spawning/stalled/live row whose window is gone at boot should become 'gone'",
    );
  } else {
    t.diagnostic(
      "card carries no spawn{} descriptor after restart -- could not directly confirm the row status reached 'gone' from /state alone; offline col is the observable proxy used here",
    );
  }
});
...
test('restart reconciliation removes a spawn-owned worktree left by a spawn interrupted before launch (BUG-153)', async (t) => {
  ...
  let callsign!: string;
  try {
    const spawnRes = await postJson(`${first.baseUrl}/api/spawn`, {
      cwd: repo.root,
      worktree: true,
    });
    assert.equal(
  ...
  assert.ok(
    ...
    `sanity: the worktree ${worktreePath} exists before the simulated crash`,
  );
  const branchGone = (): boolean =>
    !execFileSync('git', ['-C', repo.root, 'branch', '--list', `fd/${callsign}`], {
      encoding: 'utf8',
    }).trim();
  ...
  const card = await waitUntil(
    ...
    {
      label: "restart reconciliation settles the interrupted spawn's card offline",
      timeoutMs: 12000,
    },
  );
  ...
  await waitUntil(() => !existsSync(worktreePath) || null, {
    label: `stranded worktree ${worktreePath} is removed at boot`,
    timeoutMs: 12000,
  });
  const listing = execFileSync('git', ['-C', repo.root, 'worktree', 'list', '--porcelain'], {
    encoding: 'utf8',
  ...
  assert.ok(
    !listing.includes(worktreePath),
    `git worktree list must no longer register ${worktreePath} (listing: ${listing})`,
  );
...
test('ticket spawn: a spawn from a ticket-branch cwd is ticket-first (callsign, worktree dir, fd/ branch); its own SessionStart does not re-rename', async (t) => {
  ...
  const repo = makeRepoWithWorktree({
    repoName: 'fd-spawn-ticket',
    branch: 'feature/PROJ-123-work',
  });
  const daemon = await startDaemon({
    port,
    env: spawnCmdEnv({ recordFile: rec, postUrl: baseUrl }),
  ...
  assert.equal(
    res.status,
    200,
    `ticket worktree spawn should 200 (got ${res.status}: ${JSON.stringify(res.json)})`,
  );
  const { callsign, session_id: sid } = res.json as SpawnOk;
  ...
  assert.match(
    listing,
    new RegExp(`worktree ${expectedWorktree.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`),
    `git worktree list should show the ticket-first worktree (listing: ${listing})`,
  );
  assert.match(
    listing,
    new RegExp(`branch refs/heads/fd/PROJ-123-${animal}\\b`),
  ...
  assert.equal(
    card.callsign,
    callsign,
    'the spawned SessionStart must not re-rename the inherited callsign',
  );
...
test('ticket spawn leftover-artifact: a pre-existing ticket-first worktree dir forces a deduped -<sid4> workname', async (t) => {
  ...
  const rec = makeSpecRecordFile(t);
  const repo = makeRepoWithWorktree({
    repoName: 'fd-spawn-ticket-dedup',
    branch: 'feature/PROJ-123-work',
  });
  const daemon = await startDaemon({
    port,
  ...
  assert.equal(
    res.status,
    200,
    `dedup spawn should still 200 (got ${res.status}: ${JSON.stringify(res.json)})`,
  );
  const { callsign, session_id: sid } = res.json as SpawnOk;
  ...
  assert.equal(
    callsign,
    'falcon-PROJ-123',
    `the first spawn should be falcon-PROJ-123 (got ${callsign})`,
  );
  ...
  const specs = await waitForSpecRecords(rec, 1);
  const actualWorktree = specCwd((specs[specs.length - 1] as SpecRecord).parsed);
  assert.ok(actualWorktree, 'the spec should carry the effective worktree cwd');
  assert.equal(
    path.basename(actualWorktree),
    `${path.basename(cwd)}--fd-PROJ-123-falcon-${sid4}`,
    `the worktree dir should be the deduped -<sid4> workname (got ${actualWorktree})`,
  );
  assert.notEqual(
    path.resolve(actualWorktree),
    path.resolve(collidedDir),
  ...
  assert.match(
    listing,
    new RegExp(`branch refs/heads/fd/PROJ-123-falcon-${sid4}\\b`),
...
test('ticketless spawn keeps the plain --fd-<callsign> worktree format (no ticket in the path)', async (t) => {
  ...
  const repo = makeRepoWithWorktree({
    repoName: 'fd-spawn-noticket',
    branch: 'feature/no-key-here',
  });
  const daemon = await startDaemon({
    port,
    env: spawnCmdEnv({ recordFile: rec, postUrl: baseUrl }),
  ...
  assert.equal(
    res.status,
    200,
    `ticketless worktree spawn should 200 (got ${res.status}: ${JSON.stringify(res.json)})`,
  );
  const { callsign } = res.json as SpawnOk;
  assert.match(
    callsign,
    /^[a-z]+-[0-9a-f]{4}$/,
    `a ticketless spawn keeps the hex callsign (got ${callsign})`,
  );
  ...
  assert.ok(!expectedWorktree.includes('PROJ-'), 'no ticket appears in a ticketless worktree path');
});
