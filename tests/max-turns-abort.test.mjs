...
function runSessionEndShim(daemon, payload) {
...
test('SessionEnd tombstones a session even when Stop was never sent (max-turns abort shape)', async (t) => {
  ...
  assert.match(sessionEndCmd.command, /fleet-hook\.mjs"?\s+SessionEnd/,
  ...
  const payload = JSON.stringify(loadFixture('session-end', { session_id: sid, cwd }, { reason: 'other' }));
  ...
  assert.deepEqual(JSON.parse(shim.stdout || '{}'), {}, 'SessionEnd should respond {} through the shim');
