// tests/demo-accept-isolation.test.ts
//
...
const FLEETD = path.join(REPO_ROOT, 'src/daemon/fleetd.ts');

const bash = (script: string): string =>
...
test('BUG-097: demo acceptance gates allocate unique per-run resources', () => {
  ...
  const gates: [string, string][] = [
    ['run-accept-spawn.sh', spawnGate],
    ['run-accept-plan.sh', planGate],
  ];
  for (const [name, text] of gates) {
    assert.ok(
  ...
  assert.ok(
    !phase3Gate.includes('REAL_HOME='),
    'run-accept-phase3.sh still kills the real daemon pidfile',
  );
  assert.ok(
    !phase3Gate.includes('fuser'),
    'run-accept-phase3.sh still force-clears a port by name',
  );
  ...
  assert.match(
    phase3Gate,
    /WORK_ROOT="\$\(mktemp -d /,
    'run-accept-phase3.sh must mktemp a unique fixture/evidence root',
  );
  assert.ok(
