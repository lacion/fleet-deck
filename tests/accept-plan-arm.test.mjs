...
const SCRIPT = path.join(
  path.dirname(fileURLToPath(import.meta.url)), '..', 'demo', 'run-accept-plan.sh');

function gate6Body(src) {
  const start = src.indexOf('# --------------------------------------------------------------- gate 6');
  const end = src.indexOf('# --------------------------------------------------------------- gate 7');
...
test('BUG-014: plan gate arms an unsupervised executor spawn before POSTing it', () => {
  ...
  assert.match(src, /TOKEN="\$\(cat "\$SCRATCH_HOME\/token"/,
  ...
  assert.match(gate6, /POST "\$BASE\/api\/spawn\/arm-unsupervised"/,
    'gate 6 must POST the arm-unsupervised endpoint');
  assert.match(gate6, /authorization: Bearer \$TOKEN/,
  ...
  assert.ok(armIdx !== -1 && bodyIdx > armIdx && spawnIdx > bodyIdx,
  ...
  assert.match(gate6, /arm_token: process\.env\.ARM_TOKEN/,
  ...
  assert.match(gate6, /bad "arm unsupervised spawn"/,
    'gate 6 must fail loud when the arm is refused');
  assert.match(gate6, /if \[ -s "\$PLAN_FILE" \] && \[ -n "\$ARM_TOKEN" \]/,
