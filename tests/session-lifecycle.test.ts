...
test('telemetry derivation walks queued -> working -> editing -> verifying -> needsyou -> idle -> offline', async (t) => {
  ...
  assert.ok(
    (startRes.json as SessionStartAck | null)?.callsign,
  ...
  // UserPromptSubmit -> working (+ task capture)
  const prompt =
    'Add an exported function slugify(s) to util.js (lowercase, trim, spaces to dashes, strip punctuation).';
  await postHook(
    daemon.baseUrl,
    'UserPromptSubmit',
  ...
  assert.ok(
    typeof card.task === 'string' && card.task.length > 0,
  ...
  await postHook(
    daemon.baseUrl,
    'PostToolUse',
    loadFixture('post-tool-use-edit', tokens, {
      tool_name: 'Edit',
  ...
  await postHook(
    daemon.baseUrl,
    'PostToolUse',
    loadFixture('post-tool-use-bash', tokens, {
      tool_name: 'Bash',
  ...
  await postHook(
    daemon.baseUrl,
    'UserPromptSubmit',
    loadFixture('user-prompt-submit', tokens, {
  ...
  assert.equal(
    card.col,
    'working',
    'UserPromptSubmit should reset col=working before the pytest assertion',
  );

  const pytestRes = await postHook(
    daemon.baseUrl,
    'PostToolUse',
    loadFixture('post-tool-use-bash', tokens, {
      tool_name: 'Bash',
  ...
  assert.equal(
    card.col,
    'verifying',
    'pytest should derive col=verifying from a non-verifying state',
  );
  ...
  await postHook(
    daemon.baseUrl,
    'Notification',
    loadFixture('notification', tokens, {
