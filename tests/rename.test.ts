...
function cardOf(state: StateResponse, sid: string) {
  return state.sessions.find((s) => s.session_id === sid);
}

async function boot(t: TestContext, prefix: string) {
...
async function startSession(daemon: DaemonHandle, cwd: string) {
  ...
  return {
    sid,
    callsign: (res.json as SessionStartAck).callsign,
    animal: (res.json as SessionStartAck).callsign.split('-')[0] ?? '',
  };
}
...
const rename = (
  daemon: DaemonHandle,
  sid: string,
  body: { suffix?: string | number; clear?: boolean },
) => postJson(`${daemon.baseUrl}/api/sessions/${sid}/name`, body);
const command = (daemon: DaemonHandle, text: string) =>
  postJson(`${daemon.baseUrl}/command`, { text });

const git = (args: string[], cwd: string) =>
  execFileSync('git', args, { cwd, encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'] }).trim();
const writeSeed = (dir: string, content: string) => {
  writeFileSync(path.join(dir, 'seed.txt'), content);
};
...
test('renaming keeps the animal and takes the suffix, and answers in the shape Compose already renders', async (t) => {
  ...
  assert.equal((res.json as RenameAck).ok, true);
  assert.equal((res.json as RenameAck).renamed, true);
  assert.equal(
    (res.json as RenameAck).callsign,
    `${animal}-docs-review`,
    'the animal survived; the suffix is the human’s',
  );
  ...
  const row = withDb(home, (db) =>
    db
      .prepare<{ custom_suffix: string | null }>(
        'SELECT custom_suffix FROM sessions WHERE session_id = ?',
      )
      .get(sid),
  );
...
test('mail addressed to the birth name still reaches a renamed session', async (t) => {
  ...
  const res = await postJson(
    `${daemon.baseUrl}/mail`,
    { to: callsign, from: 'operator', text: 'still finds you' },
    { token: daemon.token },
  );
  assert.equal(res.status, 200, JSON.stringify(res.json));
  ...
  assert.equal(
    ...
    1,
    'the old name is a fallback route, not a dead letter',
  );
});
...
test('bad suffixes are refused loudly — charset, leading dash, length, reserved words', async (t) => {
  ...
  for (const bad of ['docs review', 'docs.review', 'docs_review', '-leading', '', 'x'.repeat(25)]) {
    ...
    assert.match((res.json as RenameAck).reason, /letters, digits and dashes/);
  }
  ...
  for (const reserved of ['all', 'everyone']) {
    ...
    assert.match((res.json as RenameAck).reason, /reserved/);
  }
});
...
test('a name already held by another card is refused', async (t) => {
  ...
  const sessions: { sid: string; callsign: string; animal: string }[] = [];
  for (let i = 0; i < 13; i++) sessions.push(await startSession(daemon, cwd));
  const a = sessions[0];
  const b = sessions[12];
  assert.ok(a && b, 'the rotation should have produced both the 1st and 13th sessions');
  assert.equal(
    b.animal,
    a.animal,
    'sessions 1 and 13 wrap the 12-animal rotation onto the same animal',
  );
...
test('clearing a custom name reverts to the ticket name, or to the birth name when there is no ticket', async (t) => {
  ...
  assert.equal((res.json as RenameAck).callsign, callsign, 'back to the name it was born with');
  ...
  assert.equal(
    (res.json as RenameAck).callsign,
    `${animal}-PROJ-9`,
    'the automatic name for a ticketed card is its ticket name',
  );
});
...
test('a human name outranks branch auto-detection: a real ticket-branch checkout must not rename or ticket it', async (t) => {
  ...
  const start = await postHook(
    daemon.baseUrl,
    'SessionStart',
    { session_id: sid, cwd: repo.worktree, source: 'startup' },
    { token: daemon.token },
  );
  ...
  await postHook(
    daemon.baseUrl,
    'UserPromptSubmit',
    { session_id: sid, cwd: repo.worktree, prompt: 'back on the ticket branch' },
    { token: daemon.token },
  );
  ...
  assert.equal(
    card.branch,
    'feature/PROJ-7-auto',
    'the daemon saw the ticket branch — the auto path was actually reachable',
  );
  assert.equal(card.callsign, `${animal}-my-name`, 'automation never overwrites a human name');
  assert.equal(
    card.ticket ?? null,
    null,
    'branch detection must not even RECORD a ticket over a custom name',
  );
  const core = withDb(home, (db) =>
    db
      .prepare<{ custom_suffix: string | null; ticket_source: string | null }>(
        'SELECT custom_suffix, ticket_source FROM sessions WHERE session_id = ?',
      )
      .get(sid),
  );
  assert.ok(core, 'the session row should exist');
...
test('an explicit ticket command still wins over a human name', async (t) => {
  ...
  assert.equal((res.json as RenameAck).callsign, `${animal}-PROJ-1`);
  const after = withDb(home, (db) =>
    db
      .prepare<{ custom_suffix: string | null; ticket_source: string | null }>(
        'SELECT custom_suffix, ticket_source FROM sessions WHERE session_id = ?',
      )
  ...
  assert.ok(after, 'the session row should exist');
  assert.equal(
    after.custom_suffix,
    null,
    'an explicit ticket takes the name over from an explicit name',
  );
...
test('the `name` command mirrors the REST route and is never silently filed as a note', async (t) => {
  ...
  assert.equal((ok.json as RenameAck).renamed, true);
  assert.equal((ok.json as RenameAck).callsign, `${animal}-via-compose`);
  assert.equal((ok.json as RenameAck).previous, callsign);
  assert.equal((ok.json as RenameAck).session_id, sid);

  // A malformed rename is an error to show, never a note to file.
  const bare = await command(daemon, 'name');
  assert.equal((bare.json as RenameAck).ok, false);
  assert.match((bare.json as RenameAck).reason, /usage: name/);
  const bad = await command(daemon, `name ${animal}-via-compose has spaces`);
  assert.equal((bad.json as RenameAck).ok, false);
  const unknown = await command(daemon, 'name nobody-here whatever');
  assert.equal((unknown.json as RenameAck).ok, false);
  assert.match((unknown.json as RenameAck).reason, /no live session/);
  const scoped = await command(daemon, 'name all whatever');
  assert.equal((scoped.json as RenameAck).ok, false);
  assert.match((scoped.json as RenameAck).reason, /one session/);
});
...
test('an offline session cannot be renamed — its name is on its way back to the pool', async (t) => {
  ...
  assert.match((res.json as RenameAck).reason, /no live session/);
});
...
test('POST /api/sessions/:id/name validates its body', async (t) => {
  ...
  assert.equal(res.status, 400);
  assert.match((res.json as RenameAck).reason, /suffix must be a string/);
