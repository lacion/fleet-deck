...
async function holdPermission(daemon, sid, cwd, holdMs) {
  const held = postHook(
...
async function waitForRearmed(daemon, sid, label = 're-armed successor to appear') {
  return waitUntil(async () => {
    const state = (await getJson(`${daemon.baseUrl}/state`)).json;
    return questionsFor(state, sid).find(x => x.status === 'pending' && x.payload?.rearmed === true);
  }, { label });
}
...
test('re-arm: hold expiry with a still-parked session raises a fresh rearmed row; its answer rides the mail pipeline', async (t) => {
  const holdMs = 400;
  const graceMs = 300;
  const daemon = await startDaemon({ env: { FLEETDECK_HOLD_MS: String(holdMs), FLEETDECK_REARM_GRACE_MS: String(graceMs) } });
  ...
  assert.deepEqual(heldRes.json, {}, 'expiry still fails open with {} — nothing is ever auto-answered');
  ...
  const orig = (await getJson(`${daemon.baseUrl}/state`)).json.questions.find(x => x.id === q.id);
  ...
  const ansRes = await postJson(`${daemon.baseUrl}/api/questions/${succ.id}/answer`, { behavior: 'allow' });
  assert.equal(ansRes.status, 200);
  assert.equal(ansRes.json?.delivered, false, 'no socket — the answer cannot be delivered synchronously');
  assert.match(ansRes.json?.note ?? '', /next turn boundary/, 'the response must say what the delivery actually is');
  ...
  const upRes = await postHook(daemon.baseUrl, 'UserPromptSubmit', loadFixture('user-prompt-submit', { session_id: sid, cwd }), { token: daemon });
  const ctx = upRes.json?.hookSpecificOutput?.additionalContext ?? '';
  assert.ok(/\[FLEETDECK ANSWER\]/.test(ctx), `the re-armed answer must arrive as a [FLEETDECK ANSWER] mail (got: ${ctx.slice(0, 120)})`);
  assert.ok(ctx.includes('allow'), 'the mail carries the decision');
  assert.ok(ctx.includes('from fleetdeck-answer'), 'the mail is from the answer pipeline, not an operator');
});
...
test('re-arm: the chain caps at two re-arms, then the daemon gets out of the way', async (t) => {
  ...
  const second = await waitUntil(async () => {
    const state = (await getJson(`${daemon.baseUrl}/state`)).json;
    const rows = questionsFor(state, sid).filter(x => x.status === 'pending' && x.payload?.rearmed === true);
  ...
  const state = (await getJson(`${daemon.baseUrl}/state`)).json;
  const rearmed = questionsFor(state, sid).filter(x => x.payload?.rearmed === true);
  assert.equal(rearmed.length, 2, `the chain must stop at MAX_REARMS=2 (chain: ${q.id} → ${first.id} → ${second.id})`);
  assert.ok(rearmed.every(x => x.status !== 'pending'), 'no re-armed row stays pending past the cap');
});
...
test('re-arm: freeform rows never re-arm (they have no timer to expire on)', async (t) => {
  ...
  assert.equal(rows.filter(x => x.status === 'pending').length, 1, 'the freeform card stays pending, alone');
  assert.equal(rows.filter(x => x.payload?.rearmed === true).length, 0, 'no re-armed card may ever spawn from a freeform row');
});
...
test('hold_ms: POST /api/settings sets the window for NEW holds; FLEETDECK_HOLD_MS stays the override', async (t) => {
  ...
  const cwd = scratchCwd();
  let held = null; // the parked hook fetch — released before daemon.stop()
  t.after(async () => {
  ...
  res = await postJson(`${daemon.baseUrl}/api/settings`, { hold_ms: 5000 }, { token: daemon.token });
  assert.equal(res.status, 200);
  assert.equal(res.json?.settings?.hold_ms?.value, 5000);
  assert.equal(res.json?.settings?.hold_ms?.source, 'override');
  ...
  assert.ok(Math.abs(windowMs - 5000) < 1500, `a new hold should carry the configured 5000ms window (got ${windowMs}ms)`);
  const bad = await postJson(`${daemon.baseUrl}/api/settings`, { hold_ms: 'soon' }, { token: daemon.token });
  ...
  t.after(async () => { await daemon2.stop(); });
  res = await postJson(`${daemon2.baseUrl}/api/settings`, { hold_ms: 5000 }, { token: daemon2.token });
  assert.equal(res.status, 200);
  res = await getJson(`${daemon2.baseUrl}/api/settings`);
  assert.equal(res.json?.settings?.hold_ms?.value, 7000, 'FLEETDECK_HOLD_MS wins over the settings row');
...
test('answer at TTL−1s: a board answer one second before the window lapses still reaches the parked hook response', async (t) => {
  ...
  assert.equal(ansRes.json?.delivered, true);
