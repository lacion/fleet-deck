...
function findConflictFor(state: StateResponse, relOrAbsName: string) {
...
test('events from two worktrees of one repo collapse to one repo_id; cross-worktree collision is severity=info', async (t) => {
  ...
  await postHook(
    daemon.baseUrl,
    'SessionStart',
    loadFixture('session-start', { session_id: sidRoot, cwd: repo.root }),
    { token: daemon },
  );
  await postHook(
    daemon.baseUrl,
    'SessionStart',
    loadFixture('session-start', { session_id: sidWt, cwd: repo.worktree }),
    { token: daemon },
  );
  ...
  assert.equal(
    cardRoot.repo_id,
    cardWt.repo_id,
    'both worktrees of one repo must collapse to the same repo_id',
  );
  assert.equal(
    cardRoot.repo_name,
    repo.repoName,
    'repo_name should be the basename of the main tree',
  );
  assert.equal(
    cardWt.repo_name,
    repo.repoName,
    'repo_name is a repo-level property, same for the worktree session',
  );
  assert.notEqual(
    cardRoot.branch,
    cardWt.branch,
    'each session should still report its own worktree branch',
  );
  ...
  await postHook(
    ...
    loadFixture(
      'post-tool-use-edit',
      { token: daemon, session_id: sidRoot, cwd: repo.root } as FixtureTokens,
      {
        tool_input: {
          file_path: path.join(repo.root, 'shared.js'),
          old_string: 'a',
          new_string: 'b',
        },
      },
  ...
  const whisperRes = await postHook(
    daemon.baseUrl,
    'PostToolUse',
    loadFixture(
      'post-tool-use-edit',
      { token: daemon, session_id: sidWt, cwd: repo.worktree } as FixtureTokens,
      {
        tool_input: {
          file_path: path.join(repo.worktree, 'shared.js'),
          old_string: 'a',
          new_string: 'c',
        },
      },
  ...
  assert.ok(
    (whisperRes.json as HookAck | null)?.hookSpecificOutput,
  ...
  assert.equal(
    conflict.severity,
    'info',
    'cross-worktree collision of one repo should be severity=info',
  );
});
...
test('two sessions in the same worktree colliding is severity=warning', async (t) => {
  ...
  await postHook(
    daemon.baseUrl,
    'SessionStart',
    loadFixture('session-start', { session_id: sidA, cwd: repo.root }),
    { token: daemon },
  );
  await postHook(
    daemon.baseUrl,
    'SessionStart',
    loadFixture('session-start', { session_id: sidB, cwd: repo.root }),
    { token: daemon },
  );
  ...
  assert.ok(
    (res.json as HookAck | null)?.hookSpecificOutput,
...
test('edits through a symlinked directory alias collide with edits to the real path (BUG-127)', async (t) => {
  ...
  const sidA = randomUUID();
  const sidB = randomUUID();
  const callsignA = await postHook(
    daemon.baseUrl,
    'SessionStart',
    loadFixture('session-start', { session_id: sidA, cwd: repo.root }),
    { token: daemon },
  ).then((r) => (r.json as HookAck | null)?.callsign);
  await postHook(
    daemon.baseUrl,
    'SessionStart',
    loadFixture('session-start', { session_id: sidB, cwd: repo.root }),
  ...
  const firstTouch = await postHook(
    daemon.baseUrl,
    'PostToolUse',
    loadFixture(
      'post-tool-use-edit',
      ...
      {
        tool_input: {
          file_path: path.join(repo.root, 'real', 'x.js'),
          old_string: 'a',
          new_string: 'b',
        },
      },
  ...
  const secondTouch = await postHook(
    daemon.baseUrl,
    'PostToolUse',
    loadFixture(
      'post-tool-use-edit',
      ...
      {
        tool_input: {
          file_path: path.join(repo.root, 'alias', 'x.js'),
          old_string: 'b',
          new_string: 'c',
        },
      },
  ...
  const hso = (secondTouch.json as HookAck | null)?.hookSpecificOutput;
...
test('root files named with leading dots (`..config`, `...hidden`) still key repo-relatively, so cross-worktree edits collide', async (t) => {
  ...
  await postHook(
    ...
    loadFixture(
      ...
      {
        tool_input: {
          file_path: path.join(repo.root, '..config'),
          old_string: 'a',
          new_string: 'b',
        },
      },
  ...
  const res = await postHook(
    ...
    loadFixture(
      ...
      {
        tool_input: {
          file_path: path.join(repo.worktree, '..config'),
          old_string: 'a',
          new_string: 'c',
        },
      },
  ...
  assert.equal(
    conflict.severity,
    'info',
    'cross-worktree collision on ..config should be severity=info',
  );
  assert.equal(
    conflict.rel_path,
    '..config',
    'ledger should keep the repo-relative name, not an absolute fallback',
  );
});

test('unit: ledgerKey keeps dot-leading root files repo-relative; real parent escapes still fall back to absolute keys', (t) => {
...
test('a non-git cwd falls back to repo_id = cwd', async (t) => {
  ...
  await postHook(
    daemon.baseUrl,
    'SessionStart',
    loadFixture('session-start', { session_id: sid, cwd: plain.dir }),
    { token: daemon },
  );
...
test('a repo whose path ends in a space keeps its full path (no trim corruption)', (t) => {
  ...
  assert.equal(
    repo.worktree,
    expected,
    'worktree must keep its trailing space, not a trimmed nonexistent path',
  );
  assert.equal(repo.main_tree, expected, 'main tree must keep its trailing space');
  assert.equal(
    repo.repo_id,
    realpathSync(path.join(spaced, '.git')),
    'repo_id should be the canonicalized common git dir',
  );
  assert.equal(
    branchOf(spaced),
...
test('a --separate-git-dir repo catalogs the real checkout, not the metadata dir', async (t) => {
  ...
  await postHook(
    daemon.baseUrl,
    'SessionStart',
    loadFixture('session-start', { session_id: sid, cwd: repo.checkout }),
    { token: daemon },
  );
  ...
  assert.equal(
    card.repo_name,
    repo.repoName,
    'repo_name must be the checkout basename, not the metadata dir',
  );
  ...
  assert.equal(
    entry.root,
    repo.checkout,
    'catalog root must be the real checkout, not the metadata dir',
  );
});
