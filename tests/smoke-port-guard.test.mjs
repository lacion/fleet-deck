// tests/smoke-port-guard.test.mjs
//
...
function runSmoke(port, claudeBin) {
...
function collectOutput(child, timeoutMs, { onOutput } = {}) {
  return new Promise((resolve, reject) => {
    let output = '';
    const onChunk = chunk => {
      output += chunk;
    ...
    const killTimer = setTimeout(() => {
      child.kill('SIGKILL');
      reject(new Error(`run-smoke.sh did not exit within ${timeoutMs}ms; output so far:\n${output}`));
    }, timeoutMs);
...
function holdSilentListener(port, onData) {
...
test('BUG-188: smoke aborts on a bound non-HTTP listener that an HTTP health probe would miss', { timeout: scaleMs(120_000) }, async (t) => {
  ...
  let bytesAtAbort = null;
  let received = Buffer.alloc(0);
  const release = await holdSilentListener(port, chunk => { received = Buffer.concat([received, chunk]); });
  ...
  assert.equal(bytesAtAbort, 0, 'guard must probe occupancy without sending any bytes to the unknown listener');
});

test('BUG-188: smoke port guard passes a genuinely free port (no false positive)', { timeout: scaleMs(120_000) }, async (t) => {
  const port = await freePort();
  const claudeBin = claudeStubBin();
  t.after(() => rmSync(claudeBin, { recursive: true, force: true }));
  ...
  assert.equal(code, 1, `smoke should fail later on the missing daemon, not the guard; output:\n${output}`);
  assert.doesNotMatch(output, /ABORT: something is already listening on isolated port/);
  assert.match(output, /T\+0 session A launched/, `guard must let a free port through; output:\n${output}`);
});
