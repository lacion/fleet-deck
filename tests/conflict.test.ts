...
interface PostToolAck {
  hookSpecificOutput?: Whisper;
}
...
async function registerAndGetCallsign(daemon: DaemonHandle, sid: string, cwd: string) {
  ...
  return (res.json as SessionStartAck).callsign;
}
...
test('second session touching the same file gets a whisper naming the rival; rival gets mail', async (t) => {
  ...
  const firstTouch = await postHook(
    daemon.baseUrl,
    'PostToolUse',
    loadFixture('post-tool-use-edit', { token: daemon, session_id: sidA, cwd } as FixtureTokens, {
      tool_name: 'Edit',
  ...
  const secondTouch = await postHook(
    daemon.baseUrl,
    'PostToolUse',
    loadFixture('post-tool-use-edit', { token: daemon, session_id: sidB, cwd } as FixtureTokens, {
      tool_name: 'Edit',
      tool_input: { file_path: filePath, old_string: 'b', new_string: 'c' },
    }),
    { token: daemon },
  );
  const hso = (secondTouch.json as PostToolAck | null)?.hookSpecificOutput;
  ...
  assert.ok(
    ...
    'rival session A should have pending mail after the conflict',
  );

  // The conflict ledger should record the file with both sessions, severity
  // "warning" because both cwds are the same worktree.
  assert.ok(
    Array.isArray(state.conflicts) && state.conflicts.length >= 1,
    '/state.conflicts should record the collision',
  );
...
test('a session that already ended still counts as a rival within the window', async (t) => {
  ...
  await postHook(
    daemon.baseUrl,
    'PostToolUse',
    loadFixture('post-tool-use-edit', { token: daemon, session_id: sidC, cwd } as FixtureTokens, {
      tool_name: 'Write',
  ...
  assert.equal(
    ...
    'offline',
    'C should be tombstoned before D touches the file',
  );
  ...
  const res = await postHook(
    daemon.baseUrl,
    'PostToolUse',
    loadFixture('post-tool-use-edit', { token: daemon, session_id: sidD, cwd } as FixtureTokens, {
      tool_name: 'MultiEdit',
      tool_input: { file_path: filePath, edits: [{ old_string: 'x', new_string: 'y' }] },
    }),
    { token: daemon },
  );
  const hso = (res.json as PostToolAck | null)?.hookSpecificOutput;
  assert.ok(hso, 'D should still get a whisper about the now-offline C');
  assert.ok(
    hso.additionalContext.includes(callsignC),
