// tests/tmux-locale-separator.test.ts
//
...
import { FIELD_SEP } from '../src/daemon/spawn.ts';

function shippedSeparator(): string {
...
function serverUnder(
  t: TestContext,
  env: NodeJS.ProcessEnv,
): { socket: string; ask: (format: string) => string } {
  ...
  const ask = (format: string): string =>
    execFileSync('tmux', ['-L', socket, 'display-message', '-p', format], {
      encoding: 'utf8',
      env: { ...process.env, ...env },
    }).replace(/\n$/, '');
  return { socket, ask };
