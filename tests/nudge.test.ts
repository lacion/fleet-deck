...
// The /api/spawn response facet this test reads: the created session id and the
// scoped tmux session/window names the daemon believes it launched.
interface SpawnResponse {
  session_id: string;
  tmux: { session: string; window: string };
}

// The /state subset this test reads. `ticker` is modelled permissively (msg OR
// a legacy text field) so the historical defensive read below stays honest.
interface NudgeState {
  sessions: SessionEntry[];
  ticker?: { msg?: string; text?: string }[];
}

// execFileSync throws an Error augmented with a captured `stderr`; this is the
// slice the new-session failure message reads.
interface TmuxSpawnError {
  message: string;
