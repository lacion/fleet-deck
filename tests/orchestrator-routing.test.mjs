...
async function makeNeedsyou(daemon, cwd) {
  const { sid, callsign } = await makeQueued(daemon, cwd);
  await postHook(daemon.baseUrl, 'Notification', loadFixture('notification', { session_id: sid, cwd }), { token: daemon });
  return { sid, callsign };
}
...
test('tie on col: fewest undelivered mail wins, overriding recency', async (t) => {
  ...
  await postJson(`${daemon.baseUrl}/mail`, { to: newer.sid, from: 'operator', text: 'unrelated noise' }, { token: daemon });
