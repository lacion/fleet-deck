...
//
// Pure unit tests for src/daemon/tickets.ts — the Jira-key grammar shared
...
import { TICKET_RE, ticketFromBranch, normalizeTicket } from '../src/daemon/tickets.ts';
...
test('ticketFromBranch: non-string / empty inputs return null (never throw)', () => {
  ...
  assert.equal(ticketFromBranch(undefined as unknown as string), null);
  assert.equal(ticketFromBranch(''), null);
  assert.equal(ticketFromBranch(42 as unknown as string), null);
  assert.equal(ticketFromBranch({} as unknown as string), null);
});
...
test('normalizeTicket: rejects anything that is not exactly one key', () => {
  ...
  assert.equal(normalizeTicket(null as unknown as string), null);
