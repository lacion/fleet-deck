...
// execFile returns a ChildProcess; the poll loop hangs the parsed record and
// the observed process-group id off it so the completion callback can report
// them together.
interface LaunchedChild extends ChildProcess {
  record?: LaunchRecord;
  pgid?: number;
}
...
test('smoke launches its workers through the portable Node launcher, never GNU timeout/setsid', () => {
  ...
  assert.ok(
    usesTimeoutLauncher,
    'run-smoke.sh must launch workers via demo/run-with-timeout.mjs (macOS ships neither GNU timeout nor setsid)',
  );
  assert.equal(
    /^\s*setsid\s/m.test(source),
    false,
    'run-smoke.sh must not invoke setsid (util-linux, Linux-only)',
  );
  assert.equal(
    /^\s*timeout\s/m.test(source),
    false,
    'run-smoke.sh must not invoke GNU timeout as a command',
  );
  // And the abort gate that rejected machines without them must be gone.
  assert.equal(
    /requires (timeout|setsid) on PATH/.test(source),
    false,
    'smoke preflight must not hard-require Linux-only utilities',
  );
});
...
test('launcher runs the command in its own process group (the setsid half)', async (t) => {
  ...
  const result = await new Promise<LaunchResult>((resolve) => {
    ...
    const poll = setInterval(() => {
      ...
      clearInterval(poll);
      const rec = JSON.parse(readFileSync(out, 'utf8')) as LaunchRecord;
      launched.record = rec;
      launched.pgid = Number(
        execFileSync('ps', ['-o', 'pgid=', '-p', String(rec.pid)], { encoding: 'utf8' }).trim(),
      );
    }, 25);
  });
  ...
  assert.equal(
    result.code,
    0,
    `launcher should relay the child's exit status, stderr: ${result.stderr}`,
  );
  ...
  assert.equal(
    result.pgid,
    result.record.pid,
    'child must lead its own process group so the smoke can signal the whole tree (setsid semantics)',
  );
});
...
test('launcher exits 124 and kills a command that outlives its deadline (GNU timeout semantics)', () => {
  assert.throws(
    () =>
      execFileSync(
        process.execPath,
        [LAUNCHER, '1', process.execPath, '-e', 'setInterval(() => {}, 1000)'],
        { stdio: 'pipe' },
      ),
    (error: unknown) => (error as { status?: number }).status === 124,
...
test('launcher forwards SIGTERM into the child process group so stop_worker cannot orphan a worker', async (t) => {
  ...
  assert.ok(childPid !== undefined, 'launcher child must have spawned with a pid');
  ...
  assert.ok(
    victimPid !== null && victimPid > 0,
    'timed out waiting for the launched child to start',
  );
  ...
  assert.equal(
    pidAlive(victimPid),
    false,
    'SIGTERM to the launcher must reach the detached worker process group',
  );
});
