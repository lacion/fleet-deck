...
async function arm(daemon: DaemonHandle): Promise<string> {
  ...
  const body = res.json as ArmResponse;
  assert.ok(typeof body.arm_token === 'string' && body.arm_token.length >= 16);
  return body.arm_token;
}
...
test('unsupervised spawn without an arm token → 403, nothing created', async (t) => {
  ...
  const daemon = await startDaemon({
    env: { FLEETDECK_SPAWN_CMD: SPAWN_CMD_FIXTURE, FLEETDECK_TEST_SPAWN_RECORD: recordFile },
  });
  t.after(async () => {
    await daemon.stop();
  ...
  for (const body of [
    ...
    assert.match((res.json as SpawnRefusal | null)?.reason ?? '', /arm/i);
  }

  // A stale/fabricated arm token is equally refused.
  const forged = await postJson(`${daemon.baseUrl}/api/spawn`, {
    cwd,
    dangerously_skip_permissions: true,
    arm_token: 'not-a-real-token',
  });
...
test('an armed unsupervised spawn works; the token is single-use', async (t) => {
  ...
  const third = await postJson(`${daemon.baseUrl}/api/spawn`, {
    cwd: cwd2,
...
test('a cased permission_mode variant cannot sneak past the gate', async (t) => {
  ...
  const ok = await postJson(`${daemon.baseUrl}/api/spawn`, {
    cwd,
    permission_mode: 'BypassPermissions',
    arm_token: armToken,
  });
  ...
  const argv = (records[0] as SpawnSpecRecord | undefined)?.parsed?.argv ?? [];
...
test('unknown permission_mode values 400', async (t) => {
  ...
  assert.match((res.json as SpawnRefusal | null)?.reason ?? '', /unknown permission_mode/);
});
...
test('revive of an unsupervised lineage requires a fresh arm', async (t) => {
  ...
  assert.match((unarmed.json as SpawnRefusal | null)?.reason ?? '', /arm/i);
  ...
  const token2 = await arm(daemon);
  const armed = await postJson(`${daemon.baseUrl}/api/spawn/${spawnId}/revive`, {
    arm_token: token2,
  });
  assert.notEqual(
    armed.status,
    403,
    `armed revive must pass the gate (got ${armed.status}: ${JSON.stringify(armed.json)})`,
  );
});
...
test('supervised spawns need no arm token', async (t) => {
  ...
  const res = await postJson(`${daemon.baseUrl}/api/spawn`, {
    cwd,
    prompt: 'ordinary supervised work',
  });
...
test('adopt with dangerously_skip_permissions:true also requires the arm', async (t) => {
  ...
  const unarmed = await postJson(`${daemon.baseUrl}/api/sessions/${sid}/adopt`, {
    dangerously_skip_permissions: true,
  });
