// tests/accept-hook-quoting.test.ts
//
...
function heredocBlock(file: string): string {
  const lines = readFileSync(path.join(REPO_ROOT, file), 'utf8').split('\n');
  const start = lines.findIndex((l) =>
    l.includes('cat > "$PROJECT_DIR/.claude/settings.json" <<EOF'),
  );
...
for (const [script] of HEREDOCS) {
  test(`${script}: hook commands survive a checkout path with spaces (BUG-092)`, (t: TestContext) => {
    ...
    const settings = JSON.parse(readFileSync(outFile, 'utf8')) as Settings;
