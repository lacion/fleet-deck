// tests/hook-missing-session-id.test.mjs
//
...
test('authenticated hooks without a session_id fail open and never mint the shared unknown card', async (t) => {
  ...
  const start = await postHook(daemon.baseUrl, 'SessionStart', { hook_event_name: 'SessionStart', cwd }, { token: daemon });
  assert.equal(start.status, 200, 'ID-less SessionStart still fails open with 200');
  assert.ok(!start.json?.ok, 'no brief was composed — the handler was not dispatched');
  ...
  await postHook(daemon.baseUrl, 'SessionStart', { hook_event_name: 'SessionStart', session_id: '', cwd }, { token: daemon });
  await postHook(daemon.baseUrl, 'SessionStart', { hook_event_name: 'SessionStart', session_id: 42, cwd }, { token: daemon });
  const prompt = await postHook(daemon.baseUrl, 'UserPromptSubmit', { hook_event_name: 'UserPromptSubmit', cwd, prompt: 'hello' }, { token: daemon });
  assert.equal(prompt.status, 200, 'ID-less UserPromptSubmit still fails open with 200');
  const stop = await postHook(daemon.baseUrl, 'Stop', { hook_event_name: 'Stop', cwd }, { token: daemon });
  assert.equal(stop.status, 200, 'ID-less Stop still fails open with 200');

  let state = (await getJson(`${daemon.baseUrl}/state`)).json;
  assert.ok(!state.sessions.find(s => s.session_id === 'unknown'), "no phantom 'unknown' card on the board");
  ...
  await postHook(daemon.baseUrl, 'FileChanged', { hook_event_name: 'FileChanged', cwd, path: 'x.mjs' }, { token: daemon });
  await postHook(daemon.baseUrl, 'SomethingNew', { hook_event_name: 'SomethingNew', cwd }, { token: daemon });
  state = (await getJson(`${daemon.baseUrl}/state`)).json;
  assert.ok(!state.sessions.find(s => s.session_id === 'unknown'), "telemetry-only hooks mint no 'unknown' card either");
...
test('a hook WITH a session_id still registers normally after the ID-less ones were dropped', async (t) => {
  ...
  await postHook(daemon.baseUrl, 'UserPromptSubmit', { hook_event_name: 'UserPromptSubmit', cwd, prompt: 'stray' }, { token: daemon });

  const sid = randomUUID();
  const res = await postHook(daemon.baseUrl, 'SessionStart', loadFixture('session-start', { session_id: sid, cwd }), { token: daemon });
  assert.ok(res.json?.ok, 'a well-formed SessionStart still gets its brief');
  const state = (await getJson(`${daemon.baseUrl}/state`)).json;
  assert.deepEqual(state.sessions.map(s => s.session_id), [sid], 'exactly the real session is on the board');
});
