...
//      elicitation accept/decline) + hold-expiry -> {} + status 'expired'
...
import { getState, questionsFor, getSession } from './helpers/state.ts';
...
interface PermissionHoldResponse {
  hookSpecificOutput?: {
...
test('F3a: PermissionRequest holds open; board answer {behavior:"allow"} resolves the held response and the question stops being pending', async (t) => {
  ...
  const callsign = (registration.json as RegisterAck | null)?.callsign;
  ...
  assert.ok(q.id, 'question needs an id to answer it by');
  ...
  assert.ok(
    q2,
    'answered question should still be present in /state (with a terminal status), not vanish',
  );
...
test('F3a: PermissionRequest board answer {behavior:"deny"} resolves the held response with behavior=deny', async (t) => {
  ...
  const heldRes = await held;
  assert.equal(
    (heldRes.json as PermissionHoldResponse | null)?.hookSpecificOutput?.hookEventName,
    'PermissionRequest',
  );
  assert.equal(
    (heldRes.json as PermissionHoldResponse | null)?.hookSpecificOutput?.decision?.behavior,
    'deny',
    'deny decision should carry behavior=deny',
  );
});
...
test('F3b: Elicitation accept resolves with the documented hookSpecificOutput schema', async (t) => {
  const holdMs = 1500;
  const { daemon, cwd, sid, registration } = await startRegisteredDaemon(t, {
    daemon: { env: { FLEETDECK_HOLD_MS: String(holdMs) } },
  });
  ...
  assert.deepEqual(
    ...
    'accept must be event-scoped; a top-level action is ignored by Claude Code',
  );
});

test('F3b: Elicitation decline resolves with no stray top-level fields or content', async (t) => {
  const holdMs = 1500;
  const { daemon, cwd, sid } = await startRegisteredDaemon(t, {
    daemon: { env: { FLEETDECK_HOLD_MS: String(holdMs) } },
  });
  ...
  const q = await waitUntil(
    ...
    { label: 'declinable elicitation question to appear in /state' },
  );

  const answer = await postJson(`${daemon.baseUrl}/api/questions/${q.id}/answer`, {
    action: 'decline',
  });
...
test('F3d: Stop trailing-question freeform detection creates a needsyou card, and the board answer is delivered as mail', async (t) => {
  ...
  assert.equal(
    card.col,
    'needsyou',
    'a trailing question at Stop should move the card to needsyou',
  );
  ...
  assert.equal(
    ansRes.status,
    200,
    'POST /api/questions/:id/answer should 200 for a freeform answer',
  );
  ...
  const upCtx = (upRes.json as UpsResponse | null)?.hookSpecificOutput?.additionalContext ?? '';
  let delivered = upCtx.includes('[FLEETDECK ANSWER]') && upCtx.includes('argon2');
  ...
  if (!delivered) {
    ...
    const reason = (stopRes2.json as StopBlockResponse | null)?.reason ?? '';
    delivered =
      (stopRes2.json as StopBlockResponse | null)?.decision === 'block' &&
      reason.includes('[FLEETDECK ANSWER]') &&
      reason.includes('argon2');
    deliveryChannel = delivered ? 'Stop block' : null;
  }

  assert.ok(
    delivered,
    'answering a freeform question should enqueue mail that reaches the session via UserPromptSubmit additionalContext or a Stop block, carrying "[FLEETDECK ANSWER]" and the answer text',
  );
  t.diagnostic(`freeform answer delivered via: ${String(deliveryChannel)}`);
});
...
test('F3d: Stop with last_assistant_message present on the live payload detects the payload question, not the transcript tail', async (t) => {
  ...
  assert.equal(
    qs.length,
    1,
    'exactly one freeform question should be created from the live Stop payload (the transcript tail is a non-question, so a fallback to it would create none)',
  );
  const q = qs[0];
  assert.ok(q, 'the single freeform question should be present');
  assert.equal(q.status, 'pending');
  assert.equal(
    (q.payload as QuestionPayload | null)?.text,
...
test('a Stop that returns a mail block does not run freeform question detection for that request', async (t) => {
  ...
  assert.equal(
    (stopRes.json as StopBlockResponse | null)?.decision,
    'block',
    'pending mail should still take priority and block, same as Phase 1/2 behavior',
  );
  ...
  assert.equal(
    qs.length,
    0,
    'freeform question detection must be skipped when Stop returns a mail block',
  );
});
...
test('Notification ingest stores notification_type', async (t) => {
  ...
  assert.equal(
    card.col,
    'needsyou',
    'Notification is the F3e safety net: col should move to needsyou',
  );

  const carriesType = JSON.stringify(card).includes('permission_prompt');
  assert.ok(
    carriesType,
    `notification_type should be stored/exposed somewhere on the session card; got ${JSON.stringify(card)}`,
  );
});
...
test('subsequent activity (UserPromptSubmit) returns col to working and expires a pending permission question ahead of its own hold timeout', async (t) => {
  ...
  assert.equal(
    card.col,
    'working',
    'subsequent activity (UserPromptSubmit) should return the card to working',
  );
  ...
  assert.equal(
    q2.status,
    'expired',
    'subsequent activity should expire the pending permission question',
  );
  ...
  assert.ok(
    elapsed < holdMs,
    `activity-triggered expiry should resolve the hold promptly, not wait out the full ${holdMs}ms hold (took ${elapsed}ms after the triggering activity)`,
  );
...
test('BUG-102: a FAILED tool call (PostToolUseFailure) retires its correlated permission hold promptly — the board must not show needsyou for an operation that already finished unsuccessfully', async (t) => {
  ...
  assert.deepEqual(
    failRes.json,
    {},
    'PostToolUseFailure should respond {} like every no-conflict tool hook',
  );
  ...
  assert.ok(
    elapsed < holdMs,
    `the failed tool call should retire its hold promptly, not wait out the full ${holdMs}ms hold (took ${elapsed}ms)`,
  );
  ...
  assert.equal(
    card.col,
    'working',
    'a failed Bash call is still activity — the card must not stay needsyou',
  );
...
test('SessionEnd expires all pending permission/elicitation questions for the session', async (t) => {
  ...
  assert.ok(
    elapsed < holdMs,
    `both holds should resolve promptly on SessionEnd, not wait out the ${holdMs}ms hold (took ${elapsed}ms)`,
  );
...
test('concurrent holds per session are capped at 4; the 5th arrival expires the oldest', async (t) => {
  ...
  const promises: Promise<JsonResponse>[] = [];
  let firstFourPending: QuestionEntry[] | null = null;
  ...
  for (let i = 0; i < kinds.length; i++) {
    ...
    const p = postHook(daemon.baseUrl, event, loadFixture(fixtureName, { session_id: sid, cwd }), {
      token: daemon,
      timeout: holdMs + 8000,
      boardClient: true,
    });
  ...
  assert.ok(
    firstFourPending,
    'sanity: should have observed exactly 4 pending questions before the 5th arrival',
  );
  const oldestId = firstFourPending[0]?.id;
  assert.ok(oldestId, 'sanity: the oldest of the four pending holds should carry an id');

  const first = promises[0];
  assert.ok(first, 'sanity: the first (oldest) hold promise should exist');
  const oldestSettled = await Promise.race<JsonResponse | 'TIMEOUT'>([
    first,
  ...
  assert.notEqual(
    oldestSettled,
    'TIMEOUT',
    'the oldest hold should be expired promptly once a 5th concurrent hold arrives for the same session (cap=4)',
  );
  assert.deepEqual(
    (oldestSettled as JsonResponse).json,
    {},
  ...
  assert.equal(
    pendingNow.length,
    4,
    'exactly 4 questions should remain pending for the session (cap=4) after the 5th arrival',
  );

  const oldestRow = state.questions.find((x) => x.id === oldestId);
  assert.ok(oldestRow, 'the oldest question should still be present in /state');
  assert.equal(
    oldestRow.status,
    'expired',
    'the oldest (1st) hold should now be marked expired, having been evicted by the cap',
  );
...
test('malformed /hook/PermissionRequest body still answers 200 and the daemon stays healthy', async (t) => {
  ...
  if (text) {
    assert.doesNotThrow(() => {
      JSON.parse(text);
    }, 'response body, if any, must be valid JSON so the hook consumer never chokes');
  }
...
test('F3d: a freeform card clears when the session moves on (answered in the terminal)', async (t) => {
  ...
  assert.notEqual(
    after?.status,
    'pending',
    'the card must not still be pending once the session moved on',
  );
});
...
test('a stale needs-you card can be dismissed without telling the session anything', async (t) => {
  ...
  const mail =
    ((await getJson(`${daemon.baseUrl}/mail?session=${sid}`)).json as MailListResponse).mail ?? [];
