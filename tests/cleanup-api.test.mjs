...
test('Clear wipes everything that is not alive: conflicts, the rail, the feed', async (t) => {
  ...
  await postHook(daemon.baseUrl, 'SessionStart', { session_id: alive, cwd: process.cwd(), source: 'startup' }, { token: daemon.token });
...
test('BUG-145: Clear and dismiss fail loud when tmux is unreachable, with a retry path', async (t) => {
  ...
  await waitUntil(async () => {
    try {
      return execFileSync('tmux', ['-L', socket, 'display-message', '-p', '-t', winName, '#{pane_dead}'], { encoding: 'utf8' }).trim() === '1' || null;
    } catch { return null; }
  ...
  assert.equal(blocked.status, 409, 'an UNKNOWN tmux listing is a non-2xx incomplete, not a success');
  assert.equal(blocked.json.ok, false);
  assert.match(blocked.json.reason, /listing unavailable/);
  let state = (await getJson(`${daemon.baseUrl}/state`)).json;
  assert.ok(state.sessions.some(s => s.session_id === sid), 'nothing was archived — Clear is fully retryable');
  ...
  assert.equal(dis.json.ok, false);
  assert.equal(dis.json.retry, true, 'the failure carries its retry path');
  assert.equal(dis.json.archived, 1, 'the DB story did land');
  assert.match(dis.json.reason, /listing unavailable/);
  ...
  assert.equal(retried.json.retry, true);
  const unknown = await postJson(`${daemon.baseUrl}/api/sessions/${randomUUID()}/dismiss/retry`, {});
  assert.equal(unknown.status, 404);

  state = (await getJson(`${daemon.baseUrl}/state`)).json;
  assert.equal(state.sessions.some(s => s.session_id === sid), false, 'the card is archived by the partial dismiss');
});
