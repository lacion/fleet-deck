...
test('one block per turn: Stop blocks once on pending mail, then passes, then blocks again after a new turn + new mail', async (t) => {
  ...
  const mailRes = await postJson(`${daemon.baseUrl}/mail`, { to: sid, from: 'tester', text: 'please wrap up soon' }, { token: daemon });
  assert.equal(mailRes.status, 200, 'POST /mail should 200');

  // First Stop: mail is pending -> block, exactly once.
  const stop1 = await postHook(daemon.baseUrl, 'Stop', loadFixture('stop', { session_id: sid, cwd }), { token: daemon });
  ...
  const stop2 = await postHook(daemon.baseUrl, 'Stop', loadFixture('stop', { session_id: sid, cwd }), { token: daemon });
  ...
  await postJson(`${daemon.baseUrl}/mail`, { to: sid, from: 'tester', text: 'second message' }, { token: daemon });

  // Stop should be able to block again now that a new turn has started.
  const stop3 = await postHook(daemon.baseUrl, 'Stop', loadFixture('stop', { session_id: sid, cwd }), { token: daemon });
...
test('UserPromptSubmit drains pending mail as additionalContext', async (t) => {
  ...
  const sid = randomUUID();
  await postHook(daemon.baseUrl, 'SessionStart', loadFixture('session-start', { session_id: sid, cwd }), { token: daemon });
  await postJson(`${daemon.baseUrl}/mail`, { to: sid, from: 'ops', text: 'ping from the board' }, { token: daemon });

  const res = await postHook(daemon.baseUrl, 'UserPromptSubmit', loadFixture('user-prompt-submit', { session_id: sid, cwd }), { token: daemon });
...
test('GET /mail?session=<sid> drains the mailbox directly', async (t) => {
  ...
  const sid = randomUUID();
  await postHook(daemon.baseUrl, 'SessionStart', loadFixture('session-start', { session_id: sid, cwd }), { token: daemon });
  await postJson(`${daemon.baseUrl}/mail`, { to: sid, from: 'skill', text: 'direct drain check' }, { token: daemon });
...
test('POST /mail targeting: session/callsign, "all", "repo:<name>"', async (t) => {
  ...
  const regX = await postHook(daemon.baseUrl, 'SessionStart', loadFixture('session-start', { session_id: sidX, cwd: repoA.root }), { token: daemon });
  await postHook(daemon.baseUrl, 'SessionStart', loadFixture('session-start', { session_id: sidY, cwd: repoA.root }), { token: daemon });
  await postHook(daemon.baseUrl, 'SessionStart', loadFixture('session-start', { session_id: sidZ, cwd: repoB.root }), { token: daemon });
  ...
  await postJson(`${daemon.baseUrl}/mail`, { to: callsignX, from: 'operator', text: 'to X only' }, { token: daemon });
  ...
  await postJson(`${daemon.baseUrl}/mail`, { to: 'all', from: 'operator', text: 'to everyone' }, { token: daemon });
  ...
  await postJson(`${daemon.baseUrl}/mail`, { to: repoTarget, from: 'operator', text: 'to repo A' }, { token: daemon });
  state = (await getJson(`${daemon.baseUrl}/state`)).json;
