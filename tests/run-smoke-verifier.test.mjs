// tests/run-smoke-verifier.test.mjs
//
...
function extractVerifierInvocation(scriptText) {
  const m = scriptText.match(/node --input-type=module -e "\n[\s\S]*?\n"[^\n]*\n/);
...
test('run-smoke verifier survives an apostrophe in DEMO_LOGS (BUG-190)', (t) => {
  ...
  writeFileSync(path.join(demoLogs, 'final-state.json'), JSON.stringify({
    sessions: [
  ...
  for (const f of ['worker-a.json', 'worker-b.json']) {
    writeFileSync(path.join(demoLogs, f), JSON.stringify({
      is_error: false, subtype: 'success', result: 'done\nFLEET-NOTE: util.js',
    }));
  }
