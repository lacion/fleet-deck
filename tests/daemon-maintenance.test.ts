...
// Permissive read-views over the snapshot: the real snapshot card is a
// conditional-spread union (spawn present only on spawn-backed cards), so a
// direct `card.spawn` access does not typecheck. These views describe exactly
// what the maintenance tests read and are bridged with a single `as unknown as`.
interface SpawnView {
  spawn_id: string;
  status: string;
  stalled: boolean;
...
interface SnapshotView {
  ...
  ticker: TickerView[];
  mail_meta: Record<string, { queued: number }>;
}

const snapshotView = (core: Core): SnapshotView => core.snapshot() as unknown as SnapshotView;
...
function fakeTmux(port = 4711): FakeTmux {
  ...
  const adapter: FakeTmuxAdapter = {
    ...
    capturePane: (target) => {
      ...
      return Promise.resolve(state.captureText);
    },
...
function memoryCore(
  t: TestContext,
  { env = {}, tmux = fakeTmux(), home = '/daemon-home' }: MemoryCoreOptions = {},
): {
  db: Db;
  core: Core;
  state: FakeTmuxState;
...
test('BUG-150: snapshot spawns/callsigns come only from visible sessions + bounded conflicts', (t) => {
  ...
  const now = Date.now();
  db.prepare(
    'INSERT INTO sessions (session_id, callsign, col, note, events, started_at, last_seen, blocked_this_turn, archived_at) VALUES (?, ?, ?, ?, 0, ?, ?, 0, ?)',
  ...
  db.prepare(
    "INSERT INTO spawns (spawn_id, session_id, callsign, tmux_window, cwd, requested_at, status) VALUES ('sp-old', 'live-1', 'wren-live', 'fd4711-wren-live', '/x', ?, 'gone')",
  ).run(now - 5000);
  db.prepare(
    "INSERT INTO spawns (spawn_id, session_id, callsign, tmux_window, cwd, requested_at, status) VALUES ('sp-new', 'live-1', 'wren-live', 'fd4711-wren-live', '/x', ?, 'live')",
  ).run(now - 1000);
  for (let i = 0; i < 100; i++) {
    db.prepare(
      "INSERT INTO spawns (spawn_id, session_id, callsign, tmux_window, cwd, requested_at, status) VALUES (?, 'arch-1', 'comet-old', ?, '/y', ?, 'gone')",
  ...
  assert.equal(
    snap.sessions.some((s) => s.session_id === 'arch-1'),
    false,
  );
...
test('stall diagnostic excerpt is line/byte bounded and redacts shape + exact secrets', () => {
  ...
  const screen =
    Array.from({ length: 30 }, (_, i) => `line-${i} ${'🚀'.repeat(200)}`).join('\n') +
...
test('stall diagnostic excerpt also scrubs URL userinfo, which no shape rule can see', () => {
  ...
  assert.ok(out !== null);
  assert.equal(
    out.includes('glpat-AbCdEf1234567890'),
    false,
    'the PAT must not survive the pane excerpt',
  );
...
test('spawn argv is deterministic and registration watchdog stalls once, then a late hook revives it', async (t) => {
  ...
  let card = snapshotView(core).sessions.find((s) => s.session_id === out.body.session_id);
  assert.ok(card);
  ...
  const stallDetail = card.spawn.stall_detail;
  assert.ok(stallDetail !== null);
  assert.match(stallDetail, /Folder trust required/);
  assert.match(
    stallDetail,
    /\[redacted\]/,
    'known credential shapes are scrubbed from the broadcast diagnostic',
  );
  assert.doesNotMatch(stallDetail, /sk-ant-/);
  assert.ok(
    state.calls.some(([kind]) => kind === 'capturePane'),
    'watchdog captures the pane once when it stalls',
  );
  assert.ok(
    snapshotView(core).ticker.some((x) => /never phoned home.*diagnostics captured/.test(x.msg)),
  );
  assert.equal(
    db.prepare<CountRow>("SELECT COUNT(*) AS n FROM events WHERE hook_event = 'SpawnStalled'").get()
      ?.n,
    1,
  );
  ...
  card = snapshotView(core).sessions.find((s) => s.session_id === out.body.session_id);
  assert.ok(card);
...
test('revive reuses the env wrapper, kills a dead remnant, inserts a new row, and the resume hook marks it live', async (t) => {
  ...
  const original = (await core.spawn({
    cwd,
  ...
  const out = (await core.revive(oldId, { arm_token: core.armUnsupervised() })) as ReviveResult;
  assert.equal(out.status, 200);
  ...
  const rows = db
    .prepare<StatusRow>('SELECT * FROM spawns WHERE session_id = ? ORDER BY requested_at, rowid')
    .all(sid);
  assert.equal(rows.length, 2);
  assert.equal(rows[0]?.status, 'gone');
  assert.equal(rows[1]?.status, 'spawning');
  let session = db.prepare<SessionRow>('SELECT * FROM sessions WHERE session_id = ?').get(sid);
  assert.ok(session);
  ...
  assert.equal(
    db.prepare<StatusRow>('SELECT status FROM spawns WHERE spawn_id = ?').get(out.body.spawn_id)
      ?.status,
    'live',
  );
  assert.equal(
  ...
  session = db.prepare<SessionRow>('SELECT * FROM sessions WHERE session_id = ?').get(sid);
  assert.ok(session);
...
test('owned-pane mail honors watcher priority and unclaims all rows when paste fails', async (t) => {
  ...
  let posted = (await core.postMail({
    to: sid,
    from: 'ops',
    text: 'line one\nline two',
  })) as PostMailResult;
  ...
  assert.ok(
    db.prepare<DeliveredRow>('SELECT delivered_at FROM mail ORDER BY id LIMIT 1').get()
      ?.delivered_at,
  );
  assert.equal(snapshotView(core).mail_meta[sid]?.queued, 0);

  state.calls.length = 0;
  const unregister = core.addWatchWaiter(sid, noop);
  // 'human' is a reserved sender (0.16.0) — postMail refuses it even in-process.
  posted = (await core.postMail({
    to: sid,
    from: 'operator',
    text: 'watcher first',
  })) as PostMailResult;
  ...
  assert.equal(await core.tryOwnedPaneDelivery(sid), false);
  const failed = db.prepare<DeliveredRow>("SELECT * FROM mail WHERE text = 'retry me'").get();
...
test('owned-pane mail: failed Enter after successful paste does NOT requeue (BUG-033)', async (t) => {
  ...
  state.enterOk = false;
  const posted = (await core.postMail({
    to: sid,
    from: 'ops',
    text: 'paste ok, enter fails',
  })) as PostMailResult;
  assert.equal(posted.targets[0]?.route, 'pane');
  assert.equal(
    await core.tryOwnedPaneDelivery(sid),
    false,
    'Enter failure still reports non-delivery',
  );
  assert.deepEqual(state.calls, [
  ...
  const row = db.prepare<DeliveredRow>('SELECT delivered_at FROM mail ORDER BY id LIMIT 1').get();
  assert.ok(row?.delivered_at, 'rows pasted into the pane must NOT be requeued when Enter fails');
  assert.equal(snapshotView(core).mail_meta[sid]?.queued, 0, 'no pending mail remains to re-paste');
...
test('tmux input/capture helpers use isolated-socket argv without shell interpolation', async (t) => {
  ...
  Reflect.deleteProperty(process.env, 'FLEETDECK_HOME');
  t.after(() => {
    if (previousHome == null) Reflect.deleteProperty(process.env, 'FLEETDECK_HOME');
...
test('retention presumes dead, archives, expires mail, hides archived rows, and resurrects late hooks', (t) => {
  ...
  db.prepare(
    ...
    VALUES ('old-offline', 'ops', 'old', ?, NULL, NULL)`,
  ).run(now);
  db.prepare(
    `INSERT INTO spawns
    (spawn_id, session_id, callsign, tmux_session, tmux_window, requested_at, status)
    VALUES ('sp-old', 'old-offline', 'old-1', 'fleetdeck-4711', 'fd4711-old-1', ?, 'live')`,
  ...
  assert.ok(
    db
      .prepare<ArchivedRow>("SELECT archived_at FROM sessions WHERE session_id = 'old-offline'")
      .get()?.archived_at,
  );
  assert.ok(
    db.prepare<ExpiredRow>("SELECT expired_at FROM mail WHERE to_session = 'old-offline'").get()
      ?.expired_at,
  );
  assert.equal(
    db.prepare<StatusRow>("SELECT status FROM spawns WHERE spawn_id = 'sp-old'").get()?.status,
    'gone',
  );
  assert.equal(
    snapshotView(core).sessions.some((s) => s.session_id === 'old-offline'),
    false,
  );
  ...
  const revived = db
    .prepare<SessionRow>("SELECT * FROM sessions WHERE session_id = 'silent'")
    .get();
  assert.ok(revived);
...
test('BUG-144: a short ledger horizon never prunes touches the conflict radar still considers', async (t) => {
  ...
  db.prepare(
    ...
    VALUES ('repo', 'a.js', '/repo/a.js', 's1', NULL, ?)`,
  ).run(now - 5 * 60_000);
  db.prepare(`INSERT INTO commands (at, text, parsed_json) VALUES (?, '/rc x', NULL)`).run(
    now - 90_000,
  );
  ...
  assert.equal(db.prepare<CountRow>('SELECT COUNT(*) AS n FROM file_touches').get()?.n, 1);
  // The other ledgers still age on the configured (short) horizon.
  assert.equal(db.prepare<CountRow>('SELECT COUNT(*) AS n FROM commands').get()?.n, 0);
});
...
test('cleanup archives offline rows, expires mail, kills eligible dead panes, and only lists worktrees', async (t) => {
  ...
  db.prepare(
    ...
    (session_id, callsign, col, note, events, started_at, last_seen, ended_at, source)
    VALUES ('offline', 'off-1', 'offline', 'ended', 0, ?, ?, ?, 'hooks')`,
  ).run(now, now, now);
  db.prepare(
    `INSERT INTO mail (to_session, from_id, text, at, delivered_at, expired_at)
    VALUES ('offline', 'ops', 'pending', ?, NULL, NULL)`,
  ).run(now);
  ...
  t.after(() => {
    rmSync(wt, { recursive: true, force: true, maxRetries: 5, retryDelay: 50 });
  });
  db.prepare(
    `INSERT INTO spawns
    (spawn_id, session_id, callsign, tmux_session, tmux_window, worktree_path, requested_at, status)
    VALUES ('sp-off', 'offline', 'off-1', 'fleetdeck-4711', 'fd4711-off-1', ?, ?, 'pane-dead')`,
  ).run(wt, now);
  db.prepare(
    `INSERT INTO spawns
    (spawn_id, session_id, callsign, tmux_session, tmux_window, worktree_path, requested_at, status)
    VALUES ('sp-gone', 'offline', 'off-1', 'fleetdeck-4711', 'fd4711-off-gone', '/tmp/fd-wt-off-already-removed', ?, 'gone')`,
  ).run(now);
  state.windows.push({
  ...
  assert.equal(
    snapshotView(core).sessions.some((s) => s.session_id === 'offline'),
    false,
  );
});
...
test('cleanup does not kill a same-name replacement pane a revive stood up mid-kill (BUG-046)', async (t) => {
  ...
  db.prepare(
    `INSERT INTO sessions
    (session_id, callsign, col, note, events, started_at, last_seen, ended_at, source)
    VALUES ('offline-race', 'off-race-1', 'offline', 'ended', 0, ?, ?, ?, 'hooks')`,
  ).run(now, now, now);
  db.prepare(
    `INSERT INTO spawns
    (spawn_id, session_id, callsign, tmux_session, tmux_window, requested_at, status)
    VALUES ('sp-race-old', 'offline-race', 'off-race-1', 'fleetdeck-4711', 'fd4711-off-race-1', ?, 'pane-dead')`,
  ).run(now);
  state.windows.push({
    session: 'fleetdeck-4711',
  ...
  tmux.adapter.killWindowVerified = (name, opts) => {
    ...
    db.prepare(
      `INSERT INTO spawns
      (spawn_id, session_id, callsign, tmux_session, tmux_window, requested_at, status)
      VALUES ('sp-race-new', 'offline-race', 'off-race-1', 'fleetdeck-4711', 'fd4711-off-race-1', ?, 'live')`,
    ).run(now + 1);
    state.windows = [
      {
        session: 'fleetdeck-4711',
    ...
    if (opts?.expectWindowId !== undefined) {
      ...
      let expectHolds = true;
      if (opts.expect !== undefined) expectHolds = opts.expect();
      if (win?.window_id !== opts.expectWindowId || !expectHolds) {
        return Promise.resolve({ ok: false, stale: true, error: 'stale window owner' });
      }
    }
    state.killed.push(name);
    return Promise.resolve({ ok: true, window_id: '@88' });
  };
  ...
  assert.equal(
    db.prepare<StatusRow>("SELECT status FROM spawns WHERE spawn_id = 'sp-race-new'").get()?.status,
    'live',
...
test('a hook-only card that dies mid-turn is eventually retired, and only then dismissable', async (t) => {
  ...
  const early = (await core.dismissSession('mid-turn')) as DismissResult;
  assert.equal(early.status, 409, 'sanity: a live-column card is not dismissable');

  await core.retentionSweep(now);

  const row = db.prepare<SessionRow>("SELECT * FROM sessions WHERE session_id = 'mid-turn'").get();
  assert.ok(row);
  assert.equal(
    row.col,
    'offline',
    'a pane-less card silent past the mid-turn horizon must be presumed ended',
  );
  assert.ok(row.ended_at, 'ended_at must be stamped');
  assert.equal(
    row.end_reason,
    'presumed',
    'the end is a GUESS — a late hook must be able to undo it',
  );

  const dismissed = (await core.dismissSession('mid-turn')) as DismissResult;
  assert.equal(
    dismissed.status,
    200,
    `the retired card must now be dismissable; got ${JSON.stringify(dismissed)}`,
  );
});
...
test('a mid-turn card inside its horizon is left alone (a long tool call is not death)', async (t) => {
  ...
  const row = db.prepare<SessionRow>("SELECT * FROM sessions WHERE session_id = 'busy'").get();
  assert.ok(row);
...
test('a late hook resurrects a mid-turn card that was presumed ended', async (t) => {
  ...
  await core.retentionSweep(now);
  assert.equal(
    db.prepare<SessionRow>("SELECT col FROM sessions WHERE session_id = 'slow'").get()?.col,
    'offline',
  );
  ...
  const row = db.prepare<SessionRow>("SELECT * FROM sessions WHERE session_id = 'slow'").get();
  assert.ok(row);
