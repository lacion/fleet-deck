...
async function startRT(t: TestContext, extraEnv: Record<string, string> = {}) {
...
test('REQUIRE_TOKEN=on: loopback exemption survives only for /health and the shell', async (t) => {
  ...
  assert.equal(
    spawn.status,
    401,
    'tokenless spawn must be refused even on loopback under the flag',
  );
  ...
  assert.equal(((await hook.json()) as HookAck).ok, true);
...
test('REQUIRE_TOKEN=on generates and persists a loopback token (0600) with no FLEETDECK_TOKEN', async (t) => {
  ...
  assert.equal(
    ...
    true,
    `token was not generated on loopback; stderr: ${raw.stderr}`,
  );
  ...
  const withTok = await fetch(`${baseUrl}/state`, {
...
test('REQUIRE_TOKEN=on: an ENV-supplied FLEETDECK_TOKEN is persisted to HOME/token (0600)', async (t) => {
  ...
  assert.equal(
    existsSync(tokenFile),
    true,
    'an env-supplied token must be persisted for file-only clients (fleet-watch)',
  );
  assert.equal(
    readFileSync(tokenFile, 'utf8').trim(),
    TOKEN,
    'the persisted file must hold the env-supplied token verbatim',
  );
...
test('REQUIRE_TOKEN=on + PROXY_AUTH=trust: a browser via the trusted proxy needs no token', async (t) => {
  ...
  const proxied = await rawGet(port, '/state', {
    Host: PROXY_HOST,
  ...
  assert.equal(
    proxied.status,
    200,
    'a trusted-proxy browser must not be gated by REQUIRE_TOKEN in trust mode',
  );
  ...
  assert.equal(
    proxiedNoOrigin.status,
    200,
    'the Host-based trusted-proxy signal must also stay exempt in trust mode',
  );
  ...
  assert.equal(
    plain.status,
    401,
    'plain loopback stays gated under REQUIRE_TOKEN even in trust mode',
  );
});
...
test('flag off (default): a loopback data route stays open with no token', async (t) => {
  ...
  assert.ok(Array.isArray(((await state.json()) as { sessions?: unknown }).sessions));
});
