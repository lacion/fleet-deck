// tests/smoke-harness-exhaustion.test.mjs
//
...
test('run-smoke.sh verifier treats harness exhaustion as inconclusive, not failure', (t) => {
  ...
  const run = (rcB) => runVerifier(renderVerifier({ demoLogs: tmp, sidA: 'sid-a', sidB: 'sid-b', rcA: 0, rcB }));
  ...
  let out = run(0);
  assert.equal(out.status, 0, 'healthy run must exit 0; stderr=' + out.stderr + ' stdout=' + out.stdout);
  assert.ok(out.stdout.includes('PASS: mail delivered at Stop boundary to both sessions'), out.stdout);
  ...
  writeFileSync(path.join(tmp, 'final-state.json'), JSON.stringify({
    ...baseState,
    ticker: [{ msg: 'alpha got fleet mail at the turn boundary' }],
  }));
  writeFileSync(path.join(tmp, 'worker-b.json'), JSON.stringify(cutOff));
  out = run(0);
  assert.equal(out.status, 0, 'harness-exhausted run must not fail; stderr=' + out.stderr + ' stdout=' + out.stdout);
  assert.ok(out.stdout.includes('INCONCLUSIVE'), 'expected INCONCLUSIVE classification; stdout=' + out.stdout);
  ...
  writeFileSync(path.join(tmp, 'worker-b.json'), JSON.stringify(healthy));
  out = run(124);
  assert.equal(out.status, 0, 'rc=124 run must not fail; stderr=' + out.stderr + ' stdout=' + out.stdout);
  ...
  writeFileSync(path.join(tmp, 'final-state.json'), JSON.stringify(baseState));
  writeFileSync(path.join(tmp, 'worker-b.json'), JSON.stringify({ is_error: true, subtype: 'error_during_execution' }));
  out = run(0);
