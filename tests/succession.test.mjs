...
async function clearInto(daemon, cwd, oldSid) {
  ...
  await postHook(daemon.baseUrl, 'SessionEnd', { session_id: oldSid, cwd, reason: 'clear' }, { token: daemon.token });
  const res = await postHook(daemon.baseUrl, 'SessionStart', { session_id: newSid, cwd, source: 'clear' }, { token: daemon.token });
...
test('pending mail and the ticket follow the card across a /clear', async (t) => {
  ...
  await postJson(`${daemon.baseUrl}/mail`, { to: ticketed, from: 'operator', text: 'read me after the clear' }, { token: daemon.token });
...
test('a straggler hook for the retired id never resurrects it — two cards under one callsign is the bug, not the cure', async (t) => {
  ...
  await postHook(daemon.baseUrl, 'PostToolUse', { session_id: sid, cwd, tool_name: 'Bash', tool_input: { command: 'echo late' } }, { token: daemon.token });
  await postHook(daemon.baseUrl, 'Notification', { session_id: sid, cwd, message: 'late notification' }, { token: daemon.token });
...
test('an unrelated new session in the same cwd is not swallowed as a continuation', async (t) => {
  ...
  await postHook(daemon.baseUrl, 'SessionStart', { session_id: stranger, cwd, source: 'startup' }, { token: daemon.token });
...
test('an ambiguous double-clear in one cwd refuses to merge — a wrong merge is worse than a spare card', async (t) => {
  ...
  await postHook(daemon.baseUrl, 'SessionStart', { session_id: heir, cwd, source: 'clear' }, { token: daemon.token });
...
test('the FORWARD /clear path refuses a lone orphan heir when TWO predecessors could claim it — a wrong merge is worse than a spare card', async (t) => {
  ...
  await postHook(daemon.baseUrl, 'SessionEnd', { session_id: c.sid, cwd, reason: 'clear' }, { token: daemon.token });
...
test('a healed card keeps the name the fleet knows — the throwaway never becomes the mail anchor', async (t) => {
  ...
  await postJson(`${healed.baseUrl}/mail`, { to: callsign, from: 'operator', text: 'the name everyone knows' }, { token: healed.token });
