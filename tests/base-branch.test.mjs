// tests/base-branch.test.mjs — the primary integration ref must be DERIVED,
// not guessed (BUG-113).
//
...
import { baseBranch } from '../scripts/fleetd/exec.mjs';
...
function makeLocalRepo(defaultBranch) {
...
test('no-remote repo with a custom default branch (trunk) resolves the base from the main worktree', async (t) => {
  ...
  assert.deepEqual(base, { ref: 'trunk', local: true },
...
test('agent worktree on a feature branch still resolves the trunk base', async (t) => {
  ...
  t.after(() => {
    try { git(['worktree', 'remove', '--force', wt], repo.dir); } catch { /* ignore */ }
  });
...
test('a main worktree in detached HEAD falls back to conventional local branches', async (t) => {
  ...
  const base = await baseBranch(repo.dir);
  assert.deepEqual(base, { ref: 'main', local: true },
