// tests/accept-plan-isolation.test.ts
//
...
test('gate 1 readiness is bound to the launched child, not to any answering daemon', () => {
  ...
  assert.ok(
    !gate1.includes('"$BASE/state"'),
    'gate 1 must not accept readiness from a bare /state',
  );
  assert.ok(gate1.includes('"$BASE/health"'), 'gate 1 must probe /health');
  assert.ok(
    gate1.includes('h.pid !== expectedPid'),
    'readiness must require /health.pid === DAEMON_PID',
  );
  ...
  assert.ok(
    src.includes('refusing to run the plan gate against an unowned listener'),
    'script must refuse to launch while the port still answers',
  );
  assert.ok(
    src.includes('aborting to avoid steering a foreign fleet'),
    'script must abort when the scratch child exits before becoming ready',
  );
});

test('a foreign listener that wins the port is never sent plan-gate mutations', async (t: TestContext) => {
  ...
  const portLine = /^FLEETDECK_PORT="\$\(node -e .+$/m.exec(original);
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    const tick = () => {
      ...
      if (Date.now() > deadline) {
        reject(new Error('script never launched its scratch fleetd'));
        return;
      }
      setTimeout(tick, 25);
    };
  ...
  const foreign = http.createServer((req, res) => {
    ...
    req.on('end', () => {
      if (req.url === '/health') {
        ...
        res.end(
          JSON.stringify({ spawn: { available: true }, sessions: [], questions: [], plans: [] }),
        );
  ...
  (foreign as http.Server & { exclusive?: boolean }).exclusive = true;
  t.after(
    () =>
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    foreign.listen({ port, host: '127.0.0.1', exclusive: true }, () => {
      resolve();
    });
  });
  ...
  const calls = fs
    ...
    .split('\n')
    .filter(Boolean)
    .map((line) => JSON.parse(line) as ForeignRequest);
  assert.equal(
