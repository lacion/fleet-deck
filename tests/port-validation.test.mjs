// tests/port-validation.test.mjs
//
...
test('resolvePort defaults to 4711 and accepts valid ports', () => {
  withPort(undefined, () => assert.equal(resolvePort(), 4711));
  withPort('', () => assert.equal(resolvePort(), 4711));
  withPort('1', () => assert.equal(resolvePort(), 1));
  withPort('65535', () => assert.equal(resolvePort(), 65535));
  withPort(String(randomPort()), () => assert.equal(resolvePort(), Number(process.env.FLEETDECK_PORT)));
});
...
test('resolvePort rejects port 0, whitespace, and out-of-range or non-numeric values', () => {
  for (const bad of ['0', ' ', '  0  ', '-1', '65536', '1.5', 'abc', 'NaN', 'Infinity']) {
    withPort(bad, () => {
      assert.throws(() => resolvePort(), /invalid FLEETDECK_PORT/, `FLEETDECK_PORT=${JSON.stringify(bad)} should be rejected`);
    });
  }
});
