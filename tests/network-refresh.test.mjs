// tests/network-refresh.test.mjs
//
...
const { createCore } = await import('../scripts/fleetd/derive.mjs');
...
function raw(port, { method = 'GET', path: reqPath = '/', headers = {} } = {}) {
...
function startBoard(t, { lan = null } = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    probe.listen(0, '127.0.0.1', () => {
      ...
      probe.close(() => {
        const handle = createHttp(core, { port, token: null, lan });
...
test('the Host allowlist follows the interface list as the network changes', async (t) => {
  ...
  assert.equal(refused.status, 403, 'an address the host does NOT have yet is a stranger and must stay refused');
  ...
  assert.equal(allowed.status, 200, 'a request via the host’s NEW LAN address must pass the Host wall');
...
test('refreshLan() swaps the share-panel URLs in one snapshot', async (t) => {
  ...
  const { port, refreshLan } = await startBoard(t, {
    lan: { enabled: true, urls: ['http://192.0.2.10:9/?t=old'], mdns: 'http://fleetdeck.local:9/?t=old' },
  });
  ...
  refreshLan({ enabled: true, urls: ['http://198.51.100.20:9/?t=new'], mdns: 'http://fleetdeck.local:9/?t=new' });
  ...
  assert.doesNotMatch(await lanOf(), /198\.51\.100\.20/, 'losing the LAN entirely collapses the panel to local-only');
});
...
test('the per-request allowlist refresh never evicts the advertised .local name', async (t) => {
  ...
  assert.equal(first.status, 200, 'the .local URL is the advertised entry point — its FIRST request must pass');
  const second = await raw(port, { path: '/state', headers: mdnsHost });
  assert.equal(second.status, 200, 'a refresh triggered by any earlier request must not evict the .local name');
