...
test('security headers: CSP on the shell, nosniff everywhere', async (t) => {
  ...
  await t.test('GET / (the HTML shell) carries both the CSP and nosniff', async () => {
    ...
    assert.equal(
      ...
      EXPECTED_CSP,
      'the shell CSP must match the pinned policy exactly',
    );
    assert.equal(
      res.headers.get('cache-control'),
      EXPECTED_SHELL_CACHE_CONTROL,
  ...
  await t.test(
    'GET /assets/<hashed> carries nosniff but no CSP (it is a subresource)',
    async () => {
      ...
      assert.equal(
        ...
        'nosniff',
        'every asset must carry nosniff',
      );
      assert.equal(
        res.headers.get('content-security-policy'),
        null,
        'CSP belongs on the document, not on each subresource',
      );
      assert.equal(
        res.headers.get('cache-control'),
        EXPECTED_ASSET_CACHE_CONTROL,
  ...
  await t.test('a JSON route (GET /health) carries nosniff', async () => {
    ...
    assert.equal(
      res.headers.get('x-content-type-options'),
      'nosniff',
      'the JSON API must carry nosniff too',
    );
