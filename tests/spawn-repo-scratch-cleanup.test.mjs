...
test('the failed-clone test cleans up its missing-origin scratch parent (BUG-212)', { timeout: 180_000 }, t => {
  ...
  // a vacuous pass. Strip it so the inner run is a real one.
  ...
  assert.match(out, /^# pass 1$/m, 'the inner run must actually execute (and pass) the focused test');
  const orphans = readdirSync(tmpRoot).filter(name => name.startsWith('fleetdeck-missing-origin-'));
