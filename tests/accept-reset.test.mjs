...
test('acceptance reset never signals a recycled legacy plain-PID pidfile entry (BUG-008)', async (t) => {
  ...
  writeFileSync(path.join(home, 'fleetd.pid'), `${sleeper.pid}\n`);

  const result = await runStop(home);
  assert.equal(result.code, 1,
    `an unverifiable live pid must be reported, not signalled (stderr: ${result.stderr})`);
  assert.match(result.stderr, /identity cannot be proven|cannot be verified|unowned|leaving the live process alone/,
    'the refusal must say WHY the pid was left alone');
  assert.equal(isAlive(sleeper.pid), true,
    'the recycled pid (an unrelated process) must survive the acceptance reset untouched');
});

test('acceptance reset refuses a live pidfile pid with a non-fleetd process shape', async (t) => {
  const home = scratchHome(t);
  const sleeper = spawnSleeper(t);
  await waitUntil(() => sleeper.pid != null, { label: 'sleeper pid' });
  ...
  writeFileSync(path.join(home, 'fleetd.pid'), JSON.stringify({ pid: sleeper.pid, port: randomPort() }));
...
test('acceptance reset stops a verified daemon: strict pidfile + /health.pid match + fleetd shape', async (t) => {
  const daemon = await startDaemon(); // strict JSON pidfile in daemon.home
  t.after(async () => { await daemon.stop(); });
  const healthPid = (await (await fetch(`${daemon.baseUrl}/health`)).json()).pid;
  ...
  await waitUntil(() => !isAlive(healthPid), { timeoutMs: 5000, label: 'daemon exit after verified SIGTERM' });
  assert.equal(isAlive(healthPid), false, 'the verified daemon must be signalled and actually exit');
});

test('acceptance reset treats dead/missing pidfile entries as nothing to do', async (t) => {
  const home = scratchHome(t);
