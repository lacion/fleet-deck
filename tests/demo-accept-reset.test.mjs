// tests/demo-accept-reset.test.mjs
//
...
function run(script, args, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    execFile('bash', [script, ...args], { timeout: 30000, ...options }, (error, stdout, stderr) => {
      if (error && error.killed) return reject(new Error(`${script} timed out`));
...
function resetHarness(t, scriptRel, body, { beforeGuard = '' } = {}) {
  ...
  assert.notEqual(cut, -1, `${scriptRel}: reset must end by refusing to kill an unidentified listener`);
  const prefix = lines.slice(0, cut + 3).map(line => { // through the guard's closing `fi`
    if (/^REAL_HOME=/.test(line)) return 'REAL_HOME="$HARNESS_HOME/real"';
    if (/^SCRATCH_HOME=/.test(line)) return 'SCRATCH_HOME="$HARNESS_HOME/scratch"';
    // plan/spawn hardcode FLEETDECK_PORT, so the port/BASE lines are rewritten
    // to honor the env override in every script.
    if (/^FLEETDECK_PORT=/.test(line)) return 'FLEETDECK_PORT="${FLEETDECK_PORT:-4711}"';
    if (/^BASE=/.test(line)) return 'BASE="http://127.0.0.1:$FLEETDECK_PORT"';
    return line;
  });
...
test('acceptance resets never kill by port: no fuser, no substring health grep', () => {
  for (const rel of ACCEPT_SCRIPTS) {
    ...
    assert.equal(/\bfuser\b/.test(executable), false, `${rel}: fuser kills every process associated with the port`);
    assert.equal(
      /curl[^|\n]*\/health[^|\n]*\|\s*grep -q '"ok"'/.test(executable),
      false,
      `${rel}: a substring health grep matches any body containing "ok" (including {"ok":false})`,
    );
    assert.match(source, /stop_identified_daemon\(\)/, `${rel}: reset must stop only positively identified daemons`);
  }
});
...
test('substring health grep accepts {"ok":false} — the old trigger was real', () => {
  ...
  return new Promise((resolve, reject) => {
    execFile('bash', ['-c', probe], (error) => {
      if (error) return reject(new Error('grep -q \'"ok"\' must match {"ok":false}'));
      resolve();
    });
  });
});
...
for (const rel of ACCEPT_SCRIPTS) {
  test(`reset harness extraction works: ${rel}`, async (t) => {
    ...
    // pidfile alone.
    const { dir, harnessPath } = resetHarness(t, rel, `
...
test('reset stops a positively identified daemon and never touches other port users', async (t) => {
  const { dir, harnessPath } = resetHarness(t, ACCEPT_SCRIPTS[0], `
  ...
  const client = spawn('node', ['-e', `
    ...
    setTimeout(() => {}, 30000);
  `], { stdio: 'ignore' });
  t.after(() => { try { client.kill('SIGKILL'); } catch {} });
  ...
  for (let i = 0; i < 50 && !healthOk; i += 1) {
    ...
    let record = null;
    try { record = JSON.parse(fs.readFileSync(pidfile, 'utf8')); } catch {}
    if (record?.pid !== daemon.pid || record.port !== port) continue;
    healthOk = await new Promise(resolve => {
      http.get({ host: '127.0.0.1', port, path: '/health' }, res => {
        let body = '';
        res.on('data', chunk => { body += chunk; });
        res.on('end', () => {
          try { resolve(JSON.parse(body).pid === daemon.pid); } catch { resolve(false); }
        });
...
test('reset refuses to signal an unidentified listener and aborts', async (t) => {
  ...
  assert.equal(out.code, 1, `reset must abort on an unidentified listener: ${out.stdout}${out.stderr}`);
  assert.match(out.stdout, /STOP1:2/, 'pidfile exists but names no health-proven daemon → refuse, no signal');
