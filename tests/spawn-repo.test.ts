...
// One JSONL record captured by the FLEETDECK_SPAWN_CMD fixture. Only `.parsed`
// (the launch spec the daemon handed the fixture) is read here.
interface SpecRecord {
  parsed: unknown;
}
// The single field of a launch spec these tests assert on.
interface ParsedSpec {
  cwd: string;
}
// POST /api/spawn accept/launch body (200 in-place launch, 202 provisioning).
interface SpawnBody {
  session_id: string;
  spawn_id: string;
  provisioning?: boolean;
}
// POST /api/spawn refusal body (4xx).
interface SpawnRejected {
  reason: string;
}
// POST /api/spawn 202 body carrying the synchronous clone echo.
interface CloneEcho {
  provisioning?: boolean;
...
interface SettingsView {
  settings: { repo_transport: { value: string; source: string } };
}
...
function scratch(prefix = 'fleetdeck-spawn-repo-'): string {
...
function countEvents(home: string, hookEvent: string): number {
  const db = openDb(path.join(home, 'fleetd.db'));
  try {
    const row = db
      .prepare<{ n: number }>('SELECT COUNT(*) AS n FROM events WHERE hook_event = ?')
      .get(hookEvent);
    return row?.n ?? 0;
  } finally {
    db.close();
...
function eventNotes(home: string, hookEvent: string): string[] {
  const db = openDb(path.join(home, 'fleetd.db'));
  try {
    return db
      .prepare<{ note: string | null }>('SELECT note FROM events WHERE hook_event = ?')
      .all(hookEvent)
      .map((row) => row.note ?? '');
  } finally {
    db.close();
...
async function cloneKilledDaemon(
  t: TestContext,
...
async function setup(
  t: TestContext,
  { branches = ['existing', 'remote-only'] }: { branches?: string[] } = {},
): Promise<{
  remote: RemoteRepo;
  root: string;
  reposDir: string;
...
test('repo mode in-place switches an existing branch and launches in that cwd', async (t) => {
  ...
  const spec = (
    await waitUntil(() => records(recordFile)[0], {
      timeoutMs: 12_000,
      label: 'spawn fixture record',
    })
  ...
  assert.ok(spawn, 'the card carries its spawn facet');
  assert.equal(spawn.requested_branch, 'existing');
  assert.equal(spawn.branch_mode, 'in-place');
  ...
  assert.equal(spawn.fail_detail, null, 'a spawn that did not fail carries no git-failure excerpt');
});
...
test('dirty in-place checkout is refused with file names', async (t) => {
  ...
  assert.match((response.json as SpawnRejected).reason, /dirty.*dirty\.txt/i);
});
...
test('in-place surfaces git stderr when the branch is checked out in another worktree', async (t) => {
  ...
  assert.match((response.json as SpawnRejected).reason, /already (used by worktree|checked out)/i);
});
...
test('clone provisioning returns 202 then launches from the cloned requested branch', async (t) => {
  ...
  assert.equal((response.json as SpawnBody).provisioning, true);
  const dest = path.join(reposDir, remote.repoName);
  await waitUntil(() => records(recordFile).length && existsSync(dest), {
    timeoutMs: 12_000,
    label: 'clone launch',
  });
...
function writeCloneShim(t: TestContext): string {
...
test('the note stays the distilled verdict while fail_detail carries the remedy', async (t) => {
  ...
  const response = await postJson(`${daemon.baseUrl}/api/spawn`, {
    repo: 'https://github.com/textemma/private-thing.git',
    branch: 'main',
    branch_mode: 'in-place',
  });
  assert.equal(response.status, 202, response.text);
  const card = await tombstonedCard(daemon, (response.json as SpawnBody).session_id);

  // The note is UNCHANGED in shape: one distilled line, no narration.
  const note = card.note;
  assert.ok(note, 'the tombstoned card carries a note');
  assert.match(note, /spawn failed: fatal: Could not read from remote repository\./);
  assert.doesNotMatch(note, /Cloning into/i);
  assert.doesNotMatch(
    note,
    /settings\/ssh\/new/,
    'the remedy does not belong in an 80-column note',
  );
  ...
  assert.ok(spawn, 'the card carries its spawn facet');
  const detail = spawn.fail_detail;
  assert.ok(detail, 'the card carries a fail_detail to expand');
  assert.match(
    detail,
    /https:\/\/github\.com\/settings\/ssh\/new/,
    'the URL to visit must reach the board',
  );
  assert.match(
    detail,
    /ssh-ed25519 AAAAC3NzaC1lZDI1NTE5/,
    'the key to paste there must reach it too',
  );
...
test('a credential in the clone URL reaches NO surface of the API', async (t) => {
  ...
  assert.ok(spawn, 'the card carries its spawn facet');
  const detail = spawn.fail_detail;
  assert.ok(detail, 'the card carries a fail_detail');
  assert.match(detail, /\[redacted\]@127\.0\.0\.1:1\/x\.git/);
  assert.match(detail, /Access denied for user \[redacted\]/);
  ...
  assert.ok(cardNote, 'the card carries a note');
  assert.equal(cardNote.includes(secret), false, 'the note must not carry the bare secret');
  assert.match(
    cardNote,
    /Access denied for user \[redacted\]/,
    'the note is hardened by the SAME pass as the detail',
  );
  ...
  for (const note of notes)
    assert.equal(note.includes(secret), false, 'no token in a durable event note');
});
...
test('repo mode rejects legacy worktree:true', async (t) => {
  ...
  assert.equal(
    (response.json as SpawnRejected).reason,
    'branch_mode replaces worktree in repo mode',
  );
});
...
test('a same-named checkout with no matching origin is not reused when the request names an origin', async (t) => {
  ...
  t.after(() => {
    rmSync(path.dirname(decoy), { recursive: true, force: true });
  });
  await postHook(
    daemon.baseUrl,
    'SessionStart',
  ...
  await waitUntil(() => records(recordFile).length && existsSync(dest), {
    timeoutMs: 12_000,
    label: 'clone-not-reuse',
  });
  const [record] = records(recordFile);
  assert.ok(record, 'the fixture captured a launch record');
  const spec = record.parsed as ParsedSpec;
...
test('repo mode rejects an unknown repo_host value', async (t) => {
  ...
  assert.equal(response.status, 400);
  assert.match((response.json as SpawnRejected).reason, /repo_host must be github or gitlab/i);
});
...
test('repo_host without repo is refused', async (t) => {
  ...
  assert.equal(response.status, 400);
  assert.match((response.json as SpawnRejected).reason, /repo_host requires repo/i);
});

test('repo_org validates and requires a repo', async (t) => {
  const daemon = await startDaemon();
  t.after(() => daemon.stop());
  const missing = await postJson(`${daemon.baseUrl}/api/spawn`, {
    repo_org: 'textemma',
    branch: 'main',
  });
  assert.equal(missing.status, 400);
  assert.match((missing.json as SpawnRejected).reason, /repo_org requires repo/i);
  ...
  assert.match((bad.json as SpawnRejected).reason, /default org/i);
});
...
test('repo_host gitlab shorthand resolves a gitlab.com clone origin (ssh by default)', async (t) => {
  ...
  const response = await postJson(`${daemon.baseUrl}/api/spawn`, {
    repo: 'mygroup/mysub/myproj',
    branch: 'main',
    branch_mode: 'in-place',
    repo_host: 'gitlab',
  });
  assert.equal(response.status, 202, response.text);
  const clone = response.json as CloneEcho;
  assert.equal(clone.provisioning, true);
  assert.equal(clone.clone.origin_url, 'git@gitlab.com:mygroup/mysub/myproj.git');
  assert.equal(clone.clone.dest, path.join(reposDir, 'myproj'));
});
...
test('explicit repo_transport composes the echoed origin (ssh + https) on a fresh daemon', async (t) => {
  ...
  const sshClone = ssh.json as CloneEcho;
  assert.equal(sshClone.clone.origin_url, 'git@github.com:org/alpha.git');
  assert.equal(sshClone.clone.dest, path.join(reposDir, 'alpha'));
  ...
  assert.equal((https.json as CloneEcho).clone.origin_url, 'https://github.com/org/beta.git');
});
...
test('absent repo_transport on a fresh daemon composes an ssh origin (the new default)', async (t) => {
  ...
  assert.equal((res.json as CloneEcho).clone.origin_url, 'git@github.com:org/repo.git');
});
...
test('persisted subgroup default org infers gitlab for a bare-name HTTPS API spawn', async (t) => {
  ...
  const set = await postJson(`${daemon.baseUrl}/api/settings`, {
    repo_default_org: 'group/subgroup',
    repo_transport: 'https',
  });
  ...
  assert.equal(res.status, 202, res.text);
  const clone = res.json as CloneEcho;
  assert.equal(clone.clone.origin_url, 'https://gitlab.com/group/subgroup/module.git');
  assert.equal(clone.clone.dest, path.join(reposDir, 'module'));
});
...
test('absent repo_transport honors the persisted https setting', async (t) => {
  ...
  assert.equal(res.status, 202, res.text);
  assert.equal((res.json as CloneEcho).clone.origin_url, 'https://github.com/org/repo.git');
});
...
test('an explicit transport on a shorthand spawn persists as an override', async (t) => {
  ...
  assert.equal(
    (before.json as SettingsView).settings.repo_transport.source,
    'default',
    'a fresh daemon has no override',
  );
  ...
  const explicitTransport = (afterExplicit.json as SettingsView).settings.repo_transport;
  assert.equal(explicitTransport.source, 'override');
  assert.equal(explicitTransport.value, 'https');
  ...
  const absentTransport = (afterAbsent.json as SettingsView).settings.repo_transport;
  assert.equal(absentTransport.value, 'https');
  assert.equal(absentTransport.source, 'override');
});
...
test('an absent transport never creates a transport override', async (t) => {
  ...
  const transport = (got.json as SettingsView).settings.repo_transport;
  assert.equal(transport.source, 'default');
  assert.equal(transport.value, 'ssh');
});
...
test('a 409-refused explicit transport never rewrites the remembered setting', async (t) => {
  ...
  assert.match((second.json as SpawnRejected).reason, /already being provisioned/i);
  ...
  await tombstonedCard(daemon, (first.json as SpawnBody).session_id);
  await waitUntil(
    () => {
      try {
        execFileSync('pgrep', ['-f', 'FD_BUG177_FIXTURE=1'], {
          stdio: ['ignore', 'pipe', 'ignore'],
        });
        return null; // fixture tree still alive
      } catch {
        return true;
      } // pgrep exit 1: nothing left
    },
...
test('repo mode rejects an unknown repo_transport value', async (t) => {
  ...
  assert.match((res.json as SpawnRejected).reason, /repo_transport must be ssh or https/i);
});
...
test('repo_transport without repo is refused', async (t) => {
  ...
  assert.equal(res.status, 400);
  assert.match((res.json as SpawnRejected).reason, /repo_transport requires repo/i);
});
...
test('2.3: a credentialed URL thrown past the spawn guard still answers a bounded redacted 500', async (t) => {
  ...
  const cardNote = card.note;
  assert.ok(cardNote, 'the banner has a headline note');
  assert.equal(cardNote.includes(secret), false, 'the banner headline (the note) is hardened');
  const spawn = card.spawn;
  assert.ok(spawn, 'the card carries its spawn facet');
  const detail = spawn.fail_detail;
  assert.ok(detail, 'the banner has a detail to expand');
  assert.equal(detail.includes(secret), false, 'the expanded detail is hardened');
